...
let report ?(scenario="all")?(checkpoints=[1;2;60;600])?(created=2)?(destroyed=2)?(ordered=true) target=
  let samples=List.init 181 sample in let samples=if ordered then samples else List.rev samples in
  `Assoc["schema",`Int 1;"qualification",`String"R12-final-facade";"target",`String target;"scenario",`String scenario;
    "duration_seconds",`Float 1800.;"frames",`Int 10000;"checkpoints",`List(List.map(fun frame->`List[`Int frame;`String"hash"])checkpoints);
    "deterministic_hash",`String"duration-dependent";"sample_capacity",`Int 256;"sample_every_seconds",`Float 10.;
    "sample_observations",`Int 181;"samples",`List samples;"rss_limit_percent",`Float 5.;"created_resources",`Int created;
