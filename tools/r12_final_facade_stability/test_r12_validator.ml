let sample ?(runtime_resources=3) ?(cache_entries=4) ?(pending=0) index=`Assoc["frame",`Int(index*100+1);"elapsed_seconds",`Float(float(index*10));"rss_kib",`Int 1000;"heap_words",`Int 1;"resource_count",`Int 2;"runtime_resource_count",`Int runtime_resources;"cache_entries",`Int cache_entries;"release_queue_pending",`Int pending]
let report ?(scenario="all")?(checkpoints=[1;2;60;600])?(created=2)?(destroyed=2)
  ...
  let samples=List.init 181(sample~runtime_resources~cache_entries~pending)in
  ...
  `Assoc["schema",`Int 1;"qualification",`String"R12-final-facade";"target",`String target;"scenario",`String scenario;
    ...
    "release_queue_pending_after_teardown",`Int 0;
    "release_queue_counter_supported",`Bool true;
    "metal_live_handles_before",`Int 4;
    "metal_live_handles_after",`Int(if metal_balanced then 4 else 5);
    "metal_total_created_before",`Intlit"10";
    "metal_total_created_after",`Intlit"14";
    "metal_total_released_before",`Intlit"6";
    "metal_total_released_after",`Intlit"10";
...
let ()=let validator=Sys.argv.(1)and directory=Filename.get_temp_dir_name()in
  let native=write directory"r12-validator-native.json"(report"native")in
  run validator[native]true;
  ...
  run validator[native;native]false;
  List.iter(fun path->try Sys.remove path with Sys_error _->())[native];
