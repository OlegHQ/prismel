...
let source_snapshot()=match command_output"git"[|"git";"rev-parse";"HEAD"|],
  command_output"git"[|"git";"status";"--porcelain=v1";"--untracked-files=all"|]with
  |Some commit,Some status->Some(commit,status="")|_->None
...
let canvas_stats_json(stats:Prismel.Canvas.Private.native_stats)=`Assoc[
  ...
  "submissions",`String(Int64.to_string stats.logical_submissions);
  ...
  |Some(before:Prismel.Canvas.Private.native_stats),
   Some(after:Prismel.Canvas.Private.native_stats)->
      ...
      `Assoc["frames",`String(delta after.frames before.frames);
        ...
        "passes",`String(delta after.logical_passes before.logical_passes);
        "submissions",`String(delta after.logical_submissions before.logical_submissions);
        "uploaded_bytes",`String(delta after.uploaded_bytes before.uploaded_bytes);
        "cache_entries",`Int after.cache_entries]
  |None,None->`Null|_->failwith"R10 Canvas offscreen stats disappeared"
let metal_device_facts()=
  ...
  Fun.protect~finally:(fun()->ignore(Metal.Device.destroy device))(fun()->
    ...
    `Assoc["selection",`String"Metal.Device.system_default";
      "name",`String info.name;"registry_id",`String(Int64.to_string info.registry_id);
...
let metal_release_facts(stats:Metal.Release_queue.stats)=`Assoc[
  "pending",`Int stats.pending;"dropped",`Int stats.dropped;
  "live_handles",`Int stats.live_handles;
  "total_created",`String(Int64.to_string stats.total_created);
  "total_released",`String(Int64.to_string stats.total_released);
  "external_deallocations",`String(Int64.to_string stats.external_deallocations);
  "external_deallocation_mismatches",
    `String(Int64.to_string stats.external_deallocation_mismatches)]
...
let create_renderer counters visibility width height =
  sdl"init"(Sdl3.Init.init[Sdl3.Init.Video]);
  let window=sdl"window"(Sdl3.Window.create~title:"native benchmark"~width~height~flags:[Hidden;Metal]())in
  ...
  let device=get(Ogpu_metal_native.Device.system_default())in let layer=metal(Metal.Metal_layer.adopt_borrowed(Ogpu_metal_native.Device.Private.metal device)token(Metal.Metal_layer.default~width:64~height:64))in
  let driver,control=Ogpu_metal_native.Backend.create~device~layer()in let driver=instrument counters driver in if Scene_execution.pipeline_variants_per_sample<>48 then failwith"pipeline family/blend cardinality drift";let cache=get(Ogpu_metal_native.Pipeline.create_cache~capacity:(Scene_execution.pipeline_variants_per_sample*4))in
  let make backend family blend samples=let source=match family with Scene_execution.Scene2->scene2_source|Scene2_textured|Scene3|Scene3_points|Scene3_textured|Scene3_shadow|Scene3_stencil|Scene3_textured_stencil|Scene3_shadow_stencil|Ui->scene3_source in let artifact entry stage bindings=get(Ogpu.Shader.create{backend="metal";label=Some"native-benchmark";bytes=Bytes.of_string source;entry_points=[{name=entry;stage}];bindings})in let vertex=artifact"scene_vertex"Vertex[{group=0;binding=0;kind=Storage_buffer;visibility=[Vertex]}]and fragment=artifact"scene_fragment"Fragment[]in let layout0=get(Ogpu.Binding.create_layout[{binding=0;kind=Buffer;visibility=[Vertex]}])in let layout=get(Ogpu.Binding.create_pipeline_layout~device:(Ogpu.Backend.device_handle backend)~capabilities:(Ogpu.Backend.capabilities backend)[0,layout0])in let depth_format=match family with Scene_execution.Scene2|Scene2_textured|Ui->Ogpu.Pipeline.No_depth|Scene3|Scene3_points|Scene3_textured|Scene3_shadow->Depth32_float|Scene3_stencil|Scene3_textured_stencil|Scene3_shadow_stencil->Depth32_float_stencil8 in let descriptor:Ogpu.Pipeline.render_descriptor={backend="metal";label=Some"native-benchmark";layout;vertex;vertex_entry="scene_vertex";fragment=Some fragment;fragment_entry=Some"scene_fragment";color_format=Rgba8_unorm;depth_format;sample_count=samples}in match Ogpu_metal_native.Pipeline.create_render_runtime_msl~blend cache device descriptor with Error _ as e->e|Ok native->Ogpu_metal_native.Backend.register_pipeline control native;Ok(Ogpu_metal_native.Pipeline.Private.portable native)in
  let config:Ogpu.Surface.configuration={logical_width=width;logical_height=height;physical_width=width;physical_height=height;format=Bgra8_unorm;present_mode=Fifo;max_acquired=2}in
  let renderer=get(Scene_execution.create_with_sampled_pipeline_variants driver config~before_device_destroy:(fun()->Ogpu_metal_native.Pipeline.clear_cache cache;Result.map_error(fun e->Ogpu.Error.make"native-benchmark"Ogpu.Error.Invalid_state(Format.asprintf"%a"Metal.pp_error e))(Metal.Metal_layer.destroy layer))make)in
...
let run_public selected warmup_seconds samples sample_seconds visibility width height =
  ...
  let release_before=metal(Metal.Release_queue.stats())in
  let metal_device=metal_device_facts()in
  ...
  let render,render_canonical,capture,stats,presentation_facts,set_visibility,
      observed_visibility,setup_canvas_stats,canvas_stats,destroy=match selected with
  |Basic|Pxui|Canvas->
      ...
      (fun()->candidate.setup_canvas_stats),
  ...
  |Scene3->
      ...
      let prepared=match staged.scene3 with
        ...
        |_->failwith"R10 Scene3 must lower to one 12-draw public native scene"in
      Array.iter(fun(entry:Scene_execution.scene3_entry)->
        if Bytes.length entry.draw.mesh.vertices<>entry.draw.mesh.vertex_count*68
           ||Option.fold~none:true~some:(fun bytes->Bytes.length bytes<>5456)
             entry.draw.state.transform_uniforms then
          failwith"R10 Scene3 did not use the production 68-byte/uniform lowering")
        prepared.entries;
      let family=function Scene_execution.Scene3->Prismel_next_execution.Scene3|Scene3_points->Scene3_points
        ...
        |Scene2->Scene2|Scene2_textured->Scene2_textured|Ui->Ui in
      let blend=function Ogpu.Pipeline.Replace->Prismel_next_execution.Replace
        |Alpha->Alpha|Add->Add|Multiply->Multiply|Screen->Screen|Subtract->Subtract in
      let scene3_draw(entry:Scene_execution.scene3_entry)=
          Prismel_next_execution.prepared_draw~family:(family entry.family)
            ~blend:(blend entry.blend)?texture:entry.texture
            ?auxiliary:entry.auxiliary~samples:entry.samples entry.draw in
      ...
      let draws=List.concat_map(function
        ...
        |Ui_layer _->invalid_arg"native benchmark scenarios contain no PXUI layers")
        staged.layers in
  ...
  let phase0_frame()=
    ...
    let interval=started-. !last_frame_started in
    last_frame_started:=started;
    ignore(execution(render()));
    let target=1./.60. in
  ...
  let run_for duration collect =
    ...
    let rec loop count =
      ...
      if count>0&&now-.started>=duration then
        Option.fold~none:[||]~some:(fun values->Array.sub values 0 count)values,
        now-.started
      else let interval=phase0_frame()in
        let completed=Sdl3.Time.monotonic_seconds()in if collect then sample_rss completed;
        Option.iter(fun values->if count>=capacity then
          failwith"R10 Phase0 frame sample capacity exceeded"else
          values.(count)<-interval)values;
        loop(count+1)
    in loop 0 in
  ...
  ignore(execution(render_canonical()));let canonical_framebuffer=Result.get_ok(capture())in
  ignore(Result.get_ok(destroy()));
  ignore(metal(Metal.Release_queue.drain()));
  let release_after=metal(Metal.Release_queue.stats())in
  ...
  let json=`Assoc["schema",`Int 1;
    "benchmark_identity",`String"runtime-next-production-public-r10";
    "scenario",`String(scenario_name selected);"backend",`String"real-m1-runtime-next-metal";
    "profile",`String Runtime_next_native_profile.value;
    ...
    "warmup_seconds",`Float warmup_seconds;"sample_frames",`Int measured;"acceptance_cook",`Null;
    ...
    "canonical_pixel_source",`String("candidate-stability-frame-1/"^protocol_scenario_name selected);
    "pixel_source",`String("runtime-next-native"^(match visibility with Visible->""|Hidden->"-hidden")^"/"^protocol_scenario_name selected);"pixel_tolerance",`Int 3;
    ...
    "offscreen_setup",Option.fold~none:`Null~some:canvas_stats_json(setup_canvas_stats());
    "offscreen_measurement",canvas_stats_delta canvas_before canvas_after;
    "metal_device",metal_device;
    "metal_lifetime",`Assoc["before",metal_release_facts release_before;
      "after",metal_release_facts release_after];
...
let ()=let selected=ref Basic and warmup=ref 5 and warmup_seconds=ref None and samples=ref 30 and sample_seconds=ref None and report=ref None and artifact_path=ref None and visibility=ref Hidden and width=ref 64 and height=ref 64 and protocol_r9=ref false in Arg.parse["--warmup",Arg.Set_int warmup,"frames";"--warmup-seconds",Arg.Float(fun x->warmup_seconds:=Some x),"duration";"--samples",Arg.Set_int samples,"frames";"--sample-seconds",Arg.Float(fun x->sample_seconds:=Some x),"duration";"--width",Arg.Set_int width,"logical width";"--height",Arg.Set_int height,"logical height";"--report",Arg.String(fun x->report:=Some x),"path";"--acceptance-artifact",Arg.String(fun x->artifact_path:=Some x),"cooked shattered artifact";"--visibility",Arg.Symbol(["visible";"hidden"],fun x->visibility:=if x="visible"then Visible else Hidden),"window visibility";"--protocol-r9",Arg.Set protocol_r9,"exact 600-frame stable camera-only upload protocol"](fun x->selected:=parse x)"runtime_next_native_benchmark scenario";if !warmup<1|| !samples<1|| !width<=0|| !height<=0||Option.fold~none:false~some:(fun x->x<=0.)!warmup_seconds||Option.fold~none:false~some:(fun x->x<=0.)!sample_seconds then invalid_arg"counts or dimensions";
  ...
  let renderer,window,cleanup=create_renderer counters!visibility !width !height in
  ...
  let work=match!selected with Scene3->let canonical=R10_scene3_legacy_equivalent.create~width:!width~height:!height in List.map(fun draw->Scene_execution.Scene3,Ogpu.Pipeline.Replace,draw)canonical.native_batched_draws|_->workload ?acceptance!selected in
  let prepared=List.map(fun(family,blend,draw):Scene_execution.sampled_draw->
    {family;blend;texture=None;auxiliary=None;
     samples=(if !selected=Scene3 then 4 else 1);draw})work in
  ...
  let sketch_invocation=match Sys.getenv_opt"PRISMEL_R11_SKETCH_EXECUTABLE",Sys.getenv_opt"PRISMEL_R11_ARTIFACT"with
  |Some invoked_executable,Some artifact->`Assoc[
      "status",`String"graph-validated-renderer-delegated";
      "evidence_class",`String"precursor";
      "frozen_r11_closure",`Bool false;
      "invoked_executable",`String invoked_executable;
      "measured_executable",`String Sys.executable_name;
      "artifact",`String artifact]
  |_->`Assoc["status",`String"not-invoked";"evidence_class",`String"none";
      "frozen_r11_closure",`Bool false;
      "invoked_executable",`Null;
      "measured_executable",`String Sys.executable_name;
      "artifact",`Null]in
  let json=`Assoc["schema",`Int 1;"scenario",`String(scenario_name!selected);"backend",`String"real-m1-runtime-next-metal";"profile",`String Runtime_next_native_profile.value;"visibility",`String(match!visibility with Visible->"visible"|Hidden->"hidden");"protocol_r9_requested",`Bool!protocol_r9;"camera_only_changes",`Bool!protocol_r9;"protocol_r11_requested",`Bool!protocol_r11;"r11_sketch_invocation",sketch_invocation;"source_before",source_json source_before;"source_after",source_json source_after;"source_stable_clean",`Bool source_stable;"window",`Assoc["pixel_density",`Float window.pixel_density;"display_scale",`Float window.display_scale;"drawable_width",`Int window.drawable_width;"drawable_height",`Int window.drawable_height;"refresh_hz",`Null;"power_state",`Null;"thermal_state",`Null];"warmup_frames",`Int!warmup;"sample_frames",`Int!samples;"pieces",`Int pieces;"triangles",`Int triangles;"acceptance_cook",acceptance_json;"wall_seconds",`Float total;"user_seconds",`Float(cpu1.tms_utime-.cpu0.tms_utime);"system_seconds",`Float(cpu1.tms_stime-.cpu0.tms_stime);"median_ms",`Float(1000.*.percentile 0.5 walls);"p95_ms",`Float(1000.*.percentile 0.95 walls);"p99_ms",`Float(1000.*.percentile 0.99 walls);"fps",`Float(float!samples/.total);"cpu_percent",`Float(100.*.cpu/.total);"allocated_bytes",`Float allocated;"promoted_bytes",`Float promoted;"rss_kib",`Int rss;"prepared_upload_bytes",`String(Int64.to_string upload0);"measurement_upload_bytes",`String(Int64.to_string(Int64.sub upload1 upload0));"native_buffer_creates",`Int native_creates;"native_buffer_create_bytes",`String(Int64.to_string native_create_bytes);"native_buffer_writes",`Int native_writes;"native_buffer_write_bytes",`String(Int64.to_string native_write_bytes);"native_mesh_buffer_creates",`Int native_mesh_creates;"native_mesh_buffer_create_bytes",`String(Int64.to_string native_mesh_create_bytes);"native_mesh_buffer_writes",`Int native_mesh_writes;"native_mesh_buffer_write_bytes",`String(Int64.to_string native_mesh_write_bytes);"draws",`Int(counters.draws-draws0);"passes",`Int(counters.render_passes-passes0);"backend_calls",`Int(counters.submissions-submits0);"cache_entries",`Int cache;"cache_hits_inferred",`Int hits;"cache_misses_observed",`Int misses;"width",`Int!width;"height",`Int!height;"workload_signature",`String(R10_scene2_legacy_equivalent.describe R10_scene2_legacy_equivalent.Scene3~width:!width~height:!height).semantic_signature;"work_units",`Int triangles;"semantics_supported",`Bool true;"scheduling",`String(match!sample_seconds with Some _->"duration-bounded"|None->"frame-count");"scheduled_frame_rate",`Null;"framebuffer_digest",`String(Digest.to_hex(Digest.bytes framebuffer));"pixel_authority",`String("phase0/runtime-next-native"^(match!visibility with Visible->""|Hidden->"-hidden")^"/scene3");"pixel_tolerance",`Int 3;"native_gpu_counters",native_gpu_counters ~supported:gpu1.supported ~duration:(gpu1.duration_seconds-.gpu0.duration_seconds) ~samples:(Int64.sub gpu1.sample_count gpu0.sample_count) ~wall:total;"machine",`Assoc["arch",`String(Sys.getenv_opt"HOSTTYPE"|>Option.value~default:"arm64");"ocaml",`String Sys.ocaml_version]]in let text=Yojson.Safe.pretty_to_string json^"\n"in match!report with None->print_string text|Some path->let out=open_out_bin path in output_string out text;close_out out
