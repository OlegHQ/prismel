...
let create_renderer counters visibility width height =
  ...
  let renderer=get(Scene_execution.create_with_sampled_pipeline_variants driver config~before_device_destroy:(fun()->Ogpu_metal.Pipeline.clear_cache cache;Result.map_error(fun e->Ogpu.Error.make"native-benchmark"Ogpu.Error.Invalid_state(Format.asprintf"%a"Metal.pp_error e))(Metal.Metal_layer.destroy layer))make)in
