...
let ()=let selected=ref Basic and warmup=ref 5 and samples=ref 30 and report=ref None and artifact_path=ref None in Arg.parse["--warmup",Arg.Set_int warmup,"frames";"--samples",Arg.Set_int samples,"frames";"--report",Arg.String(fun x->report:=Some x),"path";"--acceptance-artifact",Arg.String(fun x->artifact_path:=Some x),"cooked shattered artifact"](fun x->selected:=parse x)"runtime_next_native_benchmark scenario";if !warmup<1|| !samples<1 then invalid_arg"counts";
  let counters={buffer_creates=0;buffer_bytes=0L;submissions=0;render_passes=0;draws=0}in
  let renderer,cleanup=create_renderer counters in
