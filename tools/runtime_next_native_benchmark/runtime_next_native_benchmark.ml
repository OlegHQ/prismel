...
let native_gpu_counters = `Assoc[
  "status",`String"unsupported";
  "gpu_duration",`Null;
  "gpu_utilization",`Null;
  "reason",`String"Metal timestamp query primitives are available conditionally, but Scene_execution exposes no insertion/resolution boundary around its measured render submissions; GPU utilization has no authoritative in-process API";
]
...
let run_public selected warmup_seconds samples sample_seconds visibility width height =
  ...
  let json=`Assoc["schema",`Int 1;"scenario",`String(scenario_name selected);"backend",`String"real-m1-runtime-next-metal";"profile",`String"release";
    ...
    "native_gpu_counters",native_gpu_counters;"machine",`Assoc["arch",`String(Sys.getenv_opt"HOSTTYPE"|>Option.value~default:"arm64");"ocaml",`String Sys.ocaml_version]]in
