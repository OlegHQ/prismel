  ...
  let create_device () =
    Result.map
      (fun (device : Ogpu.Backend.driver_device) ->
        let create_buffer memory (descriptor : Ogpu.Types.buffer_descriptor) =
          let mesh =
          ...
          Result.map
            ...
            (device.create_buffer memory descriptor)
        in
        let create_queue () =
          Result.map
            (fun (queue : Ogpu.Backend.driver_queue) ->
              { queue with
                begin_commands =
                  (fun () ->
                    Result.map
                      (fun (commands : Ogpu.Backend.driver_commands) ->
                        { commands with
                          render_encoder =
                            (fun sampling target ->
                              counters.render_passes <- counters.render_passes + 1;
                              Result.map
                                (fun (encoder : Ogpu.Backend.driver_render_encoder) ->
                                  { encoder with
                                    draw =
                                      (fun ~primitive ~first ~count ~instances ->
                                        counters.draws <- counters.draws + 1;
                                        encoder.draw ~primitive ~first ~count ~instances);
                                    draw_indexed =
                                      (fun ~primitive ~index_type buffer ~offset ~count ~instances ->
                                        counters.draws <- counters.draws + 1;
                                        encoder.draw_indexed ~primitive ~index_type buffer ~offset
                                          ~count ~instances);
                                    draw_batch =
                                      (fun draws ->
                                        counters.draws <- counters.draws + Array.length draws;
                                        encoder.draw_batch draws);
                                    execute_icb =
                                      (fun icb ~location ~length ->
                                        counters.draws <- counters.draws + length;
                                        encoder.execute_icb icb ~location ~length);
                                  })
                                (commands.render_encoder sampling target));
                          commit =
                            (fun () ->
                          ...
                          commit_present =
                            (fun ~source frame ->
                              counters.submissions <- counters.submissions + 1;
                              commands.commit_present ~source frame);
                        })
                      (queue.begin_commands ()))
              })
            (device.create_queue ())
        in
