...
Its R11-only entrypoint lives in `r11_benchmark.ml` and `r11_native.ml` here;
the sketch calls it only when the protocol supplies an artifact. It cooks the
sketch's own graph, validates the 18,278-piece
