...
let () =
  ...
  let pending = symbols |> List.filter (fun symbol -> string "classification" symbol = "unreviewed") |> List.map (string "id") |> S.of_list in
  let ids = List.map (fun (item : Binding_pure_tail_plan.item) -> item.id) Binding_pure_tail_plan.items in
  if List.exists (fun id -> not (S.mem id pending)) ids then fail "pure-tail ID escaped pending inventory";
