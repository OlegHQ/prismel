...
let validate selection =
  List.iter (fun value ->
    if value.getter.classification <> "unreviewed" then invalid_arg ("Metal acceleration getter classification drift: " ^ value.getter.id);
    Option.iter (fun setter -> if setter.classification <> "unreviewed" then invalid_arg ("Metal acceleration setter classification drift: " ^ setter.id)) value.setter;
