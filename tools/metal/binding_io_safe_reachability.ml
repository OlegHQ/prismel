...
let validate()=
 ...
 if List.exists(fun id->not(List.mem id Binding_io_counter_manifest.ids)&&not(List.mem id constructors))promotable_ids then failwith"IO safe ID escaped IO/counter111"
let ()=validate()
