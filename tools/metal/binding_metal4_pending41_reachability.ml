let promoted_ids = Binding_metal4_native32_reachability.promotable_ids @ Binding_metal4_final9_reachability.promotable_ids
...
let validate () =
  if List.length promoted_ids <> 39 || List.length pending_ids <> 2 || List.length archive_noncallable_ids <> 4
