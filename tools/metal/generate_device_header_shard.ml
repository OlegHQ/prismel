...
let ()=
 ...
 let declarations=symbols|>List.filter(fun j->Yojson.Safe.Util.(j|>member"classification"|>to_string="unreviewed"&&List.mem(j|>member"header"|>to_string)Binding_device_header_plan.headers&&not(List.mem(j|>member"id"|>to_string)Binding_device_header_plan.excluded_ids)))|>List.map declaration in
 ...
 Printf.printf"MTLDevice/type: %d exact IDs (%d mechanical, %d ownership, %d metadata)\n"
