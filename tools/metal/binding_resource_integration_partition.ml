...
let () =
  validate ();
  if List.length safe_ownership_tail <> 38 then failwith "resource safe ownership38 drift";
  if List.length handwritten_ownership <> 18 then failwith "resource handwritten18 drift"
