...
let item id =
  ...
  ; status = if promotable then Promotable else Blocked "no committed safe public/test closure" }

let items = List.map item Binding_render_encoder_manifest.ids
let blocked = List.filter (fun item -> match item.status with Blocked _ -> true | Promotable -> false) items
...
let validate () =
  ...
  if List.length promotable_ids <> 37 || List.length blocked <> 65 then
