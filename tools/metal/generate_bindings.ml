...
let validate_direct_method inventory
  ...
  let expected_classification =
    if Binding_direct_plan.is_safe_device_identifier entry.sdk_id
       ...
       || List.mem entry.sdk_id Binding_compute_encoder35_safe_closure.callable_ids
    then "bound"
    else "unreviewed"
  in
...
let generator_source_paths =
  ...
  ; "tools/metal/binding_metal4_native32_reachability.ml"
  ...
  ; "tools/metal/binding_io_safe_reachability.ml"
  ; "tools/metal/binding_io_safe_reachability.mli"
