...
let ()=
 let claimed=Binding_metal4_manifest.ids in
 List.iter(fun(name,ids)->if overlap claimed ids<>[]then failwith("overlap "^name))
  ["resource100",Binding_resource_manifest.ids;"presentation125",Binding_presentation_manifest.ids;
   ...
   "command121",Binding_command_support_manifest.ids];
 let forbidden=["MTL4AccelerationStructure";"MTLDevice";"MTLTensor";"MTLRasterizationRate";
  "CAMetalLayer";"MTLMeshRenderPipelineDescriptor";"MTLTileRenderPipelineDescriptor"]in
 ...
 if List.length claimed<>190 then failwith"count drift";
 print_endline"metal4 batch190 globally disjoint from every prepared manifest"
