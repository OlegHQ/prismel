...
let safe_public_ids=List.sort_uniq String.compare(pending_generated_ids@already_bound_ids)
let blocked_ids=
...
let validate()=
 let counts = List.length pending_generated_ids,List.length already_bound_ids,List.length safe_public_ids,List.length blocked_ids in
