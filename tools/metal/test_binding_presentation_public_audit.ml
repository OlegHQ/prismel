let () =
  ...
  if List.length A.selector_reachable <> 26 then failwith "presentation selector closure drift";
  if List.length A.property_companions <> 17 then failwith "presentation companion closure drift";
  if List.length A.safe_reachable <> 43 then failwith "presentation safe closure drift";
  if List.length A.missing_public <> 82 then failwith "presentation public gap drift";
