...
let pending_items =
  List.map make Binding_presentation_callable36_reachability.callable_ids
...
let private_metadata_ids =
  Binding_presentation_callable36_reachability.private_metadata_ids
...
let validate () =
  ...
  if List.length pending_items <> 81
     || List.sort_uniq String.compare ids
        <> Binding_presentation_callable36_reachability.callable_ids
     || List.exists
