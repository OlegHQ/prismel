...
let () =
  ...
  let ids = symbols |> List.filter (fun symbol -> string "classification" symbol = "unreviewed" && string "header" symbol = "Metal/MTLParallelRenderCommandEncoder.h") |> List.map (string "id") in
