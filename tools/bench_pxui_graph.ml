...
let measure count =
  ...
  let edge_samples = Array.make repeats 0. in
  Gc.full_major ();
  let before = Gc.allocated_bytes () in
  let max_edge_candidates = ref 0 in
  ...
  Printf.printf
    "pxui_graph,%d,%d,%d,%.9f,%.0f,%.9f,%.0f,%.9f,%.0f,%d,%.9f,%.0f,%d,%d,%d,%d\n%!"
    count stats.nodes stats.wires (percentile edge_samples 0.99) allocated
    ...
    !max_edge_candidates stats.spatial_cells
    stats.spatial_edge_cells

let () =
  Printf.printf
    "benchmark,requested_nodes,nodes,wires,edge_p99_seconds,allocated_bytes,move_median_seconds,move_10_allocated_bytes,release_seconds,release_allocated_bytes,bulk_nodes,bulk_release_seconds,bulk_release_allocated_bytes,visible_nodes,max_edge_candidates,spatial_cells,spatial_edge_cells\n%!";
