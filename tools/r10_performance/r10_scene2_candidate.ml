...
let create ~target:target_kind ~width ~height scenario =
  ...
  |Canvas->let canvas=Canvas.create_exn~width~height in Canvas.render canvas Scene.[clear Color.black];
      Ok{execution;scenario;descriptor;frame=0;image=Some(Result.get_ok(Canvas.to_image canvas));canvas=Some canvas;ui=None;last_scene=None}
  |Pxui->let ui=ref(Pxui_next.create~x:348~y:16~width:276~row_height:29~padding:8~max_height:448())in
      ...
      Ok{execution;scenario;descriptor;frame=0;image=None;canvas=None;ui=Some!ui;last_scene=None}
...
let render value ~width ~height =value.frame<-value.frame+1;
  let scene=match value.scenario with Basic->basic_scene value width height|Canvas->canvas_scene value width height|Pxui->pxui_scene value in
