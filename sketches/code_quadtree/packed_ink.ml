...
let set_clip t clip =
  if t.populated then invalid_arg "Packed_ink.set_clip: publish pending marks first";
  t.clip<-clip

let set_ids t ids =
  if t.populated then invalid_arg "Packed_ink.set_ids";
...
let linef t x y x2 y2 color =
  ...
  |Error _ -> invalid_arg "Packed_ink.linef: invalid path"
  |Ok mesh ->
...
let take t =
  if not t.populated then None else begin
    ...
    t.populated<-false;
    Some (Prismel.Scene.display_list value)
  end
