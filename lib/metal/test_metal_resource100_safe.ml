...
let ()=
 ...
 (match Resource100.Texture_view_pool.create device desc with Ok pool->if Resource100.Texture_view_pool.count pool<>4L then failwith"pool count";if get(Resource100.Texture_view_pool.checked_device pool)!=device then failwith"pool device";ignore(get(Resource100.Texture_view_pool.base_resource_id pool));ignore(get(Resource100.Texture_view_pool.label pool));reject(Resource100.Texture_view_pool.set_from_buffer pool~index:0 backing_buffer~offset:(-1L)~bytes_per_row:256(Texture.descriptor_2d~storage:Buffer.Shared~usage:[Texture.Shader_read]~format:Texture.Rgba8_unorm~width:4~height:1()));reject(Resource100.Texture_view_pool.set_view pool~index:0 texture~format:Texture.Rgba8_unorm~kind:Texture.Texture_2d~level_start:0~level_count:0~slice_start:0~slice_count:1);(match Resource100.Texture_view_pool.set_view pool~index:0 texture~format:Texture.Rgba8_unorm~kind:Texture.Texture_2d~level_start:0~level_count:1~slice_start:0~slice_count:1 with Ok _->()|Error _->());get(Resource100.Texture_view_pool.destroy pool)|Error _->());
