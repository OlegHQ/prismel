...
static value *presentation_callback_root(value callback){value *root=(value*)malloc(sizeof(value));if(!root)return nullptr;*root=callback;caml_register_generational_global_root(root);return root;}
static void presentation_callback_fire(value *root){int registered=caml_c_thread_register();caml_acquire_runtime_system();caml_callback_exn(*root,Val_unit);caml_remove_generational_global_root(root);free(root);caml_release_runtime_system();if(registered)caml_c_thread_unregister();}
extern "C" CAMLprim value caml_prismel_metal_command_buffer_add_handler(value rc,value rcallback,value rscheduled){CAMLparam3(rc,rcallback,rscheduled);value *root=presentation_callback_root(rcallback);if(!root)CAMLreturn(result_error_text("unable to root presentation callback"));@try{id<MTLCommandBuffer>c=object_of_handle(rc,Handle_kind::Command_buffer);if(Bool_val(rscheduled))[c addScheduledHandler:^(id<MTLCommandBuffer>){presentation_callback_fire(root);}];else[c addCompletedHandler:^(id<MTLCommandBuffer>){presentation_callback_fire(root);}];CAMLreturn(result_unit());}@catch(NSException*x){caml_remove_generational_global_root(root);free(root);CAMLreturn(result_error(x.reason));}}
