...
module Metal_layer = struct
  ...
  let create (device : Device.t) config =
    let operation="Metal.Metal_layer.create" in on_main operation(fun()->
      ...
      if not (List.mem config.format [Texture.Bgra8_unorm;Texture.Bgra8_unorm_srgb;Texture.Rgba16_float]) then error operation Unsupported "pixel format is not supported by CAMetalLayer" else
...
module Shader_attribute_descriptors = struct
  ...
  let capacity=31
  let validate operation(value:t)index=match ensure_live operation value.lifetime with Error _ as e->e|Ok() when index<0||index>=capacity->error operation Invalid_argument "attribute index must be in [0,31)"|Ok()->Ok()
  let at (value:t) ~index=let operation="Metal.Shader_attribute_descriptors.at" in on_main operation(fun()->Result.bind(validate operation value index)(fun()->match Metal_raw.shader_attribute_descriptor_at value.raw(Int64.of_int index)with Error m->native_error operation m|Ok raw->let x:descriptor={raw;lifetime=lifetime();parent=value}in attach value.lifetime;attach_finalizer x x.lifetime value.lifetime;Ok x))
  let set (value:t) ~index (descriptor:descriptor)=let operation="Metal.Shader_attribute_descriptors.set" in on_main operation(fun()->Result.bind(validate operation value index)(fun()->match ensure_live operation descriptor.lifetime with Error _ as e->e|Ok() when descriptor.parent.lifetime!=value.lifetime->error operation Invalid_argument "descriptor belongs to another attribute array"|Ok()->match Metal_raw.shader_attribute_descriptor_set_at value.raw(Int64.of_int index)descriptor.raw with Error m->native_error operation m|Ok()->Ok()))
  let destroyed(value:t)=is_destroyed value.lifetime
...
module Shader_attribute_descriptor = struct
  ...
  let query operation raw(value:t)=on_main operation(fun()->match ensure_live operation value.lifetime with Error _ as e->e|Ok()->match raw value.raw with Error m->native_error operation m|Ok x->Ok x)
  let buffer_index value=query "Metal.Shader_attribute_descriptor.buffer_index" Metal_raw.shader_attribute_descriptor_buffer_index value
  let offset value=query "Metal.Shader_attribute_descriptor.offset" Metal_raw.shader_attribute_descriptor_offset value
  let format value=Result.bind(query "Metal.Shader_attribute_descriptor.format" Metal_raw.shader_attribute_descriptor_format value)(fun code->match Enum.Mtl_attribute_format.of_int64 code with Some x->Ok x|None->error "Metal.Shader_attribute_descriptor.format" Unsupported "unknown attribute format")
  let set_buffer_index value index=let operation="Metal.Shader_attribute_descriptor.set_buffer_index" in if index<0L||index>=31L then error operation Invalid_argument "buffer index must be in [0,31)"else query operation(fun raw->Metal_raw.shader_attribute_descriptor_set_buffer_index raw index)value
  let set_offset value offset=let operation="Metal.Shader_attribute_descriptor.set_offset" in if offset<0L then error operation Invalid_argument "offset must be nonnegative"else query operation(fun raw->Metal_raw.shader_attribute_descriptor_set_offset raw offset)value
  let set_format value format=query "Metal.Shader_attribute_descriptor.set_format"(fun raw->Metal_raw.shader_attribute_descriptor_set_format raw(Enum.Mtl_attribute_format.to_int64 format))value
  let destroyed(value:t)=is_destroyed value.lifetime
  let destroy(value:t)=destroy_leaf "Metal.Shader_attribute_descriptor.destroy" value.lifetime value.raw(fun()->detach value.parent.lifetime)
end
...
module Shader_stitching_input = struct
  ...
  let argument_index(value:t)=let operation="Metal.Shader_stitching_input.argument_index" in on_main operation(fun()->match ensure_live operation value.lifetime with Error _ as e->e|Ok()->match Metal_raw.shader_stitching_input_index value.raw with Error m->native_error operation m|Ok x->Ok x)
...
module Render_pipeline = struct
  ...
  module Mesh_tile = struct
    ...
    type mesh_descriptor={raw:Metal_raw.handle;lifetime:lifetime;device:device;functions:function_handle list;archives:binary_archive list;required_mesh:size3;required_object:size3;has_object:bool}
    type tile_descriptor={raw:Metal_raw.handle;lifetime:lifetime;device:device;function_:function_handle;archives:binary_archive list;libraries:dynamic_library list;required:size3}
    ...
    let positive s=let maximum=Int64.of_int max_int in s.width>0L&&s.height>0L&&s.depth>0L&&s.width<=maximum&&s.height<=maximum&&s.depth<=maximum
    let validate_functions operation (device:device) (values:function_handle list)=let rec loop=function []->Ok()|(x:function_handle)::xs->Result.bind(ensure_live operation x.lifetime)(fun()->Result.bind(ensure_same_device operation device x.library.device)(fun()->loop xs))in loop values
    ...
    let destroy_color(value:color_attachment)=destroy_leaf "Metal.Render_pipeline.Mesh_tile.destroy_color" value.lifetime value.raw ignore
...
module Pipeline_archive = struct
  ...
  let compile_compute (value:t) ~(library:Library.t) function_name =
    ...
    if function_name=""||contains_nul function_name then error operation Invalid_argument "compute function name is invalid"else on_main operation(fun()->Result.bind(ensure_live operation value.lifetime)(fun()->Result.bind(ensure_live operation library.lifetime)(fun()->Result.bind(ensure_same_device operation value.device library.device)(fun()->
      let descriptor:Metal_raw.metal4_compute_descriptor={label=None;library=library.raw;function_name;reflection=false;threadgroup_size_multiple=false;max_total_threads=0L;required_threads_width=0L;required_threads_height=0L;required_threads_depth=0L;support_binary_linking=false;support_indirect_commands=false;preloaded_libraries=[||];max_call_stack_depth=0L;lookup_archives=[||];binary_linked_functions=[||];static_linking=None}in
      match Metal_raw.pipeline_archive_compute value.raw descriptor false with Error m->native_error operation m|Ok raw->Ok(Compute_pipeline.make value.device~reflection:false raw[||])))))
  ...
  let compile_render (value:t) ~(library:Library.t) ~vertex ?fragment ~color_format () =
    ...
    if vertex=""||contains_nul vertex||invalid_fragment then error operation Invalid_argument "render function name is invalid"else on_main operation(fun()->Result.bind(ensure_live operation value.lifetime)(fun()->Result.bind(ensure_live operation library.lifetime)(fun()->Result.bind(ensure_same_device operation value.device library.device)(fun()->
...
module Compiler = struct
  ...
  let stage_linking ?(binary_functions = []) ?(preloaded_libraries = [])
      ?(max_call_stack_depth = 1) () =
    { binary_functions; preloaded_libraries; max_call_stack_depth }
  ...
  let raw_stage_dynamic_linking operation device ~support_binary_linking
      ~stage = function
    | None -> Ok None
    | Some (linking : stage_linking) ->
        ...
        if linking.max_call_stack_depth <= 0 then
          ...
          let* () =
            validate_binary_functions operation device linking.binary_functions
          in
          let* () =
            validate_dynamic_libraries operation device
          ...
          then
            ...
            Ok
              (Some
                 ({ max_call_stack_depth =
                      Int64.of_int linking.max_call_stack_depth
                  ; binary_linked_functions =
                      Array.of_list
                        (List.map
                           (fun (function_ : Binary_function.t) -> function_.raw)
      ...
      then
        ...
        let* vertex_dynamic_linking =
          raw_stage_dynamic_linking operation value.device
            ~support_binary_linking:support_vertex_binary_linking
            ~stage:"vertex" vertex_dynamic_linking
        in
  ...
  let create_render_pipeline_async ?label ?fragment ?(reflection = false)
    ...
    with_render_descriptor operation
      (fun reflection vertex_descriptor color_attachments color_formats
           descriptor ->
        if
          (Option.is_some descriptor.vertex_dynamic_linking
        ...
        then
          error operation Unsupported
            "asynchronous dynamic render linking requires an Apple9/M3-or-newer GPU"
        else
          match
            Metal_raw.compiler_create_render_pipeline_async value.raw
              descriptor
          with
          | Error message -> native_error operation message
          | Ok raw ->
...
module Command4 = struct
  ...
  module Compute_encoder = struct
    ...
    let dispatch_indirect_threadgroups value ~(indirect_buffer : Buffer.t) ~offset ~threads_per_threadgroup =
      let operation="Metal.Command4.Compute_encoder.dispatch_indirect_threadgroups"in
      if not(positive64 threads_per_threadgroup)then error operation Invalid_argument "indirect dispatch is invalid"else callable operation value(fun()->Result.bind(require_pipeline operation value)(fun()->Result.bind(check_buffer operation value.command_buffer.allocator.device indirect_buffer)(fun()->if offset<0L||Int64.rem offset 4L<>0L||offset>Int64.sub indirect_buffer.length 12L then error operation Invalid_argument "indirect buffer range is invalid"else Result.bind(native_unit operation(Metal_raw.metal4_compute_dispatch_indirect_groups value.raw value.command_buffer.raw((indirect_buffer.raw,offset,12L),threads_per_threadgroup)))(fun()->retain_command4_buffer value.command_buffer indirect_buffer;Ok()))))
    let dispatch_indirect_threads value ~(indirect_buffer : Buffer.t) ~offset =let operation="Metal.Command4.Compute_encoder.dispatch_indirect_threads"in callable operation value(fun()->Result.bind(require_pipeline operation value)(fun()->Result.bind(check_buffer operation value.command_buffer.allocator.device indirect_buffer)(fun()->if offset<0L||Int64.rem offset 4L<>0L||offset>Int64.sub indirect_buffer.length 12L then error operation Invalid_argument "indirect buffer range is invalid"else Result.bind(native_unit operation(Metal_raw.metal4_compute_dispatch_indirect_threads value.raw value.command_buffer.raw(indirect_buffer.raw,offset,12L)))(fun()->retain_command4_buffer value.command_buffer indirect_buffer;Ok()))))
    ...
    let execute_icb_indirect value (icb:indirect_command_buffer) ~(indirect_buffer : Buffer.t) ~offset=let operation="Metal.Command4.Compute_encoder.execute_icb_indirect"in callable operation value(fun()->Result.bind(check_icb operation value.command_buffer.allocator.device icb)(fun()->Result.bind(check_buffer operation value.command_buffer.allocator.device indirect_buffer)(fun()->if offset<0L||Int64.rem offset 4L<>0L||offset>Int64.sub indirect_buffer.length 4L then error operation Invalid_argument "indirect range buffer is invalid"else Result.bind(native_unit operation(Metal_raw.metal4_compute_execute_icb_indirect value.raw value.command_buffer.raw(icb.raw,(indirect_buffer.raw,offset,4L))))(fun()->retain_command4_other value.command_buffer icb.lifetime;retain_command4_buffer value.command_buffer indirect_buffer;Ok()))))
...
module Resource_state_encoder = struct
  ...
  let update_texture_mapping_indirect (value:t) ~mode (texture:Texture.t)
      ~(indirect_buffer:Buffer.t) ~offset =
    let operation="Metal.Resource_state_encoder.update_texture_mapping_indirect"in
    on_main operation(fun()->match ensure_live operation value.lifetime with Error _ as e->e|Ok()->match ensure_texture_usable operation texture with Error _ as e->e|Ok()->match ensure_buffer_usable operation indirect_buffer with Error _ as e->e|Ok()->match ensure_same_device operation value.command_buffer.queue.device texture.device with Error _ as e->e|Ok()->match ensure_same_device operation texture.device indirect_buffer.device with Error _ as e->e|Ok()->if not(match texture_heap texture with Some heap->heap.descriptor.kind=Sparse|None->false)then error operation Invalid_argument "texture is not sparse-heap-backed"else if indirect_buffer.storage=Private then error operation Unsupported "safe indirect mapping requires a CPU-readable argument buffer"else if offset<0L||Int64.rem offset 4L<>0L||offset>indirect_buffer.length||4L>Int64.sub indirect_buffer.length offset then error operation Invalid_argument "indirect mapping header requires a four-byte-aligned buffer range"else match Buffer.read_bytes indirect_buffer~offset~length:4 with Error _ as e->e|Ok bytes->let byte i=Char.code(Bytes.get bytes i)in let count=Int64.logor(Int64.of_int(byte 0))(Int64.logor(Int64.shift_left(Int64.of_int(byte 1))8)(Int64.logor(Int64.shift_left(Int64.of_int(byte 2))16)(Int64.shift_left(Int64.of_int(byte 3))24)))in if count>Int64.div(Int64.sub Int64.max_int 4L)32L then error operation Invalid_argument "indirect mapping count overflows its byte range"else let required=Int64.add 4L(Int64.mul 32L count)in if required>Int64.sub indirect_buffer.length offset then error operation Invalid_argument "indirect mapping buffer is shorter than its declared mapping count"else match Metal_raw.resource_encoder_indirect_mapping value.raw texture.raw(Int64.of_int(mode_code mode))indirect_buffer.raw offset with Error m->native_error operation m|Ok()->retain_command_buffer_texture value.command_buffer texture;retain_command_buffer_buffer value.command_buffer indirect_buffer;Ok())

  let update_texture_mappings (value:t) ~mode (texture:Texture.t)
      ~(regions:tile_region array) ~mip_levels ~slices =
    ...
    on_main operation(fun()->match ensure_live operation value.lifetime with
      ...
      |Ok()->Result.bind(ensure_texture_usable operation source)(fun()->
          Result.bind(ensure_buffer_usable operation destination)(fun()->
            ...
            if source_slice<0||source_slice>=Texture.total_slices d||source_level<0
               ||source_level>=d.mip_levels||source_region.x<0||source_region.y<0
               ||source_region.z<0||source_region.width<=0||source_region.height<=0
               ||source_region.depth<=0||destination_offset<0L
            ...
            else let total=Int64.mul destination_bytes_per_image(Int64.of_int source_region.depth)in
              if total<0L||destination_offset>destination.length
                 ||total>Int64.sub destination.length destination_offset then
                error operation Invalid_argument "texture-to-buffer destination range exceeds buffer"
              else Result.bind(ensure_same_device operation value.command_buffer.queue.device source.device)(fun()->
                Result.bind(ensure_same_device operation source.device destination.device)(fun()->
                  match Metal_raw.blit_copy value.raw 3 source.raw destination.raw
                    (Metal_raw.Blit_texture_to_buffer(Int64.of_int source_slice,Int64.of_int source_level,
                      ...
                      destination_offset,destination_bytes_per_row,destination_bytes_per_image,options))with
                  |Error message->native_error operation message
                  |Ok()->retain_command_buffer_texture value.command_buffer source;
...
module Resource100 = struct
  ...
  module Texture_view_pool = struct
    ...
    let destroyed (t:t)=is_destroyed t.lifetime
    let destroy (t:t)=destroy_parent "Metal.Resource100.Texture_view_pool.destroy" t.lifetime t.raw(fun()->Array.iter(Option.iter(fun (x:texture)->detach x.lifetime))t.pool_views;Array.iter(Option.iter(fun(x:buffer)->detach x.lifetime))t.pool_buffers;detach t.device.lifetime)
  end
...
module Tensor = struct
  ...
  module Descriptor = struct
    ...
    let set_options t ~storage ~cpu_cache ~hazard_tracking ~usage=let op="Metal.Tensor.Descriptor.set_options"in on_main op(fun()->match ensure_live op t.lifetime with Error _ as e->e|Ok()when usage<0L->error op Invalid_argument "tensor usage must be nonnegative"|Ok()->let resource_options=Int64.of_int(resource_options_code~storage~cpu_cache~hazard_tracking)in let calls=[Metal_raw.tensor_descriptor_set_storage_mode t.raw(Int64.of_int(storage_code storage));Metal_raw.tensor_descriptor_set_cpu_cache_mode t.raw(Int64.of_int(cache_code cpu_cache));Metal_raw.tensor_descriptor_set_hazard_tracking_mode t.raw(Int64.of_int(hazard_code hazard_tracking));Metal_raw.tensor_descriptor_set_resource_options t.raw resource_options;Metal_raw.tensor_descriptor_set_usage t.raw usage]in match List.find_opt Result.is_error calls with Some(Error m)->native_error op m|_->t.storage<-storage;t.cpu_cache<-cpu_cache;t.hazard<-hazard_tracking;t.usage<-usage;Ok())
    let checked_options t=let op="Metal.Tensor.Descriptor.checked_options"in on_main op(fun()->match ensure_live op t.lifetime with Error _ as e->e|Ok()->match Metal_raw.tensor_descriptor_storage_mode t.raw,Metal_raw.tensor_descriptor_cpu_cache_mode t.raw,Metal_raw.tensor_descriptor_hazard_tracking_mode t.raw,Metal_raw.tensor_descriptor_resource_options t.raw,Metal_raw.tensor_descriptor_usage t.raw,Metal_raw.tensor_descriptor_data_type t.raw with Ok storage,Ok cache,Ok hazard,Ok resource_options,Ok usage,Ok data when storage=Int64.of_int(storage_code t.storage)&&cache=Int64.of_int(cache_code t.cpu_cache)&&hazard=Int64.of_int(hazard_code t.hazard)&&resource_options=Int64.of_int(resource_options_code~storage:t.storage~cpu_cache:t.cpu_cache~hazard_tracking:t.hazard)&&usage=t.usage&&data=Data_type.to_int64 t.data_type->Ok(t.storage,t.cpu_cache,t.hazard,t.usage)|Error m,_,_,_,_,_|_,Error m,_,_,_,_|_,_,Error m,_,_,_|_,_,_,Error m,_,_|_,_,_,_,Error m,_|_,_,_,_,_,Error m->native_error op m|_->native_error op "native tensor descriptor disagrees with safe metadata")
    let destroyed t=is_destroyed t.lifetime
  ...
  module Device_owned = struct
    type t={raw:Metal_raw.handle;lifetime:lifetime;device:device;descriptor:Descriptor.t}
    let create (device:Device.t)(descriptor:Descriptor.t)=let op="Metal.Tensor.Device_owned.create"in on_main op(fun()->match ensure_live op device.lifetime with Error _ as e->e|Ok()->match ensure_live op descriptor.lifetime with Error _ as e->e|Ok()->match Metal_raw.tensor_device_create device.raw descriptor.raw with Error m->native_error op m|Ok raw->let value:t={raw;lifetime=lifetime();device;descriptor}in attach device.lifetime;attach descriptor.lifetime;attach_finalizer~on_finalize:(fun()->detach descriptor.lifetime)value value.lifetime device.lifetime;Ok value)
    let size_and_align (device:Device.t)(descriptor:Descriptor.t)=let op="Metal.Tensor.Device_owned.size_and_align"in on_main op(fun()->match ensure_live op device.lifetime with Error _ as e->e|Ok()->match ensure_live op descriptor.lifetime with Error _ as e->e|Ok()->match Metal_raw.tensor_device_size_align device.raw descriptor.raw with Error m->native_error op m|Ok(size,alignment)when size>0L&&alignment>0L->Ok(size,alignment)|Ok _->native_error op "native tensor size/alignment is invalid")
    let device t=t.device
    let descriptor t=t.descriptor
    let destroyed t=is_destroyed t.lifetime
    let destroy t=destroy_leaf "Metal.Tensor.Device_owned.destroy" t.lifetime t.raw(fun()->detach t.descriptor.lifetime;detach t.device.lifetime)
  end
