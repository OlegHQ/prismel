...
let () =
  ...
  Fun.protect ~finally:(fun () ->
    ...
    close_out channel;
    let legacy = get (Editor_core.Store.Settings.load ~sketch:"test" path) in
    assert (Editor_core.Store.Settings.bool legacy "animate" = Some true);
    assert (Editor_core.Store.Settings.float legacy "density" = Some 0.625);
    assert (Result.is_error (Editor_core.Store.Settings.save ~sketch:"test" path
      ["bad", Float infinity]));
    assert (get (Editor_core.Store.Settings.load ~sketch:"test" path) = legacy));
  let open Prismel in
