...
type t={pass:Ogpu.Render_pass.t;color:Texture.t;resolve:Texture.t option;depth:Texture.t option;stencil:Texture.t option;draws:draw list;owned_samplers:Sampler.t list;indirect:Metal.Indirect_command_buffer.t option}
...
let with_indirect value indirect={value with indirect=Some indirect}
