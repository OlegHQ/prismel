...
type draw={pipeline:Pipeline.t;buffers:buffer_binding list;textures:texture_binding list;samplers:sampler_binding list;primitive:primitive;vertex_start:int;vertex_count:int;instance_count:int;index:(index_type*Buffer.t*int64*int64)option}
...
let indexed_preparable_draw draw=
  draw.instance_count=1&&draw.textures=[]&&draw.samplers=[]&&
  not(List.exists(fun(binding:buffer_binding)->binding.index=7)draw.buffers)&&
  Option.is_some draw.index
let scene3_winding draws=
...
let issue_draw op encoder draw=
  ...
  |Some(kind,buffer,offset,count)->
      let primitive=match draw.primitive with
        |Point_list->Metal.Render_encoder.Point|Line_list->Line
        |Triangle_list->Triangle|Triangle_strip->Triangle_strip in
      let index_type=match kind with Uint16->Metal.Render_encoder.Uint16
        |Uint32->Uint32 in
      let index_buffer=Buffer.Private.metal buffer in
      adapt_metal op(if draw.instance_count=1 then
        Metal.Render_encoder.draw_indexed_basic encoder ~primitive ~index_type
          ~index_buffer ~index_offset:offset ~index_count:count
        else Metal.Render_encoder.draw_indexed_instances encoder ~primitive
          ~index_type ~index_buffer ~index_offset:offset ~index_count:count
          ~instances:(Int64.of_int draw.instance_count))
...
let prepare_indexed_draw (draw:draw)=
  ...
  ({Metal.Render_encoder.Private.prepared_pipeline;prepared_bindings;
    prepared_primitive=(match draw.primitive with
      |Point_list->Metal.Render_encoder.Point|Line_list->Line
      |Triangle_list->Triangle|Triangle_strip->Triangle_strip);
      ...
      |Ok depth_state->cleanup:=(fun()->ignore(Metal.Depth_stencil.destroy depth_state))::!cleanup;
        ...
        let rec draws=function
          ...
          |draw::rest->let native=match Pipeline.Private.native draw.pipeline with Render p->p|Compute _->assert false in
            ...
            let primitive=match draw.primitive with
              |Point_list->Metal.Command4.Render_encoder.Point|Line_list->Line
              |Triangle_list->Triangle|Triangle_strip->Triangle_strip in
            let issued=match draw.index with None->Metal.Command4.Render_encoder.draw_primitives encoder primitive~vertex_start:draw.vertex_start~vertex_count:draw.vertex_count|Some(kind,buffer,offset,count)->
              let kind=match kind with Uint16->Metal.Command4.Render_encoder.Uint16|Uint32->Uint32 in
              let index_buffer=Buffer.Private.metal buffer and index_count=Int64.to_int count in
              if draw.instance_count=1 then Metal.Command4.Render_encoder.draw_indexed_primitives encoder primitive kind ~index_buffer ~index_offset:offset ~index_count
              else Metal.Command4.Render_encoder.draw_indexed_primitives_instanced encoder primitive kind ~index_buffer ~index_offset:offset ~index_count ~instance_count:draw.instance_count ~base_vertex:0 ~base_instance:0 in
