...
type t={pass:Ogpu.Render_pass.t;color:Texture.t;resolve:Texture.t option;depth:Texture.t option;stencil:Texture.t option;draws:draw list;owned_samplers:Sampler.t list}
...
let create device pass ~attachments draw=let op="Ogpu_metal.Render_pass.create"in
  ...
  |Some target->
    ...
    |Error e,_,_->Error e|_,Error e,_->Error e|_,_,Error e->Error e|Ok td,Ok resolve,Ok()->
      ...
      let stencil=match descriptor.stencil with None->Ok None|Some s when s.load<>Clear||s.store<>Store->error op Ogpu.Error.Unsupported"classic stencil execution requires clear/store semantics"|Some s->match List.find_map(find s.texture.id)attachments with None->error op Ogpu.Error.Invalid_argument"stencil attachment texture is absent from the typed texture graph"|Some texture->Result.map(fun _->Some texture)(Texture.descriptor device texture)in
