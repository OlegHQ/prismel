...
module Private=struct
  ...
  let encode command value=let op="Ogpu_metal.Render_pass.encode"in let descriptor=Ogpu.Render_pass.descriptor value.pass in let color=List.hd(Array.to_list descriptor.colors|>List.filter_map Fun.id)in
    ...
    let store=Result.map_error(Adapter.error~operation:op)(Metal.Render_encoder.set_color_store_action encoder(match value.resolve with None->Metal.Render_encoder.Store|Some _->Store_and_multisample_resolve))in
    match store with Error _ as e->ignore(Metal.Render_pass_descriptor.destroy native_pass);e|Ok()->
