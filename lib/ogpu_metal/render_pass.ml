...
type t={pass:Ogpu.Render_pass.t;color:Texture.t;resolve:Texture.t option;depth:Texture.t option;draws:draw list;owned_samplers:Sampler.t list}
...
let create device pass ~attachments draw=let op="Ogpu_metal.Render_pass.create"in
  ...
  |Some target->
    ...
    |Error e,_,_->Error e|_,Error e,_->Error e|_,_,Error e->Error e|Ok td,Ok resolve,Ok()->
      ...
      match draw.index with None->Ok{pass;color=target;resolve;depth;draws=[draw];owned_samplers=[]}|Some(kind,buffer,offset,count)->match Buffer.descriptor device buffer with Error _ as e->e|Ok bd->let stride=match kind with Uint16->2L|Uint32->4L in if count<=0||offset<0L||Int64.rem offset stride<>0L||Int64.of_int count>Int64.div(Int64.sub bd.size offset)stride then error op Ogpu.Error.Invalid_argument"index range is invalid"else Ok{pass;color=target;resolve;depth;draws=[draw];owned_samplers=[]})
...
module Private=struct
  ...
  let retain value=let retained=ref[]and seen=Hashtbl.create 32 in let keep retain release=retain_one retained retain release in let once key f=if Hashtbl.mem seen key then(fun()->Ok())else(Hashtbl.add seen key();f)in
    ...
    let resources=texture value.color::(match value.resolve with None->[]|Some t->[texture t])@(match value.depth with None->[]|Some t->[texture t])@List.concat_map draw_resources value.draws in
  ...
  let encode command value=let op="Ogpu_metal.Render_pass.encode"in let descriptor=Ogpu.Render_pass.descriptor value.pass in let color=List.hd(Array.to_list descriptor.colors|>List.filter_map Fun.id)in
    ...
    let depth_state=match value.depth with None->Ok None|Some _->Result.map Option.some(Metal.Depth_stencil.create~label:"ogpu-metal-render-pass"~depth_compare:(metal_compare raster.depth_compare)~depth_write:raster.depth_write(Metal.Command_buffer.device command)())in
    let store=match depth_state with Error e->Error(Adapter.error~operation:op e)|Ok depth_state->match Metal.Render_encoder.set_cull_mode encoder(metal_cull raster.cull)with Error e->Option.iter(fun state->ignore(Metal.Depth_stencil.destroy state))depth_state;Error(Adapter.error~operation:op e)|Ok()->match Metal.Render_encoder.set_depth_stencil_state encoder depth_state with Error e->Option.iter(fun state->ignore(Metal.Depth_stencil.destroy state))depth_state;Error(Adapter.error~operation:op e)|Ok()->Result.map(fun()->depth_state)(Result.map_error(Adapter.error~operation:op)(Metal.Render_encoder.set_color_store_action encoder(match value.resolve with None->Metal.Render_encoder.Store|Some _->Store_and_multisample_resolve)))in
