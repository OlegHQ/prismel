...
let ()=match Device.system_default()with Error _->print_endline"scene execution Metal: skipped (no M1 device)"|Ok native_device->
  ...
  let renderer=get(Scene_execution.create_with_pipeline driver configuration(fun device->let empty=get(Ogpu.Binding.create_layout[])in let layout=get(Ogpu.Binding.create_pipeline_layout~device:(Ogpu.Backend.device_handle device)~capabilities:(Ogpu.Backend.capabilities device)[0,empty])in let descriptor:Ogpu.Pipeline.render_descriptor={backend="metal";label=Some"scene-execution-metal";layout;vertex=shader;vertex_entry="scene_vertex";fragment=Some shader;fragment_entry=Some"scene_fragment";color_format=Rgba8_unorm;depth_format=No_depth;sample_count=1}in let native=get(Pipeline.create_render_runtime_msl cache native_device descriptor)in Backend.register_pipeline control native;Ok(Pipeline.Private.portable native)))in
