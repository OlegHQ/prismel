...
let run () =match Device.system_default()with Error _->print_endline"ogpu_metal render pass: skipped (no device)"|Ok device->
  ...
  let draw primitive index={Render_pass.pipeline;buffers=[];textures=[];samplers=[];primitive;vertex_start=0;vertex_count=3;instance_count=1;index}in
  ...
  |Ok stencil_pipeline->
    ...
    let stencil_draw={Render_pass.pipeline=stencil_pipeline;buffers=[];textures=[];samplers=[];primitive=Triangle_list;vertex_start=0;vertex_count=3;instance_count=1;index=None}in
  ...
  let limits=(Device.capabilities device).Ogpu.Caps.limits in
  ...
  let depth_draw={Render_pass.pipeline=depth_pipeline;buffers=[];textures=[];
    samplers=[];primitive=Triangle_strip;vertex_start=0;vertex_count=3;instance_count=1;
  ...
  let indirect_draw={Render_pass.pipeline=icb_pipeline;buffers=[];textures=[];
    samplers=[];primitive=Triangle_list;vertex_start=0;vertex_count=3;instance_count=1;index=None}in
