...
let run () =match Device.system_default()with Error _->print_endline"ogpu_metal sampler pixels: skipped (no device)"|Ok native_device->
  ...
  let bad_native,bad_pipeline=List.hd pipelines in let missing:Ogpu.Render_pass.draw={pipeline_key=Pipeline.key bad_native;buffers=[];textures=[{stage=Ogpu.Command.Fragment;index=0;texture_id=sampled_id}];samplers=[];primitive=Triangle_list;vertex_start=0;vertex_count=3;instance_count=1;index=None}in let unchanged=get(Ogpu.Backend.read_texture target~bytes_per_row:4)in(match Ogpu.Backend.render pass[missing]with Error error when error.Ogpu.Error.kind=Invalid_argument->()|_->failwith"missing sampler was accepted");if get(Ogpu.Backend.read_texture target~bytes_per_row:4)<>unchanged then failwith"missing sampler mutated target";ignore bad_pipeline;
  List.iter(fun(entry,expected,sampler)->let native,pipeline=List.find(fun(native,_)->Pipeline.label native=Some entry)pipelines in let draw:Ogpu.Render_pass.draw={pipeline_key=Pipeline.key native;buffers=[];textures=[{stage=Ogpu.Command.Fragment;index=0;texture_id=sampled_id}];samplers=[{stage=Ogpu.Command.Fragment;index=1;sampler}];primitive=Triangle_list;vertex_start=0;vertex_count=3;instance_count=1;index=None}in let command=get(Ogpu.Backend.render pass[draw])in let receipt=get(Ogpu.Backend.submit queue command~resources:[`Texture target;`Texture sampled]~pipelines:[pipeline])in get(Ogpu.Backend.complete_through queue receipt.epoch);let r,g,b,a=rgba(get(Ogpu.Backend.read_texture target~bytes_per_row:4))in if not(close expected[r;g;b;a])then failwith(Printf.sprintf"%s pixel mismatch: %d,%d,%d,%d"entry r g b a))cases;
