...
type t =
  { device : Device.t; portable : Ogpu_core.Acceleration.t;
...
let plan_triangle ~ray_tracing ~buffer_size ~offset ~length ~vertex_stride ~vertex_count =
  ...
  then error operation Ogpu_core.Error.Invalid_argument "triangle buffer range, stride, or cardinality is invalid"
  else if not ray_tracing then error operation Ogpu_core.Error.Unsupported "ray tracing is unavailable on this adapter"
...
let create_triangle device ~vertices ~offset ~length ~vertex_stride ~vertex_count ~allow_refit =
  ...
  let ray_tracing = (Device.capabilities device).Ogpu_core.Caps.ray_tracing in
  ...
  let range : Ogpu_core.Acceleration.buffer_range =
    {buffer=Buffer.Private.resource_handle vertices;buffer_size=buffer_descriptor.size;offset;length} in
  let portable_descriptor = Ogpu_core.Acceleration.Blas
    {geometries=[|Ogpu_core.Acceleration.Triangles {vertices=range;vertex_stride;vertex_count}|];allow_refit} in
  match Ogpu_core.Acceleration.create (Device.Private.handle device) ~ray_tracing:true portable_descriptor with Error _ as failure -> failure | Ok portable ->
  ...
  | Error metal -> Ogpu_core.Acceleration.destroy portable; Error (Device.of_metal_error ~operation metal)
...
let build device value ~scratch ~scratch_offset =
  ...
  | Ok () -> Ogpu_core.Acceleration.build (Device.Private.handle device) value.portable

let refit device value ~scratch ~scratch_offset =
  let operation = "Ogpu_metal.Acceleration.refit" in
  if not value.allow_refit then error operation Ogpu_core.Error.Unsupported "descriptor does not permit refit" else
  ...
  | Error _ as failure -> failure
  | Ok () -> Ogpu_core.Acceleration.refit (Device.Private.handle device) value.portable
...
let copy device value =
  ...
  | Ok target -> (match with_encoder operation device (fun encoder -> Metal.Acceleration_encoder.copy encoder ~source:value.metal ~destination:target) with
      ...
      | Ok () -> match Ogpu_core.Acceleration.copy (Device.Private.handle device) value.portable with
        | Error _ as failure -> ignore (Metal.Acceleration_structure.destroy target); failure
...
let compact device value =
  ...
  | Ok target -> (match with_encoder operation device (fun encoder -> Metal.Acceleration_encoder.copy_and_compact encoder ~source:value.metal ~destination:target) with
      | Error _ as failure -> ignore (Metal.Acceleration_structure.destroy target); failure
      | Ok () -> match Ogpu_core.Acceleration.compact (Device.Private.handle device) value.portable with
        | Error _ as failure -> ignore (Metal.Acceleration_structure.destroy target); failure
...
let destroy value =
  ...
  | Ok () -> value.dead <- true; Ogpu_core.Acceleration.destroy value.portable;
