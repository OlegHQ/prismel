...
let acquire_common ~scoped value=let op="Ogpu_metal.Surface.acquire"in if destroyed value then error op Ogpu.Error.Stale_handle"surface is destroyed"else match Ogpu.Surface.acquire value.portable with Error _ as e->e|Ok Timeout->Ok Timeout|Ok Occluded->Ok Occluded|Ok Device_lost->Ok Device_lost|Ok(Acquired portable)->match (if scoped then Metal.Drawable.Private.acquire_scoped value.layer else Metal.Drawable.acquire value.layer)with Error e->ignore(Ogpu.Surface.discard value.portable portable);Error(Device.of_metal_error~operation:op e)|Ok(Error Metal.Drawable.Timeout_or_unavailable)->ignore(Ogpu.Surface.discard value.portable portable);Ok Timeout|Ok(Ok drawable)->match (if scoped then Metal.Drawable.Private.texture_scoped drawable else Metal.Drawable.texture drawable)with Error e->ignore(Metal.Drawable.destroy drawable);ignore(Ogpu.Surface.discard value.portable portable);Error(Device.of_metal_error~operation:op e)|Ok texture->let frame={surface=value;portable;drawable;texture;generation=generation value;state=Live}in value.frames<-frame::value.frames;Ok(Acquired frame)
