...
type t={device:Device.t;layer:Metal.Metal_layer.t;portable:Ogpu_core.Surface.t;presentation:Presentation.t;readable_drawables_for_test:bool;mutable frames:frame list;mutable in_flight_presentations:int;mutable destroy_pending:bool;mutable dead:bool}
and frame={surface:t;portable:Ogpu_core.Surface.frame;drawable:Metal.Drawable.t;texture:Metal.Texture.t;generation:int64;mutable state:frame_state}
...
let metal_config ~readable_drawables_for_test (value:Ogpu_core.Surface.configuration)=
  let format=match value.format with Bgra8_unorm->Ok Metal.Texture.Bgra8_unorm|Rgba8_unorm->error"Ogpu_metal.Surface.configure"Ogpu_core.Error.Invalid_argument"CAMetalLayer does not expose RGBA8 drawable format"in
...
let create_common ~readable_drawables_for_test device~layer config=let op="Ogpu_metal.Surface.create"in if Device.destroyed device then error op Ogpu_core.Error.Stale_handle"device is destroyed"else if not(Metal.Device.same(Device.Private.metal device)(Metal.Metal_layer.device layer))then error op Ogpu_core.Error.Cross_device"layer belongs to another device"else match Ogpu_core.Surface.create(Device.Private.handle device)config with Error _ as e->e|Ok portable->match metal_config~readable_drawables_for_test config with Error _ as e->Ogpu_core.Surface.destroy portable;e|Ok native->match Presentation.create device with Error _ as e->Ogpu_core.Surface.destroy portable;e|Ok presentation->match Metal.Metal_layer.configure layer native with Error e->Presentation.destroy presentation;Ogpu_core.Surface.destroy portable;Error(Device.of_metal_error~operation:op e)|Ok()->Device.Private.attach_resource device;Ok{device;layer;portable;presentation;readable_drawables_for_test;frames=[];in_flight_presentations=0;destroy_pending=false;dead=false}
...
let frame_texture (value:frame)=match value.state with Live->Ok value.texture|Presented|Discarded|Stale->error"Ogpu_metal.Surface.frame_texture"Ogpu_core.Error.Stale_handle"frame is consumed or stale"
...
let teardown value=if not value.dead then(stale_frames value;Presentation.destroy value.presentation;Ogpu_core.Surface.destroy value.portable;value.destroy_pending<-false;value.dead<-true;Device.Private.detach_resource value.device)
let preflight_portable value config=match Ogpu_core.Surface.create(Device.Private.handle value.device)config with Error _ as e->e|Ok probe->Ogpu_core.Surface.destroy probe;Ok()
let configure value config=let op="Ogpu_metal.Surface.configure"in if destroyed value then error op Ogpu_core.Error.Stale_handle"surface is destroyed"else if value.in_flight_presentations<>0 then error op Ogpu_core.Error.Invalid_state"surface has presentations in flight"else match metal_config~readable_drawables_for_test:value.readable_drawables_for_test config with Error _ as e->e|Ok native->match preflight_portable value config with Error _ as e->e|Ok()->match Metal.Metal_layer.configure value.layer native with Error e->Error(Device.of_metal_error~operation:op e)|Ok()->stale_frames value;Ogpu_core.Surface.configure value.portable config
let resize value~logical_width~logical_height~physical_width~physical_height=let current=Metal.Metal_layer.config value.layer in let format=if current.format=Metal.Texture.Bgra8_unorm then Ogpu_core.Surface.Bgra8_unorm else Rgba8_unorm in let config={Ogpu_core.Surface.logical_width;logical_height;physical_width;physical_height;format;present_mode=(if current.display_sync then Fifo else Immediate);max_acquired=current.maximum_drawables}in configure value config
let set_availability value availability=Ogpu_core.Surface.set_availability value.portable availability
let acquire_common ~scoped value=let op="Ogpu_metal.Surface.acquire"in if destroyed value then error op Ogpu_core.Error.Stale_handle"surface is destroyed"else match Ogpu_core.Surface.acquire value.portable with Error _ as e->e|Ok Timeout->Ok Timeout|Ok Occluded->Ok Occluded|Ok Device_lost->Ok Device_lost|Ok(Acquired portable)->match (if scoped then Metal.Drawable.Private.acquire_scoped value.layer else Metal.Drawable.acquire value.layer)with Error e->ignore(Ogpu_core.Surface.discard value.portable portable);Error(Device.of_metal_error~operation:op e)|Ok(Error Metal.Drawable.Timeout_or_unavailable)->ignore(Ogpu_core.Surface.discard value.portable portable);Ok Timeout|Ok(Ok drawable)->match (if scoped then Metal.Drawable.Private.texture_scoped drawable else Metal.Drawable.texture drawable)with Error e->ignore(Metal.Drawable.destroy drawable);ignore(Ogpu_core.Surface.discard value.portable portable);Error(Device.of_metal_error~operation:op e)|Ok texture->let frame={surface=value;portable;drawable;texture;generation=generation value;state=Live}in value.frames<-frame::value.frames;Ok(Acquired frame)
let acquire=acquire_common~scoped:false
let validate op value frame=if destroyed value then error op Ogpu_core.Error.Stale_handle"surface is destroyed"else if frame.surface!=value then error op Ogpu_core.Error.Cross_device"frame belongs to another surface"else if frame.generation<>generation value||frame.state=Stale then error op Ogpu_core.Error.Stale_handle"frame generation is stale"else match frame.state with Live->Ok()|Presented|Discarded->error op Ogpu_core.Error.Invalid_state"frame was already consumed"|Stale->error op Ogpu_core.Error.Stale_handle"frame generation is stale"
let remove value frame=value.frames<-List.filter(fun candidate->candidate!=frame)value.frames
let commit_presented value (frame:frame)=let result=Ogpu_core.Surface.present value.portable frame.portable in frame.state<-Presented;remove value frame;result
...
let encode_present ?(scoped=false) pending commands=
  ...
  |_->error"Ogpu_metal.Surface.encode_present"Ogpu_core.Error.Invalid_state
...
let prepare_present pending value frame ~source=
  ...
  if not(pending_presentation_available pending)then
    error op Ogpu_core.Error.Capacity"bounded presentation slot is already in use"
  ...
  |Ok descriptor,Ok format->
    if format<>Texture.Rgba8_unorm then error op Ogpu_core.Error.Invalid_argument"presentation source must use RGBA8 format"
    else if descriptor.sample_count<>1||not(List.mem Ogpu_core.Types.Texture_binding descriptor.usage)then error op Ogpu_core.Error.Invalid_argument"presentation source must be single-sample and texture-bindable"
    else let target=Metal.Texture.descriptor frame.texture in
      if descriptor.width<>target.width||descriptor.height<>target.height then error op Ogpu_core.Error.Invalid_argument"presentation source and drawable extents differ"
...
let commit_present pending ~epoch=
  ...
  |true,Some{owner=value;frame;_}when pending.epoch=0L->
      ignore(Ogpu_core.Surface.present value.portable frame.portable);
...
let discard value frame=let op="Ogpu_metal.Surface.discard"in match validate op value frame with Error _ as e->e|Ok()->match Ogpu_core.Surface.discard value.portable frame.portable with Error _ as e->e|Ok()->frame.state<-Discarded;remove value frame;release frame;Ok()
