  ...
  let create_device()=if c.device_live then error"Ogpu_metal.Backend.create_device"Ogpu.Error.Invalid_state"adapter already owns a live device"else match obtain_device()with Error _ as e->e|Ok device->c.device_live<-true;let device_token=token c in
    ...
    let create_queue()=match Queue.create device with Error _ as e->e|Ok queue->let queue_token=token c in
      ...
      let submit_with ~presenting presentation command ~resources ~pipelines=
        match command with
        |(Ogpu.Backend.Transfer _|Compute _)when presenting->error"Ogpu_metal.Backend.submit_present"Ogpu.Error.Unsupported"combined presentation requires a render command"
        ...
        |Render submission->
          let cached_classic=
            List.find_opt(fun(old_command,old_resources,old_pipelines,_)->
              old_command=command&&same_resources old_resources resources&&
              same_pipelines old_pipelines pipelines)!classic_submissions in
          (match cached_classic with
          |Some(_,_,_,encoded)->submit_native_render~presenting presentation queue encoded
          |None->
          let cached_retained=List.find_opt(fun(old_command,old_pipelines,key,_)->
          ...
          |Some(_,_,key,template)->
              ...
              if not dependencies_live then begin
                retained_replays:=List.filter(fun(_,_,stored,_)->stored<>key)
                  !retained_replays;
              ...
              |Error _ as e->e
              |Ok encoded->
                  let encoded=Render_pass.replay_indirect encoded ~template in
          ...
          let maybe_indirect encoded=
            ...
            match c.plan_cache with None when List.exists argument_pipeline native_draws->error"Ogpu_metal.Backend.render"Ogpu.Error.Unsupported"retained plan cache is unavailable"|None->Ok(encoded,None,None)|Some _ when native_draws=[]||not(List.for_all eligible native_draws)->Ok(encoded,None,None)|Some cache->
            let portable_draws=Ogpu.Render_pass.submission_draws submission in
            let key,generation=
              match List.find_opt(fun(draws,tokens,_,_)->
                draws=portable_draws&&same_pipelines tokens pipelines)
                !retained_identities with
              |Some(_,_,key,generation)->key,generation
              |None->
                  ...
                  retained_identities:=(portable_draws,pipelines,key,generation)::
                    !retained_identities;
                  if List.length!retained_identities>retained_identity_capacity then
                    retained_identities:=List.filteri(fun index _->
                      index<retained_identity_capacity)!retained_identities;
                  key,generation in
