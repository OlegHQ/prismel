...
type plan_owner={queue_token:int64;mutable last_epoch:int64;dependencies:int64 list;pipeline_keys:string list;commands:Metal.Indirect_command_buffer.Render_command.t list;samplers:Metal.Sampler.t list;encoders:Metal.Shader_argument_encoder.t list;buffers:Metal.Buffer.t list;vertex_buffers:Metal.Buffer.t list;textures:Metal.Texture.t list}
...
let retained_plan_stats c={builds=c.plan_builds;hits=c.plan_hits;misses=c.plan_misses;evictions=c.plan_evictions;executions=c.plan_executions;entries=Hashtbl.length c.plan_owners;capacity=c.plan_capacity}
...
module Private=struct let disable_retained_plans_for_test=disable_retained_plans_for_test end
let first_error current=function
  |Ok()->current
  |Error error->(match current with Some _->current|None->Some error)
let destroy_owner_front owner=let failure=List.fold_left(fun e command->first_error e(Metal.Indirect_command_buffer.Render_command.destroy command))None owner.commands in List.fold_left(fun e encoder->first_error e(Metal.Shader_argument_encoder.destroy encoder))failure owner.encoders
...
let destroy_retired retired=let failure=Option.fold~none:None~some:destroy_owner_front retired.owner in let failure=first_error failure(Metal.Indirect_command_buffer.destroy retired.icb)in Option.fold~none:failure~some:(fun owner->destroy_owner_back owner failure)retired.owner
let create ?device:provided_device ?layer ?(retained_plan_capacity=64) ()=
  let c={resources=Hashtbl.create 32;pipelines=Hashtbl.create 16;pipeline_tokens=Hashtbl.create 16;sampler_cache=Hashtbl.create 64;plan_owners=Hashtbl.create 64;classic_invalidators=Hashtbl.create 4;active_queues=Hashtbl.create 4;completed_epochs=Hashtbl.create 4;retired=[];plan_cache=None;cleanup_error=None;device_live=false;next=1L;plan_builds=0L;plan_hits=0L;plan_misses=0L;plan_evictions=0L;plan_executions=0L;plan_capacity=retained_plan_capacity;layer}in
  let record_cleanup failure=Option.iter(fun error->if Option.is_none c.cleanup_error then c.cleanup_error<-Some error)failure in
  ...
  let create_device()=if c.device_live then error"Ogpu_metal.Backend.create_device"Ogpu.Error.Invalid_state"adapter already owns a live device"else match obtain_device()with Error _ as e->e|Ok device->c.device_live<-true;let device_token=token c in
    ...
    let invalidate_pipeline key id=Hashtbl.iter(fun _ (_,invalidate,_,_)->invalidate id)c.classic_invalidators;match c.plan_cache with None->()|Some cache->let keys=Hashtbl.fold(fun plan_key(owner:plan_owner) acc->if List.mem key owner.pipeline_keys then plan_key::acc else acc)c.plan_owners[]in List.iter(fun plan_key->ignore(Metal.Retained_render_plan.invalidate cache plan_key))keys in
    let compact_sampler_cache()=
      ...
      Hashtbl.iter(fun _ (_,_,_,clear)->clear())c.classic_invalidators;
    ...
    let create_queue()=match Queue.create device with Error _ as e->e|Ok queue->let queue_token=token c in Hashtbl.add c.active_queues queue_token();
      ...
      let submit command ~resources ~pipelines=let find id=Option.bind(List.assoc_opt id resources)native_resource in
        ...
        |Render submission->
          ...
          let maybe_indirect encoded=
            ...
            match c.plan_cache with None when List.exists argument_pipeline native_draws->error"Ogpu_metal.Backend.render"Ogpu.Error.Unsupported"retained plan cache is unavailable"|None->Ok(encoded,None)|Some _ when native_draws=[]||not(List.for_all eligible native_draws)->Ok(encoded,None)|Some cache->
            let pipeline_identities=List.map(fun(draw:Render_pass.draw)->Pipeline.Private.native_identity draw.pipeline)native_draws in
            ...
            let build icb =
              ...
              let rec loop i=function
                ...
                |(draw:Render_pass.draw)::rest->
                  ...
                  let length=Metal.Shader_argument_encoder.encoded_length encoder in
                  ...
                  encoders:=encoder::!encoders;buffers:=argument_buffer::!buffers;
