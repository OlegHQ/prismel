  ...
  let create_device()=if c.device_live then error"Ogpu_metal.Backend.create_device"Ogpu_core.Error.Invalid_state"adapter already owns a live device"else match obtain_device()with Error _ as e->e|Ok device->c.device_live<-true;c.device_id<-Some(Ogpu_core.Handle.device_id(Device.Private.handle device));let device_token=token c in
          ...
          let maybe_indirect encoded=
            ...
            let build icb =
              ...
              let configure command (draw:Render_pass.draw) argument_buffer =
                ...
                Metal.Indirect_command_buffer.Render_command.draw_primitives command~primitive:(match draw.primitive with Point_list->Metal.Point|Line_list->Metal.Line|Triangle_list->Metal.Triangle|Triangle_strip->Metal.Triangle_strip)~vertex_start:draw.vertex_start~vertex_count:draw.vertex_count()
              in
    ...
    let destroy_device()=
      if Hashtbl.length c.active_queues<>0 then
        ...
        (match destroyed with Ok()->c.device_live<-false;c.device_id<-None|Error _->());
        let cleanup=take_cleanup_error()in
