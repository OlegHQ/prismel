...
let operations=Ogpu.Caps.[Buffer;Texture;Sampler;Compute_pipeline;Render_pipeline;Queue;Surface;Memory;Event_synchronization;Timeline_fence;Timestamp_queries;Ray_tracing;Metal_fx;Sparse_memory;Heaps;Residency_sets;Fences;Mesh_shaders;Tile_shaders;Dynamic_libraries;Binary_archives;Unknown"future"]
...
let run () =
  ...
  List.iter(fun value->if outcome(Ogpu.Caps.require value Ogpu.Caps.Metal_fx)<>"unsupported"||outcome(Ogpu.Caps.require value(Ogpu.Caps.Unknown"future"))<>"unsupported"then failwith"unsupported feature fallback")profiles;
  ...
  if outcome(Device.supports device Ogpu.Caps.Metal_fx)<>(if get_metal(Metal.Fx.Spatial_scaler.supported native)then"supported"else"unsupported")then failwith"MetalFX support drift";
  List.iter(fun feature->if outcome(Device.supports device feature)<>"supported"then failwith"G7 feature reported unsupported on Metal")Ogpu.Caps.[Mesh_shaders;Tile_shaders;Binary_archives];
  ...
  List.iter(fun feature->if outcome(Device.supports device feature)<>"supported"then failwith"G6 feature reported unsupported on Metal")Ogpu.Caps.[Heaps;Fences;Event_synchronization;Timestamp_queries];
  if outcome(Device.supports device Ogpu.Caps.Residency_sets)<>(if get_metal(Metal.Device.supports_residency_sets native)then"supported"else"unsupported")then failwith"residency set support drift";
