...
type pipeline_kind
type t={native:native;library:Metal.Library.t;mutable functions:Metal.Function.t list;descriptor_destroy:(unit->unit)option;handle:pipeline_kind Ogpu_core.Handle.t;device:Device.t;portable:Ogpu_core.Pipeline.t;mutable dead:bool;mutable submission_uses:int;mutable destroy_requested:bool}
type cache={values:t Ogpu_core.Cache.t}
...
let create_cache~capacity=Result.map(fun values->{values})(Ogpu_core.Cache.create~capacity~on_evict:(fun~key:_ value _->ignore(destroy value)))
let cache_length value=Ogpu_core.Cache.length value.values
let cache_keys_lru value=Ogpu_core.Cache.keys_lru value.values
let clear_cache value=Ogpu_core.Cache.clear value.values
let metal_kind=function Ogpu_core.Shader.Uniform_buffer|Storage_buffer->`Buffer|Sampled_texture|Storage_texture->`Texture|Sampler->`Sampler
...
let create_render_runtime_msl ?primitive_topology ?blend cache device descriptor=
  create_render_internal ?primitive_topology ?blend cache device descriptor
let create_render_argument_buffer ?blend cache device descriptor=
  ...
  match descriptor.fragment_entry with None->error operation Ogpu_core.Error.Invalid_argument"argument-buffer pipeline requires a fragment entry"|Some entry->
  ...
  let rollback error=ignore(Ogpu_core.Cache.remove cache.values(Ogpu_core.Pipeline.cache_key value.portable));Error error in
  match value.native with Compute _->rollback(Ogpu_core.Error.make operation Ogpu_core.Error.Invalid_state"argument render factory returned compute pipeline")|Render native->match Metal.Render_pipeline.supports_indirect_command_buffers native with Error e->rollback(Device.of_metal_error~operation e)|Ok false->rollback(Ogpu_core.Error.make operation Ogpu_core.Error.Unsupported"native pipeline does not support indirect commands")|Ok true->
  if value.functions<>[]then Ok value else match Metal.Function.find~library:value.library entry with Error e->rollback(Device.of_metal_error~operation e)|Ok function_->value.functions<-[function_];Ok value
