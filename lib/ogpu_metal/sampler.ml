...
let create device (descriptor:descriptor)=let operation="Ogpu_metal.Sampler.create"in
  if Device.destroyed device then Error(Ogpu.Error.make operation Ogpu.Error.Stale_handle "device is destroyed")
    ...
    match Metal.Sampler.create~device:(Device.Private.metal device)native with Error e->Error(Device.of_metal_error~operation e)|Ok metal->let value={metal;handle=Ogpu.Handle.create~device:(Device.Private.handle device);device;descriptor;submissions=0}in Device.Private.attach_resource device;Ok value
