...
module Private=struct let metal value=value.metal let retain_submission value=if destroyed value then Error(Ogpu.Error.make"Ogpu_metal.Sampler.retain"Ogpu.Error.Stale_handle"sampler is destroyed")else(value.submissions<-value.submissions+1;Ok())let release_submission value=if value.submissions>0 then value.submissions<-value.submissions-1 end
