...
type control={mutable next:int64;queue_clocks:(int64,queue_clock)Hashtbl.t;memory:(int64,bytes)Hashtbl.t;mutable lost:bool;mutable fail_submission:bool;mutable fail_completion:bool;mutable fail_texture_after:int option;mutable fail_depth_after:int option;mutable fail_configure:bool;mutable trace:string list;mutable buffers:int;mutable textures:int;mutable pipelines:int;mutable queues:int;mutable surfaces:int}
...
let execute_buffer_transfers c resources command =
  ...
  |Backend.Private.Transfer descriptions->
      Array.fold_left (fun result description->
        Result.bind result (fun()->match description with
          ...
          (* ponytail: texture transfers still need a mip-aware mock layout;
             implement them when texture data conformance is added. *)
          |Transfer_pass.Buffer_to_texture _
          |Transfer_pass.Texture_to_buffer _
          |Transfer_pass.Copy_texture _->Ok())) (Ok()) descriptions
...
let mock_submit c clock ~operation ~commit command ~resources ~pipelines=
  if c.fail_submission then begin
    ...
    else match execute_buffer_transfers c resources command with
...
let create ?(capabilities=Caps.minimum_m1)()=
  let c={next=1L;queue_clocks=Hashtbl.create 4;memory=Hashtbl.create 16;lost=false;fail_submission=false;fail_completion=false;fail_texture_after=None;fail_depth_after=None;fail_configure=false;trace=[];buffers=0;textures=0;pipelines=0;queues=0;surfaces=0}in
