...
type draw={pipeline_key:string;buffers:buffer_binding list;textures:texture_binding list;samplers:sampler_binding list;primitive:primitive;vertex_start:int;vertex_count:int;instance_count:int;index:(index_type*int64*int64*int)option}
...
let submit pass draws =
  ...
  let seen_buffers=Hashtbl.create 16 and seen_textures=Hashtbl.create 16 and seen_samplers=Hashtbl.create 16 in
    ...
    let rec loop = function
      | [] -> Ok { pass; draws }
      | d :: _ when d.pipeline_key = "" || d.vertex_start < 0 || d.vertex_count <= 0 || d.instance_count <= 0 ->
          invalid "draw pipeline/range is invalid"
      | d :: _ when d.instance_count <> 1 && Option.is_none d.index ->
          invalid "nonindexed instancing is unavailable"
      | d :: _ when Option.is_none d.index &&
          (d.primitive=Point_list || d.primitive=Line_list) ->
          invalid "point and line draws require an index buffer"
      ...
      | d :: _ when d.primitive = Line_list && Option.is_none d.index
             && d.vertex_count mod 2 <> 0 ->
          invalid "line-list count is not divisible by two"
      | d :: ds ->
          ...
          | Ok () -> (
              ...
              | Some (_, _, _, count) when d.primitive=Line_list&&count mod 2<>0 ->
                  invalid "line-list index count is not divisible by two"
              | _ -> loop ds))
    in
  loop draws
