...
let create (c : configuration) =
  let op = "Runtime_next_orchestrator.create" in
  if
    c.logical_width <= 0 || c.logical_height <= 0 || c.drawable_width <= 0 || c.drawable_height <= 0
  then invalid op "dimensions must be positive"
  else
    let finish runtime facts =
      Ok
        {
          runtime;
          facts;
          pacing = { frames = 0L; presented = 0L; last_presented = false };
          logical_draws = 0L;
          logical_passes = 0L;
    ...
    in
    match
      Runtime_next.create ~vsync:c.vsync ~hidden:false ~title:c.title ~width:c.logical_width
        ~height:c.logical_height ()
    with
    | Error _ as e -> e
    | Ok runtime -> (
        match (Runtime_next.set_title runtime c.title, Runtime_next.set_resizable runtime true) with
        | Ok (), Ok () -> (
            match Runtime_next.window_facts runtime ~vsync:c.vsync with
            | Ok f ->
                finish runtime
                  {
                    title = f.title;
                    logical_width = f.logical_width;
                    logical_height = f.logical_height;
                    drawable_width = f.drawable_width;
                    drawable_height = f.drawable_height;
                    position = Some f.position;
                    pixel_density = f.pixel_density;
                    display_scale = f.display_scale;
                    refresh_rate = f.refresh_rate;
                    vsync = f.vsync;
                  }
            | Error e ->
                ignore (Runtime_next.destroy runtime);
                Error e)
        | Error e, _ | _, Error e ->
            ignore (Runtime_next.destroy runtime);
            Error e)
...
let pacing value =
  Result.map (fun () -> value.pacing) (ensure "Runtime_next_orchestrator.pacing" value)

let stats value =
  match ensure "Runtime_next_orchestrator.stats" value with
  | Error _ as e -> e
  | Ok () ->
      ...
      let ( uploaded_bytes,
            ...
            gpu_sample_count,
            retained_plan_builds,
            retained_plan_hits,
            retained_plan_misses,
            retained_plan_evictions,
            retained_plan_executions,
            retained_plan_entries,
            retained_plan_capacity ) =
        ( s.uploaded_bytes,
          s.mesh_cache_entries,
          s.gpu_timing_supported,
          s.gpu_duration_seconds,
          s.gpu_sample_count,
          s.retained_plan_builds,
          s.retained_plan_hits,
          s.retained_plan_misses,
          s.retained_plan_evictions,
          s.retained_plan_executions,
          s.retained_plan_entries,
          s.retained_plan_capacity )
      in
      Ok
        {
          frames = value.pacing.frames;
          presented = value.pacing.presented;
          logical_draws = value.logical_draws;
          logical_passes = value.logical_passes;
          logical_submissions = value.logical_submissions;
          uploaded_bytes;
          cache_entries;
          gpu_timing_supported;
          gpu_duration_seconds;
...
let account value draw_count result =
  ...
  | Ok presented ->
      ...
      value.logical_submissions <- Int64.succ value.logical_submissions
  | Error _ -> ());
  result

let render value draws =
  match ensure "Runtime_next_orchestrator.render" value with
  | Error _ as e -> e
  | Ok () -> account value (List.length draws) (Runtime_next.render value.runtime draws)

let scene_family = function
  | Scene2 -> Scene_execution.Scene2
...
let pipeline_blend = function
  | Replace -> Ogpu.Pipeline.Replace
  | Alpha -> Alpha
  | Add -> Add
  | Multiply -> Multiply
  | Screen -> Screen
...
let sampled_of_prepared value draws =
  if draws == value.last_prepared_in then value.last_sampled_out
    ...
    let out =
      List.map
        (fun x ->
          (scene_family x.family, pipeline_blend x.blend, x.texture, x.auxiliary, x.samples, x.draw))
        draws
    in
...
let pull_window_facts value =
  ...
  if
    f.logical_width = live.logical_width
    && f.logical_height = live.logical_height
    && f.drawable_width = live.drawable_width
    && f.drawable_height = live.drawable_height
    && f.pixel_density = live.pixel_scale_x
    && f.display_scale = live.pixel_scale_x
  then ()
  else
    value.facts <-
      {
...
let render_prepared ?after_prepare ?clear value draws =
  match ensure "Runtime_next_orchestrator.render_prepared" value with
  | Error _ as e ->
  ...
  | Ok () ->
      ...
      in
      (match result with Ok _ -> pull_window_facts value | Error _ -> ());
      result

let render_retained ?after_prepare ?clear ~identity ~version value draws =
  match ensure "Runtime_next_orchestrator.render_retained" value with
  | Error _ as e ->
      Option.iter (fun f -> f ()) after_prepare;
      e
  | Ok () ->
...
let replay_retained ?clear ~identity ~version value =
  ...
  | Ok () -> (
      match
        Runtime_next.replay_prepared_sampled_resources ?clear ~identity ~version value.runtime
      with
      | Error _ as e -> e
      | Ok None ->
...
let resize value ~logical_width ~logical_height ~drawable_width ~drawable_height =
  match ensure "Runtime_next_orchestrator.resize" value with
  | Error _ as e -> e
  | Ok () ->
      let result = Runtime_next.resize value.runtime ~width:logical_width ~height:logical_height in
      (match result with
      | Error _ -> ()
      | Ok () -> (
...
let capture value ~bytes_per_row =
  match ensure "Runtime_next_orchestrator.capture" value with
  | Error _ as e -> e
  | Ok () -> Runtime_next.read_pixels value.runtime ~bytes_per_row

let capture_into value ~bytes_per_row ~destination =
  match ensure "Runtime_next_orchestrator.capture_into" value with
  | Error _ as e -> e
  | Ok () -> Runtime_next.read_pixels_into value.runtime ~bytes_per_row ~destination

let native_call operation value call =
  match ensure operation value with Error _ as e -> e | Ok () -> call value.runtime

let set_title value title =
  match
    native_call "Runtime_next_orchestrator.set_title" value (fun x ->
        Runtime_next.set_title x title)
  with
  | Ok () ->
      value.facts <- { value.facts with title };
      Ok ()
  | Error _ as e -> e

let set_position value ~x ~y =
  match
    native_call "Runtime_next_orchestrator.set_position" value (fun r ->
        Runtime_next.set_position r ~x ~y)
  with
  | Ok () ->
      value.facts <- { value.facts with position = Some (x, y) };
      Ok ()
  | Error _ as e -> e

let center value = native_call "Runtime_next_orchestrator.center" value Runtime_next.center

let set_bordered value x =
  native_call "Runtime_next_orchestrator.set_bordered" value (fun r ->
      Runtime_next.set_bordered r x)

let set_resizable value x =
  native_call "Runtime_next_orchestrator.set_resizable" value (fun r ->
      Runtime_next.set_resizable r x)

let set_always_on_top value x =
  native_call "Runtime_next_orchestrator.set_always_on_top" value (fun r ->
      Runtime_next.set_always_on_top r x)

let set_fullscreen value x =
  native_call "Runtime_next_orchestrator.set_fullscreen" value (fun r ->
      Runtime_next.set_fullscreen r x)

let set_relative_mouse value x =
  native_call "Runtime_next_orchestrator.set_relative_mouse" value (fun r ->
      Runtime_next.set_relative_mouse r x)

let set_cursor value shape =
  native_call "Runtime_next_orchestrator.set_cursor" value (fun r ->
      Runtime_next.set_cursor r shape)

let set_text_input_area value area =
  native_call "Runtime_next_orchestrator.set_text_input_area" value (fun r ->
      Runtime_next.set_text_input_area r area)

let show value =
  match native_call "Runtime_next_orchestrator.show" value Runtime_next.show with
