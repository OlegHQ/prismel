...
let run () =match Runtime_next.create~width:4~height:4() with Error _->print_endline"runtime_next Scene3 lighting: skipped"|Ok runtime->
  ...
  List.iter(fun _frame->let first={base with transform_uniforms=Some red;stencil_state=Some(stencil Ogpu.Render_pass.Always Replace 1);stencil_load=Clear;stencil_clear=0}and second={base with transform_uniforms=Some green;depth_load=Load;stencil_state=Some(stencil Equal Keep 1);stencil_load=Load}in ignore(get(Runtime_next.render_sampled_resources runtime[Scene_execution.Scene3_stencil,Ogpu.Pipeline.Replace,None,None,1,{Scene_execution.mesh;state=first};Scene3_stencil,Ogpu.Pipeline.Replace,None,None,1,{Scene_execution.mesh;state=second}]));let bytes=get(Runtime_next.read_pixels runtime~bytes_per_row:16)in if Char.code(Bytes.get bytes 1)<>255 then failwith"nested stencil pass pixel")[1;2;60;600];
