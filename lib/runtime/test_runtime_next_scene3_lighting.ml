...
let run () =match Runtime_next.create~width:4~height:4() with Error _->print_endline"runtime_next Scene3 lighting: skipped"|Ok runtime->
  ...
  begin match Runtime_next.render_sampled_resources runtime[entry Scene3 {base with transform_uniforms=Some too_many}]with Error error when error.Ogpu.Error.kind=Invalid_argument&&uploaded=(Runtime_next.stats runtime).uploaded_bytes->()|_->failwith"over-64 light block was not rejected atomically"end;
  ...
  List.iter(fun _frame->let first={base with transform_uniforms=Some red;stencil_state=Some(stencil Ogpu.Render_pass.Always Replace 1);stencil_load=Clear;stencil_clear=0}and second={base with transform_uniforms=Some green;depth_load=Load;stencil_state=Some(stencil Equal Keep 1);stencil_load=Load}in ignore(get(Runtime_next.render_sampled_resources runtime[entry Scene3_stencil first;entry Scene3_stencil second]));let bytes=get(Runtime_next.read_pixels runtime~bytes_per_row:((Runtime_next.frame_facts runtime).drawable_width*4))in if Char.code(Bytes.get bytes 1)<>255 then failwith"nested stencil pass pixel")[1;2;60;600];
