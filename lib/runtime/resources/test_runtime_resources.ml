...
let run () =
  ...
  if Canvas.Private.gpu_snapshot direct<>None then failwith"fresh Canvas reports a GPU frame";
  get(Canvas.Private.forget_gpu direct);
  if Canvas.generation direct<>direct_generation then
    failwith"forgetting an absent GPU frame changed the Canvas generation";
  get(Canvas.destroy direct);
  ...
  print_endline"runtime_resources: image/canvas/assets frame600 PNG 100k passed"
