...
let facts window =
  ...
  | Error e, _ -> sdl "Runtime_next.facts" (Error e)
  | _, Error e -> sdl "Runtime_next.facts" (Error e)
  | _ -> error "Runtime_next.facts" "window dimensions are invalid"
  ...
  let make_pipeline backend_device family blend samples =
    let source, extra =
      ...
      | Scene_execution.Scene2 | Scene2_textured ->
          ( Runtime_next_shaders.scene2_textured_argument,
            [
              {
                Ogpu.Shader.group = 0;
                binding = 1;
...
let render ?clear (value : t) draws =
  if value.dead then
    Error (Ogpu.Error.make "Runtime_next.render" Stale_handle "runtime is destroyed")
  else
...
let render_sampled_resources ?after_prepare ?clear (value : t) draws =
  if value.dead then (
    ...
    Error
      (Ogpu.Error.make "Runtime_next.render_sampled_resources" Stale_handle "runtime is destroyed"))
  else
    match sync_window_facts value with
    | Error _ as error ->
        Option.iter (fun f -> f ()) after_prepare;
...
let render_prepared_sampled_resources ?after_prepare ?clear ~identity ~version (value : t) draws =
  if value.dead then (
    Option.iter (fun f -> f ()) after_prepare;
    Error
      (Ogpu.Error.make "Runtime_next.render_prepared_sampled_resources" Stale_handle
...
let replay_prepared_sampled_resources ?clear ~identity ~version (value : t) =
  if value.dead then
    Error
      (Ogpu.Error.make "Runtime_next.replay_prepared_sampled_resources" Stale_handle
         "runtime is destroyed")
  else
    match sync_window_facts value with
    | Error _ as error -> error
    | Ok () ->
        Scene_execution.replay_prepared_sampled_resources ?clear ~identity ~version value.renderer

let resize (value : t) ~width ~height =
  if value.dead then
    Error (Ogpu.Error.make "Runtime_next.resize" Stale_handle "runtime is destroyed")
  else
    match sdl "Runtime_next.resize" (Sdl3.Window.set_size value.window ~width ~height) with
    | Error _ as e -> e
    | Ok () -> Result.bind (facts value.window) (apply_facts value)

let read_pixels (value : t) = Scene_execution.read_pixels value.renderer

let read_pixels_into (value : t) ~bytes_per_row ~destination =
  if value.dead then
    Error (Ogpu.Error.make "Runtime_next.read_pixels_into" Stale_handle "runtime is destroyed")
...
let window_facts (value : t) ~vsync:_ =
  live_window "Runtime_next.window_facts" value (fun window ->
      match
        ( sdl "Runtime_next.window_facts" (Sdl3.Window.presentation_facts window ~vsync:value.vsync),
          sdl "Runtime_next.window_facts" (Sdl3.Window.title window),
          sdl "Runtime_next.window_facts" (Sdl3.Window.position window) )
      with
...
let set_title value title =
  window_call "Runtime_next.set_title" (fun window -> Sdl3.Window.set_title window title) value

let set_position value ~x ~y =
  window_call "Runtime_next.set_position"
          ...
          | None ->
              ...
              Result.map
                (fun cursor ->
                  ...
                  cursor)
                (sdl "Runtime_next.set_cursor" (Sdl3.Cursor.create native))
        in
        match cursor with
        | Error _ as error -> error
        | Ok cursor -> (
...
let set_text_input_area value area =
  window_call "Runtime_next.set_text_input_area"
    (fun window ->
...
let show (value : t) =
  if value.dead then Error (Ogpu.Error.make "Runtime_next.show" Stale_handle "runtime is destroyed")
  else
    match sdl "Runtime_next.show" (reveal value.window) with
    | Error _ as error -> error
    | Ok () -> sync_window_facts value

let hide = window_call "Runtime_next.hide" Sdl3.Window.hide

let visible value =
  live_window "Runtime_next.visible" value (fun window ->
      Result.map
...
let destroy (value : t) =
  if value.dead then Ok ()
    ...
    List.iter
      (fun (_, cursor) -> record (sdl "Runtime_next.destroy" (Sdl3.Cursor.destroy cursor)))
      value.cursors;
    value.cursors <- [];
...
let render_offscreen_prepared ?after_prepare ?clear ~identity ~version value draws =
  if value.dead then (
    Option.iter (fun f -> f ()) after_prepare;
    Error
      (Ogpu.Error.make "Runtime_next.render_offscreen_prepared" Stale_handle
...
let read_offscreen value ~bytes_per_row =
  if value.dead then
    Error
      (Ogpu.Error.make "Runtime_next.read_offscreen" Stale_handle "offscreen target is destroyed")
  else Scene_execution.read_pixels value.renderer ~bytes_per_row

let read_offscreen_into value ~bytes_per_row ~destination =
  if value.dead then
    Error
      (Ogpu.Error.make "Runtime_next.read_offscreen_into" Stale_handle
         "offscreen target is destroyed")
  else Scene_execution.read_pixels_into value.renderer ~bytes_per_row ~destination
...
let resize_offscreen value ~logical_width ~logical_height ~width ~height =
  if value.dead then
    Error
      (Ogpu.Error.make "Runtime_next.resize_offscreen" Stale_handle "offscreen target is destroyed")
