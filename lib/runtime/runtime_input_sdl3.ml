...
let key_name ~scancode keycode =
  (* Space (32) is a named key, not a printable character: hosts match
     [Input.Space]. *)
  if keycode > 32 && keycode <= 126 then String.make 1 (Char.lowercase_ascii (Char.chr keycode))
  else
    match scancode with
    | 40 -> "Enter"
    | 41 -> "Escape"
...
let modifiers bits =
  ...
  [] |> add 0x8000 Scroll_lock |> add 0x2000 Caps_lock |> add 0x1000 Num_lock |> add 0x0c00 Meta
  |> add 0x0300 Alt |> add 0x00c0 Control |> add 0x0003 Shift
...
let translate = function
  ...
  | Text_editing { text; start; length; _ } -> Some (Text_editing { text; start; length })
  | Window { change = Resized (width, height); _ } ->
      Some (Runtime_input.Resized (width, height))
...
let push value event =
  match event with
  | Sdl3.Event.Drop { change = File path; _ } -> Runtime_input.push value (File_dropped path)
  | Sdl3.Event.Mouse_motion { x; y; dx; dy; _ } ->
      Runtime_input.add_motion value ~dx ~dy;
      Runtime_input.push value (Pointer_moved (x, y))
  | _ -> (
      match translate event with None -> Ok () | Some event -> Runtime_input.push value event)
...
let pump value =
  ...
  | Ok events ->
      ...
      List.fold_left
        (fun first event ->
          match (push value event, first) with (Error _ as error), Ok () -> error | _ -> first)
        (Ok ()) events
