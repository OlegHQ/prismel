...
let run () =
  ...
  | Ok runtime ->
      ...
      let mesh : Scene_execution.mesh =
        { key = "scene2-retained-argument"; vertices; vertex_count = 3; indices; index_count = 3; primitive=Ogpu.Render_pass.Triangle_list }
      in
      ...
      for frame = 1 to 20 do
        ...
        if frame = 1 then first_uploaded := Some (Runtime.stats runtime).uploaded_bytes
      done;
      let uploaded = (Runtime.stats runtime).uploaded_bytes in
      ...
      for frame = 1 to 1000 do
        ...
        if not (get (Runtime.render_sampled_resources runtime [changing_draw])) then
      ...
      let churn = Runtime.stats runtime in
      ...
      for _frame = 1 to 600 do
        if not (get (Runtime.render_sampled_resources runtime
      ...
      if promoted_per_frame > 1_024. then
        failwith (Printf.sprintf
          ...
          promoted_per_frame);
      ignore (get (Runtime.render_sampled_resources runtime [animated 2.]));
      let shifted = get (Runtime.read_pixels runtime ~bytes_per_row:((Runtime.frame_facts runtime).drawable_width * 4)) in
      if Bytes.sub shifted 0 4 <> Bytes.of_string "\000\000\000\000" then
        failwith "animated affine translated pixel mismatch";
      ignore (get (Runtime.render_sampled_resources runtime [animated 0.]));
      let restored = get (Runtime.read_pixels runtime ~bytes_per_row:((Runtime.frame_facts runtime).drawable_width * 4)) in
