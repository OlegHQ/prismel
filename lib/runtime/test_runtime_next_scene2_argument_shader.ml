...
let run () =
  ...
  let direct = shader Runtime_next.Private.scene2_textured_direct
  ...
  let argument = shader Runtime_next.Private.scene2_textured_argument
  ...
  let direct_pipeline = get (Backend.create_render_pipeline device (descriptor
    (layout (vertex_entries @ [{binding=1;kind=Binding.Texture;visibility=[Binding.Fragment]};
      {binding=2;kind=Binding.Sampler;visibility=[Binding.Fragment]}])) Runtime_next.Private.scene2_textured_direct direct "scene_fragment")) in
  let argument_pipeline = get (Backend.create_render_pipeline ~indirect:true device (descriptor
    ...
    Runtime_next.Private.scene2_textured_argument argument "scene_fragment_argument")) in
