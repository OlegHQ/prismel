...
let run () =
  ...
  let affine=Bytes.make 24 '\000'in Bytes.set_int32_le affine 0(Int32.bits_of_float 1.);Bytes.set_int32_le affine 16(Int32.bits_of_float 1.);
  let affine_buffer=get(Buffer.create_copy~device~storage:Buffer.Shared affine)in
  ...
  let render pipeline target bind=let command=get(Command_buffer.create queue())in let pass=get(Render_encoder.create command~target())in get(Render_encoder.set_pipeline pass pipeline);get(Render_encoder.set_vertex_buffer pass~index:0~offset:0L vertex_buffer);get(Render_encoder.set_vertex_buffer pass~index:6~offset:0L affine_buffer);bind pass;get(Render_encoder.draw_triangles pass~first:0~count:3());get(Render_encoder.end_encoding pass);get(Command_buffer.commit command);get(Command_buffer.wait_until_completed command);get(Command_buffer.destroy command);get(Texture.read_bytes target~region~mip_level:0~slice:0~bytes_per_row:4~bytes_per_image:4)in
  ...
  let argument_pixels=render pipeline argument_target(fun pass->get(Render_encoder.set_fragment_buffer pass~index:1~offset:0L argument_buffer);get(Render_encoder.use_resource pass(Texture_resource sampled)~usage:[Read;Sample]~stages:[Fragment]))in
  if direct_pixels<>argument_pixels||direct_pixels<>Bytes.of_string"\x11\x22\x33\xff"then failwith(Printf.sprintf"direct/argument-buffer pixel mismatch: direct=%S argument=%S"(Bytes.to_string direct_pixels)(Bytes.to_string argument_pixels))else();
  get(Shader_argument_encoder.destroy encoder);
  get(Command_queue.destroy queue);get(Texture.destroy argument_target);get(Texture.destroy direct_target);get(Buffer.destroy argument_buffer);get(Sampler.destroy sampler);get(Texture.destroy sampled);get(Buffer.destroy affine_buffer);get(Buffer.destroy vertex_buffer);
