...
type t={server:Wap.t;mutable destroyed:bool}
let create ?config ()=match Wap.start ?config()with Error message->Error(Transport message)|Ok server->Ok{server;destroyed=false}
let validate frame=
