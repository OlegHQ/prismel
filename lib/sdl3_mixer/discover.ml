...
let () =
  C.main ~name:"sdl3_mixer" (fun config ->
    let package, core_package =
      match C.Pkg_config.get config with
      | None -> None, None
      | Some pkg_config ->
          C.Pkg_config.query pkg_config ~package:"sdl3-mixer",
          C.Pkg_config.query pkg_config ~package:"sdl3"
    in
    let cflags =
      match environment "PRISMEL_SDL3_MIXER_INCLUDE_DIR", package with
      | Some directory, _ -> ["-I" ^ directory]
      | None, Some package -> package.cflags
      | None, None -> []
    in
    let libraries =
      match environment "PRISMEL_SDL3_MIXER_LIB_DIR", package with
      | Some directory, _ -> ["-L" ^ directory; "-lSDL3_mixer"]
      | None, Some package ->
