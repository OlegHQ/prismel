...
let () =
  ...
  List.iter (fun scale ->
    ...
    Fun.protect ~finally:(fun () -> ignore (get (Prismel_next_execution.destroy execution)))
      (fun () ->
        let render ir =
          let draws=get (Prismel_next_execution.lower_scene2 execution ~density:scale
            ~resource:(fun _ -> None) ir) in
          ignore (get (Prismel_next_execution.step ~clear:(0.02,0.03,0.04,1.) execution draws));
          get (Prismel_next_execution.capture execution) in
        List.iter (fun count ->
          for frame=0 to 3 do
        ...
        let retained identity draws =
          ...
          let batch=get (Prismel_next_execution.Private.adopt_draws submission draws) in
          ignore (get (Prismel_next_execution.Private.step ~identity ~version:1L
