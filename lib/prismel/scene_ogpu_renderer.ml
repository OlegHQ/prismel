...
let create driver configuration=Result.map(fun execution->{execution;draw_states=[||];configuration;destroyed=false})(backend(Scene_execution.create_variants driver configuration))
...
let render_with_scene3 value resources scene=if value.destroyed then Error Destroyed else let scene2,views=split_scene value scene in match Scene_raster2_lowering.lower scene2 with Error(Unsupported _)->Error Unsupported_resource|Error e->Error(Scene e)|Ok ir->let _,draws2=Raster2.Render_ir.commands ir|>Array.fold_left(fun(active,out)->function Raster2.Render_ir.Set_blend blend->blend,out|Geometry g->active,(pipeline_blend active,draw2 value.configuration g)::out|_->active,out)(Raster2.Composite.Source_over,[])in let draws2=List.rev draws2 in let rec lower ds ss=function []->Ok(List.rev ds,List.rev ss)|Scene_description.View3d(camera,scene,Some viewport)::rest->let node=Scene_description.View3d(camera,scene,Some viewport)in(match Scene3_raster2_lowering.lower_view3d~resources~default_viewport:viewport node with Error e->Error(Scene3 e)|Ok prepared when prepared.samples<>1->Error(Unsupported_samples prepared.samples)|Ok prepared when Array.exists(fun(draw:Raster2.Scene3_consumer.draw)->Option.is_some draw.program)prepared.draws->Error Unsupported_resource|Ok prepared->let rec pairs acc=function []->Ok(List.rev acc)|(draw:Raster2.Scene3_consumer.draw)::tail->let shadows=Array.to_list draw.shadows|>List.filter_map Fun.id in if List.length shadows>1||shadows<>[]&&Option.is_some draw.texture then Error Unsupported_resource else let auxiliary=match shadows with []->Ok None|shadow::_->Result.map Option.some(auxiliary_shadow shadow)in Result.bind auxiliary(fun auxiliary->let rendered,state=draw3 draw in pairs((pipeline_blend state.blend,Option.map sampled_texture state.texture,auxiliary,rendered,state)::acc)tail)in Result.bind(pairs[](Array.to_list prepared.draws))(fun pairs->lower(List.rev_append(List.map(fun(blend,texture,auxiliary,draw,_)->blend,texture,auxiliary,draw)pairs)ds)(List.rev_append(List.map(fun(_,_,_,_,s)->{s with depth_clear=prepared.clear_depth})pairs)ss)rest))|_::_->assert false in Result.bind(lower[][]views)(fun(draws3,states)->value.draw_states<-Array.of_list states;let draws=List.map(fun(blend,draw)->Scene_execution.Scene2,blend,None,None,draw)draws2@List.map(fun(blend,texture,auxiliary,draw)->(match auxiliary,texture with Some _,None->Scene_execution.Scene3_shadow|None,Some _->Scene3_textured|None,None->Scene3|Some _,Some _->assert false),blend,texture,auxiliary,draw)draws3 in backend(Scene_execution.render_resources value.execution draws))
