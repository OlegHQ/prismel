...
let transform_uniforms (draw:Raster2.Scene3_consumer.draw)=
  ...
  let values=Array.concat[draw.matrix;draw.model_matrix;normal;[|draw.camera_position.x;draw.camera_position.y;draw.camera_position.z;0.|]]in
  let bytes=Bytes.create 208 in Array.iteri(fun index value->Bytes.set_int32_le bytes(index*4)(Int32.bits_of_float value))values;bytes
