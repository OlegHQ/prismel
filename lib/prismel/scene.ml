...
let draw_style draw_filled draw_outline style =
  Option.iter draw_filled style.fill;
  Option.iter draw_outline style.stroke
...
let rec render_node = function
  ...
  | Rect (at, w, h, radius, style) ->
      let draw filled color =
        match radius with
        | None -> Graphics.rect ~pos:at ~w ~h ~filled ~color ()
        | Some radius ->
            Graphics.rounded_rect ~pos:at ~w ~h ~radius ~filled ~color ()
      in
      draw_style (draw true) (draw false) style
  | Circle (at, radius, style) ->
      let draw filled color =
        Graphics.circle ~center:at ~radius ~filled ~color ()
      in
      draw_style (draw true) (draw false) style
  | Ellipse (at, rx, ry, style) ->
      let draw filled color =
        Graphics.ellipse ~center:at ~rx ~ry ~filled ~color ()
      in
      draw_style (draw true) (draw false) style
  | Triangle (p1, p2, p3, style) ->
      let draw filled color =
        Graphics.triangle ~p1 ~p2 ~p3 ~filled ~color ()
      in
      draw_style (draw true) (draw false) style
  | Polygon (points, style) ->
      let draw filled color =
        Graphics.polygon ~points ~filled ~color ()
      in
      draw_style (draw true) (draw false) style
