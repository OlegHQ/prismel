...
module Private=struct
 ...
 let glyph ?(density=1) font code=
   ...
   match Prismel_next_resources.Font.glyph_metrics_at font.resource~density code with
   ...
   |Ok metrics->
     ...
     match Prismel_next_resources.Font.render font.resource~density
     ...
     |Ok(Some rendered)->
       Fun.protect~finally:(fun()->ignore(Prismel_next_resources.Text.destroy rendered))
         (fun()->match Prismel_next_resources.Text.size rendered,
             Prismel_next_resources.Text.pixels rendered with
