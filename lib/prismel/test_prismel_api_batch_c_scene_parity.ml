...
let run () =
  ...
  List.iter
    (function
      | identity, Prismel_execution.Image image when Some identity= !automatic_text_id ->
          require (Result.is_ok (Runtime_resources.Image.pixels image)) "released automatic cache image"
      | _, Prismel_execution.Image image ->
          require (Result.is_error (Runtime_resources.Image.pixels image)) "released owned staged image"
      | _, Prismel_execution.Text _ | _, Prismel_execution.Canvas _ ->
          failwith "unexpected staged resource")
    first_resources;
  Font.shutdown();
  List.iter(function
    |identity,Prismel_execution.Image image when Some identity= !automatic_text_id->
      require(Result.is_error(Runtime_resources.Image.pixels image))"automatic image survived shutdown"
    |_->())first_resources;
