...
let instance_uniforms ~camera ~viewport scene drawing transforms =
  ...
  let base=uniforms ~camera ~viewport scene drawing in
  ...
  Array.iteri (fun i transform ->
    let world=Mat4.mul parent transform in
    let offset=1364+i*48 in
    matrix bytes offset (Mat4.mul projection world);
    matrix bytes (offset+16) world;
    matrix bytes (offset+32) (match Mat4.inverse world with
      |None->Mat4.identity|Some inverse->Mat4.transpose inverse)) transforms;
  bytes
type packed_mesh={
  mesh:Mesh.t;
  ...
  Scene3.Private.iter_batches (fun (drawing:Scene3.Private.drawing) transforms->
    ...
    |Ok packed->
        ...
        |Ok texture,Ok auxiliary->
          ...
          |None->add(uniforms ~camera ~viewport scene drawing)
          |Some transforms->
              ...
              for start=0 to (Array.length transforms-1)/batch do
                ...
                add(instance_uniforms ~camera ~viewport scene drawing
                  (Array.sub transforms first count))
              done)
