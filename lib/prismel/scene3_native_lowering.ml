...
let triangle_indices mode source=match mode with
  ...
  |_->None
let packed_of_mesh mesh=
  match List.find_opt(fun packed->packed.mesh==mesh)!packed_meshes with
  |Some packed->Ok packed
  |None->
      let view=Mesh.Private.view mesh in
      match triangle_indices view.mode view.indices,pack_vertices view with
      |Some native_indices,Ok vertices->
          incr next_packed_id;
          let packed={mesh;key=Printf.sprintf"scene3:%d"!next_packed_id;
