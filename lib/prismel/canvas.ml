type t={resource:Runtime_resources.Canvas.t;
  mutable execution:Prismel_execution.t option;mutable destroyed:bool}
...
let execution_message operation error=
  Format.asprintf"%s: %a"operation Prismel_execution.pp_error error
let execution value=
...
let render value scene=
  ...
  |Error error->
    ...
    match Prismel_execution.offscreen_target execution with
    |Error error->failwith(execution_message"Canvas.render"error)
    |Ok texture->
...
module Private=struct
  ...
  let native_stats value=match value.execution with
    ...
    |Some execution->match Prismel_execution.stats execution with
...
let destroy value=if not value.destroyed then(
  Option.iter(fun execution->
    ...
    ignore(Prismel_execution.destroy execution))
    value.execution;
