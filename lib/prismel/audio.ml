type state={audio:Runtime_resources.Audio.t;mutable music_playing:bool}
...
let set_master_volume volume=match current"Audio.set_master_volume"with Ok value->ignore(Runtime_resources.Audio.set_master_volume value.audio volume)|Error _->()
let stop_all()=match!engine with None->()|Some value->List.init(Runtime_resources.Audio.channel_count value.audio)Fun.id|>List.iter(fun channel->ignore(Runtime_resources.Audio.stop_channel value.audio channel()));value.music_playing<-false
...
module Sample=struct
  ...
  type t={resource:Runtime_resources.Audio.sample;mutable destroyed:bool;mutable volume:float}
  ...
  let stop channel=match!engine with Some value->ignore(Runtime_resources.Audio.stop_channel value.audio channel())|None->()
  let pause channel=match!engine with Some value->ignore(Runtime_resources.Audio.pause_channel value.audio channel)|None->()
  let resume channel=match!engine with Some value->ignore(Runtime_resources.Audio.resume_channel value.audio channel)|None->()
  let is_playing channel=match!engine with Some value->Runtime_resources.Audio.channel_playing value.audio channel=Ok true|None->false
...
module Music=struct
  ...
  let play ?loops ?(fade_ms=0) value=match current"Audio.Music.play"with Error _ as error->error|Ok audio->
    begin match Runtime_resources.Audio.play_music audio.audio ?loops ~fade_in_ms:fade_ms value.Sample.resource with Ok()->audio.music_playing<-true;Ok()|Error error->Error(message"Audio.Music.play"error)end
  let set_volume value=match!engine with Some audio->ignore(Runtime_resources.Audio.set_music_volume audio.audio value)|None->()
  let pause()=match!engine with Some audio->ignore(Runtime_resources.Audio.pause_music audio.audio)|None->()
  let resume()=match!engine with Some audio->ignore(Runtime_resources.Audio.resume_music audio.audio)|None->()
  let stop ?(fade_ms=0)()=match!engine with Some audio->ignore(Runtime_resources.Audio.stop_music audio.audio ~fade_out_ms:fade_ms());audio.music_playing<-false|None->()
