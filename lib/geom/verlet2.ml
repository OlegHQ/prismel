...
and behavior = particle -> dt:float -> Vec2.t
...
let particle ?(mass = 1.) ?(locked = false) ?(velocity = Vec2.zero) position =
  if not (Float.is_finite mass) || mass <= 0. then invalid_arg "Verlet2.particle: mass must be finite and positive";
  { position; previous = Vec2.sub position velocity; force = Vec2.zero;
...
let velocity particle = Vec2.sub particle.position particle.previous
...
let with_position ?(preserve_velocity = false) position particle =
  let previous = if preserve_velocity then Vec2.sub position (velocity particle) else position in
  { particle with position; previous }

let with_velocity velocity particle = { particle with previous = Vec2.sub particle.position velocity }
let add_force force particle = { particle with force = Vec2.add particle.force force }
let clear_force particle = { particle with force = Vec2.zero }
...
let validate_spring strength rest_length =
  if not (Float.is_finite strength) || strength < 0. || strength > 1. then invalid_arg "Verlet2.spring: strength must be in zero to one";
  if not (Float.is_finite rest_length) || rest_length < 0. then invalid_arg "Verlet2.spring: rest length must be finite and non-negative"

let spring ?(strength = 1.) ?rest_length a b =
  if a < 0 || b < 0 || a = b then invalid_arg "Verlet2.spring: particle indices must be distinct and non-negative";
...
let pullback_spring ?(strength = 1.) ~rest_length ~min_length a b =
  validate_spring strength rest_length;
  if not (Float.is_finite min_length) || min_length < 0. then invalid_arg "Verlet2.pullback_spring: min_length must be non-negative";
...
let validate_indices count springs =
  ...
  | None -> Ok ()
  | Some _ -> Error "Verlet2.create: spring particle index is out of range"

let create ?(drag = 0.01) ?(iterations = 4) ?(behaviors = []) ?(constraints = []) particles springs =
  if not (Float.is_finite drag) || drag < 0. || drag > 1. then invalid_arg "Verlet2.create: drag must be in zero to one";
  if iterations <= 0 then invalid_arg "Verlet2.create: iterations must be positive";
  Result.map (fun () ->
      { particles = Array.of_list particles; springs = Array.of_list springs;
        drag; iterations; behaviors; constraints })
...
let particle_at index world = if index < 0 || index >= particle_count world then None else Some world.particles.(index)

let with_particle index particle world =
  if index < 0 || index >= particle_count world then Error "Verlet2.with_particle: index is out of range"
  else let particles = Array.copy world.particles in particles.(index) <- particle; Ok { world with particles }

let add_particle particle world = { world with particles = Array.append world.particles [|particle|] }
...
let add_behavior_to_world behavior world = { world with behaviors = behavior :: world.behaviors }
let add_constraint_to_world constraint_ world = { world with constraints = constraint_ :: world.constraints }
...
let integrate drag dt global_behaviors particle =
  if particle.locked then clear_force particle
    ...
    let add_behavior (fx, fy) behavior =
      let added = behavior particle ~dt in
      fx +. added.Vec2.x, fy +. added.y
    in
    let fx, fy =
      List.fold_left add_behavior
        (particle.force.x, particle.force.y) global_behaviors
    ...
    let position = Vec2.create
        ...
        (particle.position.y
         ...
         +. (fy *. acceleration_scale)) in
    { particle with position; previous = particle.position; force = Vec2.zero }
...
let solve_spring particles spring =
  ...
  and dy = right.position.y -. left.position.y in
  let distance = sqrt ((dx *. dx) +. (dy *. dy)) in
  let active = match spring.min_length with None -> true | Some minimum -> distance > minimum in
  if active && distance > 1e-12 then begin
    ...
    if total > 0. then begin
      ...
      if left_weight > 0. then begin
        ...
        particles.(spring.a) <- { left with position = Vec2.create
      ...
      if right_weight > 0. then begin
        ...
        particles.(spring.b) <- { right with position = Vec2.create
...
let step ~dt world =
  if not (Float.is_finite dt) || dt <= 0. then invalid_arg "Verlet2.step: dt must be finite and positive";
  ...
  for _ = 1 to world.iterations do
    ...
    if has_constraints then
      Array.iteri (fun index (particle : particle) ->
...
let attract ~center ~radius ~strength particle ~dt:_ =
  if not (Float.is_finite radius) || radius <= 0. then invalid_arg "Verlet2.attract: radius must be positive";
  let delta = Vec2.sub center particle.position in
  let distance_sq = Vec2.length_sq delta in
  let radius_sq = radius *. radius in
  if distance_sq <= 1e-18 || distance_sq >= radius_sq then Vec2.zero
  else Vec2.scale (Vec2.normalize delta) (strength *. (1. -. (distance_sq /. radius_sq)))

let align ~velocity:target ~strength particle ~dt:_ = Vec2.scale (Vec2.sub target (velocity particle)) strength

let set_constrained_position position particle = { particle with position }

let inside_bounds bounds particle =
  set_constrained_position (Bounds2.closest_point bounds particle.position) particle

let on_circle circle particle = set_constrained_position
    (let delta = Vec2.sub particle.position circle.Circle2.center in
     if Vec2.length_sq delta <= 1e-18 then Circle2.point_at circle 0.
     else Vec2.add circle.center (Vec2.scale (Vec2.normalize delta) circle.radius)) particle

let inside_circle circle particle = if Circle2.contains circle particle.position then particle else on_circle circle particle

let outside_circle circle particle = if Circle2.contains circle particle.position then on_circle circle particle else particle
