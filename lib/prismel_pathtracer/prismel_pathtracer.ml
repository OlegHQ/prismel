...
type pending = {
  commands : Metal.Command_buffer.t;
  slot : int;
  mutable stale : bool;
  preview : bool;
  camera : camera;
...
type gpu_mesh = {
  positions : Metal.Buffer.t;
  normals : Metal.Buffer.t;
  material_ids : Metal.Buffer.t;
  materials : Metal.Buffer.t;
...
type mesh_build = { gpu : gpu_mesh; scratch : Metal.Buffer.t; commands : Metal.Command_buffer.t }
  ...
  Ok
    {
      ...
      mesh_ids;
      triangles;
      mesh_materials = List.map snd objects;
      instances = None;
    }
...
let mesh_instanced ~prototype:(geometry, material) transforms =
  ...
  else if count > max_int / 84 then Error "path tracer transform storage overflows"
  else
      ...
      (fun instance matrix ->
        let values =
          Array.init 16 (fun index ->
              Prismel.Mat4.get matrix ~row:(index / 4) ~column:(index mod 4))
        in
        let a = values.(0)
        and b = values.(1)
        and c = values.(2)
        ...
        if
          (not
             (Array.for_all
                (fun value -> Float.is_finite value && Float.abs value <= 3.402823466e38)
                values))
          || Float.abs values.(12) > 1e-6
          || Float.abs values.(13) > 1e-6
          || Float.abs values.(14) > 1e-6
          || Float.abs (values.(15) -. 1.) > 1e-6
          || (not (Float.is_finite determinant))
          || determinant = 0.
        then
          failure := Some "path tracer instance transform must be finite, affine, and nonsingular"
        else begin
          matrices.(instance) <- values;
          let base = instance * 84 in
          for column = 0 to 3 do
            for row = 0 to 2 do
              put_f32 shader_bytes (base + (((column * 3) + row) * 4)) values.((row * 4) + column)
            done
          done;
          for column = 0 to 2 do
            for row = 0 to 2 do
              ...
              put_f32 shader_bytes (base + 48 + (((column * 3) + row) * 4)) value
            done
          done
        end)
      transforms;
...
let shared device bytes =
  metal (Metal.Buffer.create_copy ~device ~storage:Metal.Buffer.Shared bytes)

let destroy_gpu gpu =
  Option.iter
    (fun descriptor -> ignore (Metal.Acceleration_structure.Instance.destroy descriptor))
  ...
  if gpu.owns_prototype then begin
    List.iter
      (fun buffer -> ignore (Metal.Buffer.destroy buffer))
      [ gpu.positions; gpu.normals; gpu.material_ids; gpu.materials ];
    Option.iter
      (fun primitive -> ignore (Metal.Acceleration_structure.destroy primitive))
      gpu.primitive
  end
...
let upload_mesh_async ?reuse device queue mesh =
  let buffers = ref []
  and structures = ref []
  and instance_descriptor = ref None
  ...
  let result =
    let* positions, normals, material_ids, materials, primitive, triangle, primitive_scratch =
      match reused with
      | Some (gpu, primitive) ->
          Ok (gpu.positions, gpu.normals, gpu.material_ids, gpu.materials, primitive, None, 0L)
      | None ->
          ...
          let* triangle =
            metal
              (Metal.Acceleration_structure.Triangle.create ~vertex_buffer:positions
                 ~vertex_stride:12L
                 ~triangle_count:(Int64.of_int (Bytes.length mesh.mesh_positions / 36))
                 ())
          in
          let* sizes = metal (Metal.Acceleration_structure.sizes ~device triangle) in
          let* primitive =
            metal
              (Metal.Acceleration_structure.create ~device ~size:sizes.acceleration_structure_size)
          in
    ...
    let* structure, descriptor, transforms, scratch_size =
      ...
      | Some instances ->
          ...
          let* descriptor =
            metal
              (Metal.Acceleration_structure.Instance.create ~device ~primitive
                 ~transforms:instances.matrices)
          in
          ...
          Ok
            ( top,
              Some descriptor,
              Some transforms,
              Int64.max primitive_scratch sizes.build_scratch_buffer_size )
    in
    let* work =
      metal (Metal.Buffer.create ~device ~length:scratch_size ~storage:Metal.Buffer.Private ())
    in
    scratch := Some work;
    ...
    let* () =
      ...
      | Some triangle ->
          ...
          let* () =
            metal
              (Metal.Acceleration_encoder.build encoder ~destination:primitive ~descriptor:triangle
                 ~scratch:work ~scratch_offset:0L)
          in
          metal (Metal.Acceleration_encoder.end_encoding encoder)
    in
    let* () =
      ...
      | Some descriptor ->
          ...
          let* () =
            metal
              (Metal.Acceleration_encoder.build_instances encoder ~destination:structure ~descriptor
                 ~scratch:work ~scratch_offset:0L)
          in
          metal (Metal.Acceleration_encoder.end_encoding encoder)
    in
        ...
        gpu =
          {
            ...
            prototype_key =
              (match mesh.instances with
              | None -> None
              | Some _ -> Some (mesh.mesh_positions, mesh.mesh_normals, mesh.mesh_ids, material_data));
...
let create ?(spp = 1) ?(bounces = 6) ?(exposure = 1.) ?(round_samples = 4) ~width ~height scene =
  if width <= 0 || height <= 0 then Error "path tracer size must be positive"
    ...
    if not info.raytracing then Error "Metal device does not support ray tracing"
      ...
      let* library =
        metal (Metal.Library.compile_source ~label:"prismel_pathtracer" ~device source)
      in
      ...
      let* instance_library =
        metal
          (Metal.Library.compile_source ~label:"prismel_pathtracer_instanced" ~device
             ("#define INSTANCED 1\n" ^ source))
      in
      let* instance_function =
        metal (Metal.Function.find ~library:instance_library "pathtrace_instanced")
      in
      let* instance_pipeline = metal (Metal.Compute_pipeline.create instance_function) in
      let pixel_count = width * height in
      let* accum =
        metal
          (Metal.Buffer.create ~device
             ~length:(Int64.of_int (pixel_count * 16))
             ~storage:Metal.Buffer.Private ())
      in
      let history () =
        metal
          (Metal.Buffer.create ~device
      ...
      in
      let* outputs =
        match Prismel_next_execution.Private.active_window_runtime () with
        | None -> (
            let create () =
              Result.map
                (fun native -> { portable = None; native })
                (metal
                   (Metal.Texture.create ~device
            ...
            | Error _ as failure ->
                ignore (Metal.Texture.destroy output0.native);
                failure)
        | Some runtime -> (
            let create () =
              Result.map_error Ogpu.Error.to_string
                (Runtime_next_orchestrator.gpu_film_texture runtime ~width ~height)
            in
            let* portable0, native0 = create () in
            match create () with
            | Ok (portable1, native1) ->
                Ok
                  [|
                    { portable = Some portable0; native = native0 };
                    { portable = Some portable1; native = native1 };
                  |]
            | Error _ as failure ->
                ignore (Ogpu.Backend.destroy_texture portable0);
                failure)
      in
      Ok
        {
...
let publish t (pending : pending) =
  ...
  if pending.stale then Ok ()
    ...
    match t.outputs.(pending.slot).portable with
    | Some film ->
        let* () =
          Result.map_error
            (Format.asprintf "%a" Prismel_next_resources.pp_error)
...
let render t (camera : camera) =
  ...
  if not free then Ok ()
    ...
    let set index buffer =
      metal (Metal.Compute_encoder.set_buffer encoder ~index ~offset:0L buffer)
    in
    let* () =
      metal
        (Metal.Compute_encoder.set_pipeline encoder
           (if t.gpu.transforms = None then t.pipeline else t.instance_pipeline))
    in
    let* () =
      metal
        (Metal.Compute_encoder.set_acceleration_structure encoder ~index:0 (Some t.gpu.structure))
    in
    ...
    let* () = metal (Metal.Compute_encoder.set_texture encoder ~index:0 t.outputs.(slot).native) in
    ...
    let* () =
      metal (Metal.Compute_encoder.dispatch_threads encoder ~threads ~threadgroup:(16, 16, 1))
    in
    ...
    let* () =
      if not t.moving then Ok ()
        ...
        let set index buffer =
          metal (Metal.Compute_encoder.set_buffer resolve ~index ~offset:0L buffer)
        in
        ...
        let* () =
          metal (Metal.Compute_encoder.set_texture resolve ~index:0 t.outputs.(slot).native)
        in
        let* () =
          metal (Metal.Compute_encoder.dispatch_threads resolve ~threads ~threadgroup:(16, 16, 1))
        in
        metal (Metal.Compute_encoder.end_encoding resolve)
    in
...
let destroy t =
  ...
  destroy_gpu t.gpu;
  List.iter
    (fun buffer -> release (fun () -> Metal.Buffer.destroy buffer))
    [
      t.panels;
      t.lights;
      t.accum;
  ...
  Array.iter
    (fun film ->
      match film.portable with
      | Some portable -> ignore (Ogpu.Backend.destroy_texture portable)
      | None -> release (fun () -> Metal.Texture.destroy film.native))
    t.outputs;
