...
let run () =
  ...
  let sphere = get (Result.map_error Pdk.Error.to_string
    (Pdk.Uv_sphere.run ~center:(Prismel.Vec3.create 0. 1. 0.) ~segments:24 ~rings:12 ~radius:1. ())) in
  let floor = get (Result.map_error Pdk.Error.to_string
    (Pdk.Box_generator.box ~center:(Prismel.Vec3.create 0. (-0.05) 0.) ~size:(Prismel.Vec3.create 20. 0.1 20.) ())) in
  let scene =
    { P.objects =
