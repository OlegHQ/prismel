...
let test_constant_linear_and_quadratic () =
  ...
  let source = with_attribute "constant" (Attribute.Float (Array.make count 7.))
      source in
  let constant = Laplacian.run ~source:"constant" source |> get in
  ...
  let linear = Laplacian.run ~points:center ~source:"linear" source
      |> get |> scalar "linear_laplacian" in
  check (near linear.(4) 0.) "cotangent Laplacian lost planar linear precision";
  let quadratic = Laplacian.run ~points:center ~source:"quadratic"
      source |> get |> scalar "quadratic_laplacian" in
  let quadratic_sum = Laplacian.run ~points:center ~normalize:false
...
let test_uniform_and_integrated () =
  ...
  let center = center_group source in
  let average = Laplacian.run ~points:center
      ~weighting:Laplacian.Laplacian_uniform ~source:"impulse" source
      |> get |> scalar "impulse_laplacian" in
  check (average.(4) = -1.) "normalized uniform Laplacian is not neighbor average";
  let sum = Laplacian.run ~points:center
      ~weighting:Laplacian.Laplacian_uniform ~normalize:false ~source:"impulse"
      ~output:"sum" source |> get |> scalar "sum" in
  check (sum.(4) = -4.) "integrated uniform Laplacian has wrong valence sum";
  let cotan = Laplacian.run ~points:center ~normalize:false
...
let test_position_scale_and_storage () =
  ...
  let integer = Laplacian.run ~weighting:Laplacian.Laplacian_uniform
      ~source:"integer" base |> get in
  ignore (scalar "integer_laplacian" integer);
  let pair = Laplacian.run ~source:"pair" base |> get in
  ...
  let tuple = Laplacian.run ~source:"tuple" base |> get in
...
let test_selection_and_exact_domains () =
  ...
  let source = with_attribute "result" (Attribute.Float (Array.make 9 42.))
      source in
  let center = center_group source in
  let output = Laplacian.run ~points:center ~source:"value"
  ...
  List.iter (fun weighting ->
    ...
    [Laplacian.Laplacian_cotan;Laplacian.Laplacian_positive_cotan;Laplacian.Laplacian_uniform]
...
let test_validation_and_cancellation () =
  let source = grid () in
  expect_invalid (fun () -> Laplacian.run ~grain:0 ~source:"P" source)
    "zero grain";
  expect_invalid (fun () -> Laplacian.run ~source:"missing" source)
    "missing source";
  expect_invalid (fun () -> Laplacian.run ~source:"P" ~output:"P" source)
    "position output";
  let primitive_group = Group.init ~owner:Group.Primitive ~name:"wrong"
      (Geometry.primitive_count source) (fun _ -> true) in
  expect_invalid (fun () -> Laplacian.run ~points:primitive_group
      ~source:"P" source) "wrong group owner";
  let text = with_attribute "text" (Attribute.Text (Array.make 9 "x")) source in
  expect_invalid (fun () -> Laplacian.run ~source:"text" text)
    "text source";
  ...
  expect_invalid (fun () -> Laplacian.run ~source:"P" degenerate)
    "degenerate metric";
  let cancel = Cancel.create () in Cancel.cancel cancel;
  (match Laplacian.run ~cancel ~source:"P" source with
