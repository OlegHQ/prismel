...
let test_unique_and_selection () =
  let source = base () in
  let all = Point_split.run source |> get in
  ...
  let points = Point_split.run ~selection:point_selection source |> get in
  ...
  let vertex = Point_split.run ~selection:vertex_selection source |> get in
  ...
  let primitive = Point_split.run ~selection:primitive_selection source |> get in
  ...
  let empty = Group.init ~grain:1 ~owner:Group.Point ~name:"empty" 5
      (Fun.const false) in
  check (Point_split.run ~selection:(Transform_ops.Selected_points empty) source |> get == source)
    "Point Split empty selection identity"

let test_attribute_clusters_and_tolerance () =
  let source = base () in
  let split = Point_split.run ~attributes:"u*" source |> get in
  check (Geometry.point_count split = 7) "Point Split float2 seam clustering";
  let exact = base ~delta:0. () in
  check (Point_split.run ~attributes:"uv" exact |> get == exact)
    "Point Split equal attribute identity";
  let near = base ~delta:0.005 () in
  check (Point_split.run ~attributes:"uv" ~tolerance:0.01 near |> get == near)
    "Point Split inclusive tolerance merge";
  check (Geometry.point_count
      (Point_split.run ~attributes:"uv" ~tolerance:0.001 near |> get) = 7)
    "Point Split tolerance separation";
  check (Geometry.point_count
      (Point_split.run ~attributes:"material" source |> get) = 7)
    "Point Split primitive integer clustering";
  check (Geometry.point_count
      (Point_split.run ~attributes:"weights" source |> get) = 7)
    "Point Split float-array clustering";
  check (Geometry.point_count
      (Point_split.run ~attributes:"tag" source |> get) = 7)
    "Point Split text clustering"

let test_group_clusters () =
  let source = base ~delta:0. () in
  let vertex = Point_split.run ~attributes:"one_corner" source |> get in
  check (Geometry.point_count vertex = 6)
    "Point Split vertex-group seam clustering";
  let primitive = Point_split.run ~attributes:"first_face" source |> get in
  check (Geometry.point_count primitive = 7)
    "Point Split primitive-group seam clustering";
  let both = Point_split.run ~attributes:"*_corner first_*" source |> get in
  check (Geometry.point_count both = 7)
    "Point Split wildcard group seam clustering";
  let excluded = Point_split.run
  ...
  let promoted = Point_split.run ~attributes:"first_face"
  ...
  check (match Point_split.run ~attributes:(Group.name seam_points) source with
...
let test_partial_attribute_selection () =
  ...
  check (Point_split.run ~selection:selected ~attributes:"uv" exact |> get == exact)
    "Point Split separated an equal selected/unselected seam";
  let differing = base () in
  let selected = Transform_ops.Selected_vertices
      (group Group.Vertex "one_corner" differing) in
  let split = Point_split.run ~selection:selected ~attributes:"uv" differing
      |> get in
...
let test_payload_promotion_and_groups () =
  let source = base () in
  let output = Point_split.run ~attributes:"uv tag weights"
      ~promote_attributes:true source |> get in
...
let test_malformed_and_cancellation () =
  ...
  expect_error "negative tolerance" (Point_split.run ~tolerance:(-1.) source);
  expect_error "non-finite tolerance" (Point_split.run ~tolerance:nan source);
  expect_error "missing attribute pattern"
    (Point_split.run ~attributes:"missing" source);
  expect_error "malformed pattern" (Point_split.run ~attributes:"[" source);
  let wrong = group Group.Primitive "first_face" source in
  expect_error "wrong selection owner"
    (Point_split.run ~selection:(Transform_ops.Selected_points wrong) source);
  ...
  expect_error "native edge selection"
    (Point_split.run ~selection:(Transform_ops.Selected_edges edge) source);
  let bad = Geometry.with_attribute
      (attribute Attribute.Vertex "bad"
        (Attribute.Float [|0.;0.;0.;nan;0.;0.|])) source |> Result.get_ok in
  expect_error "non-finite seam value" (Point_split.run ~attributes:"bad" bad);
  ...
  expect_error "ambiguous promoted owner"
    (Point_split.run ~attributes:"duplicate" ~promote_attributes:true duplicate);
  let cancel = Cancel.create () in
  Cancel.cancel cancel;
  (match Point_split.run ~cancel source with
   | Error error when Error.code error = "cancelled" -> ()
   | Error error -> fail ("Point Split cancellation code: " ^ Error.to_string error)
   | Ok _ -> fail "Point Split ignored cancellation")

let large_fixture () =
  let geometry = Plane_generators.grid ~connectivity:Plane_generators.Grid_quads ~columns:160 ~rows:120
      ~size:10. () |> get in
...
let test_parallel_exact () =
  ...
  let cook domains = Parallel.run ~domains (fun () ->
    Point_split.run ~grain:257 ~attributes:"material seam_region"
      ~promote_attributes:true source |> get) in
