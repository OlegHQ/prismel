...
let run () =
  ...
  let transport = Edge_transport_ops.run_checked ~attribute:"value" source |> get_pdk in
  check (close_array (values "value" transport)
      [|10.;10.;10.;10.;10.;20.;20.|]) "Edge Transport hold";
  let zero = Edge_transport_ops.run_checked ~attribute:"value"
      ~root_value:Edge_transport_ops.Transport_root_zero source |> get_pdk in
  check (values "value" zero = Array.make 7 0.) "Edge Transport zero root";
  let from_root = Edge_transport_ops.run_checked ~attribute:"value"
      ~operation:Edge_transport_ops.Transport_from_root source |> get_pdk in
  check (values "value" from_root = values "value" transport)
    "Edge Transport from root";
  let from_zero = Edge_transport_ops.run_checked ~attribute:"value"
      ~operation:Edge_transport_ops.Transport_from_root
      ~root_value:Edge_transport_ops.Transport_root_zero source |> get_pdk in
  check (values "value" from_zero = Array.make 7 0.)
    "Edge Transport from-root zero policy";
  let total = Edge_transport_ops.run_checked ~attribute:"value"
      ~operation:Edge_transport_ops.Transport_total source |> get_pdk in
  check (close_array (values "value" total) [|0.;10.;10.;12.;12.;0.;20.|])
    "Edge Transport total";
  let depth = Edge_transport_ops.run_checked ~attribute:"depth"
      ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true source |> get_pdk in
  check (close_array (values "depth" depth) [|0.;1.;1.;2.;2.;0.;1.|])
    "Edge Transport constant depth";
  let distance = Edge_transport_ops.run_checked ~attribute:"distance"
  ...
  let maximum = Edge_transport_ops.run_checked ~attribute:"value"
      ~operation:Edge_transport_ops.Transport_maximum source |> get_pdk in
  check (close_array (values "value" maximum) [|10.;10.;10.;10.;10.;20.;20.|])
    "Edge Transport maximum";
  let minimum = Edge_transport_ops.run_checked ~attribute:"value"
      ~operation:Edge_transport_ops.Transport_minimum source |> get_pdk in
  check (close_array (values "value" minimum) [|10.;2.;5.;2.;1.;20.;3.|])
    "Edge Transport minimum";
  let split = Edge_transport_ops.run_checked ~attribute:"value"
      ~split:Edge_transport_ops.Transport_split source |> get_pdk in
  check (close_array (values "value" split)
      [|10.;5.;5.;2.5;2.5;20.;20.|]) "Edge Transport split";
  let backward merge = Edge_transport_ops.run_checked ~attribute:"value"
      ~direction:Edge_transport_ops.Transport_backward ~merge source |> get_pdk
      |> values "value" in
  check (backward Edge_transport_ops.Transport_merge_add = [|13.;8.;5.;7.;1.;3.;3.|])
    "Edge Transport backward add";
  check (backward Edge_transport_ops.Transport_merge_maximum = [|7.;7.;5.;7.;1.;3.;3.|])
    "Edge Transport backward maximum merge";
  check (backward Edge_transport_ops.Transport_merge_minimum = [|1.;1.;5.;7.;1.;3.;3.|])
    "Edge Transport backward minimum merge";
  let backward_zero = Edge_transport_ops.run_checked ~attribute:"value"
      ~direction:Edge_transport_ops.Transport_backward ~root_value:Edge_transport_ops.Transport_root_zero
      source |> get_pdk in
  check (values "value" backward_zero = Array.make 7 0.)
    "Edge Transport backward zero leaves";
  let backward_total = Edge_transport_ops.run_checked ~attribute:"depth"
      ~direction:Edge_transport_ops.Transport_backward ~operation:Edge_transport_ops.Transport_total
      ~integrate_constant:true ~merge:Edge_transport_ops.Transport_merge_add source |> get_pdk in
  check (values "depth" backward_total = [|4.;2.;0.;0.;0.;1.;0.|])
    "Edge Transport backward total";
  let backward_distance = Edge_transport_ops.run_checked ~attribute:"distance"
      ~direction:Edge_transport_ops.Transport_backward ~operation:Edge_transport_ops.Transport_total
      ~integrate_constant:true ~scale_by_edge_length:true
      ~merge:Edge_transport_ops.Transport_merge_add source |> get_pdk in
  check (close_array (values "distance" backward_distance)
      [|7.;4.;0.;0.;0.;4.;0.|]) "Edge Transport backward distance";
  let backward_max = Edge_transport_ops.run_checked ~attribute:"value"
      ~direction:Edge_transport_ops.Transport_backward ~operation:Edge_transport_ops.Transport_maximum
      ~merge:Edge_transport_ops.Transport_merge_maximum source |> get_pdk in
  let backward_min = Edge_transport_ops.run_checked ~attribute:"value"
      ~direction:Edge_transport_ops.Transport_backward ~operation:Edge_transport_ops.Transport_minimum
      ~merge:Edge_transport_ops.Transport_merge_minimum source |> get_pdk in
  check (values "value" backward_max = [|10.;7.;5.;7.;1.;20.;3.|]
      && values "value" backward_min = [|1.;1.;5.;7.;1.;3.;3.|])
    "Edge Transport backward extrema";
  let normalized = Edge_transport_ops.run_checked ~attribute:"depth"
      ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true
      ~normalization:Edge_transport_ops.Transport_normalize_global source |> get_pdk in
  check (close_array (values "depth" normalized)
      [|0.;0.5;0.5;1.;1.;0.;0.5|]) "Edge Transport global normalization";
  let normalized = Edge_transport_ops.run_checked ~attribute:"depth"
      ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true
      ~normalization:Edge_transport_ops.Transport_normalize_components source |> get_pdk in
  check (close_array (values "depth" normalized)
      [|0.;0.5;0.5;1.;1.;0.;1.|]) "Edge Transport component normalization";

  let roots = group Group.Point "roots" 7 (fun point -> point = 2) in
  let rooted = Edge_transport_ops.run_checked ~attribute:"value"
      ~roots:(Edge_transport_ops.Transport_root_group roots) source |> get_pdk in
  ...
  let unrooted = Edge_transport_ops.run_checked ~attribute:"value"
      ~roots:(Edge_transport_ops.Transport_root_group no_roots) source |> get_pdk in
  ...
  let restricted = Edge_transport_ops.run_checked ~attribute:"value" ~points:selected source
      |> get_pdk in
  check (close_array (values "value" restricted)
      [|10.;10.;5.;10.;1.;20.;3.|]) "Edge Transport point restriction";
  let last = Edge_transport_ops.run_checked ~attribute:"value"
      ~roots:Edge_transport_ops.Transport_last_point source |> get_pdk in
  ...
  let missing = Edge_transport_ops.run_checked ~attribute:"missing" source |> get_pdk in
  check (missing == source) "Edge Transport missing attribute was not identity";

  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_checked ~attribute:"P" source);
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_checked ~attribute:"value" ~integrate_constant:true source);
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_checked ~attribute:"value" ~scale_by_edge_length:true source);
  let wrong = group Group.Primitive "wrong" 5 (Fun.const true) in
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_checked ~attribute:"value" ~points:wrong source);
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_checked ~attribute:"value"
       ~roots:(Edge_transport_ops.Transport_root_group wrong) source);
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_checked ~grain:0 ~attribute:"value" source);
  ...
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_checked ~attribute:"integer" integer_source);
  let non_finite = geometry_owned [0.,0.,0.;nan,0.,0.] [|0,1|]
      (Some [|1.;2.|]) in
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_checked ~attribute:"value" non_finite);
  ...
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_checked ~attribute:"value" ~operation:Edge_transport_ops.Transport_total
       ~scale_by_edge_length:true overflow_path);
  let cancelled = Cancel.create () in Cancel.cancel cancelled;
  expect_code "cancelled"
    (Edge_transport_ops.run_checked ~cancel:cancelled ~attribute:"value" source);

  let large = long_curve 100_000 in
  let run domains = Parallel.run ~domains (fun () ->
      Edge_transport_ops.run_checked ~grain:127 ~attribute:"distance"
        ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true
  ...
  let run_backward domains = Parallel.run ~domains (fun () ->
      Edge_transport_ops.run_checked ~grain:127 ~attribute:"depth"
        ~direction:Edge_transport_ops.Transport_backward ~operation:Edge_transport_ops.Transport_total
        ~integrate_constant:true ~merge:Edge_transport_ops.Transport_merge_add backward_scale
        |> get_pdk) in
  ...
  let curve_transport = Edge_transport_ops.run_curves_checked ~attribute:"value" curves
      |> get_pdk in
  check (close_array (values "value" curve_transport)
      [|10.;10.;10.;20.;20.;20.|]) "Edge Transport Each Curve direction";
  let curve_zero = Edge_transport_ops.run_curves_checked ~attribute:"value"
      ~operation:Edge_transport_ops.Transport_from_root ~root_value:Edge_transport_ops.Transport_root_zero
      curves |> get_pdk in
  check (values "value" curve_zero = Array.make 6 0.)
    "Edge Transport Each Curve root zero";
  let curve_distance = Edge_transport_ops.run_curves_checked ~attribute:"distance"
      ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true
  ...
  let curve_reverse = Edge_transport_ops.run_curves_checked ~attribute:"distance"
      ~direction:Edge_transport_ops.Transport_backward ~operation:Edge_transport_ops.Transport_total
  ...
  let curve_maximum = Edge_transport_ops.run_curves_checked ~attribute:"value"
      ~operation:Edge_transport_ops.Transport_maximum curves |> get_pdk in
  let curve_minimum = Edge_transport_ops.run_curves_checked ~attribute:"value"
      ~operation:Edge_transport_ops.Transport_minimum curves |> get_pdk in
  ...
  let curve_components = Edge_transport_ops.run_curves_checked ~attribute:"distance"
      ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true
      ~scale_by_edge_length:true
      ~normalization:Edge_transport_ops.Transport_normalize_components curves |> get_pdk in
  check (close_array (values "distance" curve_components)
      [|0.;1. /. 3.;1.;0.;0.4;1.|])
    "Edge Transport Each Curve component normalization";
  let curve_global = Edge_transport_ops.run_curves_checked ~attribute:"distance"
      ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true
      ~scale_by_edge_length:true
      ~normalization:Edge_transport_ops.Transport_normalize_global curves |> get_pdk in
  check (close_array (values "distance" curve_global)
      [|0.;0.2;0.6;0.;0.4;1.|])
    "Edge Transport Each Curve global normalization";
  let vertex_transport = Edge_transport_ops.run_curves_checked ~owner:Attribute.Vertex
  ...
  let restricted_curve = Edge_transport_ops.run_curves_checked ~primitives:first_curve
  ...
  let closed_forward = Edge_transport_ops.run_curves_checked ~attribute:"depth"
      ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true closed |> get_pdk in
  let closed_backward = Edge_transport_ops.run_curves_checked ~attribute:"depth"
      ~direction:Edge_transport_ops.Transport_backward ~operation:Edge_transport_ops.Transport_total
  ...
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_curves_checked ~attribute:"value" source);
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_curves_checked ~owner:Attribute.Primitive ~attribute:"value"
       curves);
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_curves_checked ~grain:0 ~attribute:"value" curves);
  let integer_attribute = Attribute.create_owned ~owner:Attribute.Point
  ...
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_curves_checked ~attribute:"integer" integer_curves);
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_curves_checked ~primitives:wrong ~attribute:"value" curves);
  let non_finite_curve = curve_geometry [0.,0.,0.;nan,0.,0.] [|0;1|]
      [|0;2|] [|Topology.Open_polyline|] [] in
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_curves_checked ~attribute:"distance"
       ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true
       ~scale_by_edge_length:true non_finite_curve);
  let cancelled = Cancel.create () in Cancel.cancel cancelled;
  expect_code "cancelled"
    (Edge_transport_ops.run_curves_checked ~cancel:cancelled ~attribute:"value" curves);
  let large_curves = many_curves 10_000 10 in
  let run_curves domains = Parallel.run ~domains (fun () ->
      Edge_transport_ops.run_curves_checked ~grain:127 ~attribute:"distance"
        ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true
  ...
  let parent_transport = Edge_transport_ops.run_parent_checked ~attribute:"value"
      parent_source |> get_pdk in
  check (values "value" parent_transport
      = [|10.;10.;10.;10.;10.;20.;20.|])
    "Edge Transport Parent forward transport";
  let parent_zero = Edge_transport_ops.run_parent_checked ~attribute:"value"
      ~operation:Edge_transport_ops.Transport_from_root
      ~root_value:Edge_transport_ops.Transport_root_zero parent_source |> get_pdk in
  check (values "value" parent_zero = Array.make 7 0.)
    "Edge Transport Parent from-root zero";
  let parent_total = Edge_transport_ops.run_parent_checked ~attribute:"value"
      ~operation:Edge_transport_ops.Transport_total parent_source |> get_pdk in
  check (values "value" parent_total = [|0.;10.;10.;12.;12.;0.;20.|])
    "Edge Transport Parent total";
  let parent_distance = Edge_transport_ops.run_parent_checked ~attribute:"distance"
      ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true
  ...
  let parent_maximum = Edge_transport_ops.run_parent_checked ~attribute:"value"
      ~operation:Edge_transport_ops.Transport_maximum parent_source |> get_pdk in
  let parent_minimum = Edge_transport_ops.run_parent_checked ~attribute:"value"
      ~operation:Edge_transport_ops.Transport_minimum parent_source |> get_pdk in
  check (values "value" parent_maximum
      = [|10.;10.;10.;10.;10.;20.;20.|]
      && values "value" parent_minimum = [|10.;2.;5.;2.;1.;20.;3.|])
    "Edge Transport Parent forward extrema";
  let parent_split = Edge_transport_ops.run_parent_checked ~attribute:"value"
      ~split:Edge_transport_ops.Transport_split parent_source |> get_pdk in
  check (close_array (values "value" parent_split)
      [|10.;5.;5.;2.5;2.5;20.;20.|]) "Edge Transport Parent split";
  let parent_components = Edge_transport_ops.run_parent_checked ~attribute:"depth"
      ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true
      ~normalization:Edge_transport_ops.Transport_normalize_components parent_source |> get_pdk in
  check (close_array (values "depth" parent_components)
      [|0.;0.5;0.5;1.;1.;0.;1.|])
    "Edge Transport Parent component normalization";

  let backward merge = Edge_transport_ops.run_parent_checked ~attribute:"value"
      ~direction:Edge_transport_ops.Transport_backward ~merge parent_source |> get_pdk
      |> values "value" in
  check (backward Edge_transport_ops.Transport_merge_add = [|13.;8.;5.;7.;1.;3.;3.|])
  ...
  let backward_total = Edge_transport_ops.run_parent_checked ~attribute:"depth"
      ~direction:Edge_transport_ops.Transport_backward ~operation:Edge_transport_ops.Transport_total
      ~integrate_constant:true ~merge:Edge_transport_ops.Transport_merge_add parent_source
      |> get_pdk in
  check (values "depth" backward_total = [|4.;2.;0.;0.;0.;1.;0.|])
    "Edge Transport Parent backward total";
  let backward_distance = Edge_transport_ops.run_parent_checked ~attribute:"distance"
      ~direction:Edge_transport_ops.Transport_backward ~operation:Edge_transport_ops.Transport_total
      ~integrate_constant:true ~scale_by_edge_length:true
      ~merge:Edge_transport_ops.Transport_merge_add parent_source |> get_pdk in
  check (close_array (values "distance" backward_distance)
      [|7.;4.;0.;0.;0.;4.;0.|])
    "Edge Transport Parent backward distance";
  let backward_max = Edge_transport_ops.run_parent_checked ~attribute:"value"
      ~direction:Edge_transport_ops.Transport_backward ~operation:Edge_transport_ops.Transport_maximum
      ~merge:Edge_transport_ops.Transport_merge_maximum parent_source |> get_pdk in
  let backward_min = Edge_transport_ops.run_parent_checked ~attribute:"value"
      ~direction:Edge_transport_ops.Transport_backward ~operation:Edge_transport_ops.Transport_minimum
      ~merge:Edge_transport_ops.Transport_merge_minimum parent_source |> get_pdk in
  check (values "value" backward_max = [|10.;7.;5.;7.;1.;20.;3.|]
      && values "value" backward_min = [|1.;1.;5.;7.;1.;3.;3.|])
  ...
  let cycle_output = Edge_transport_ops.run_parent_checked ~attribute:"value" cycle_source
      |> get_pdk in
  check (values "value" cycle_output = [|10.;2.;5.;7.;1.;20.;20.|])
    "Edge Transport Parent did not preserve unreachable cycle";
  let restricted_parent = Edge_transport_ops.run_parent_checked ~points:selected
  ...
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_parent_checked ~attribute:"value" missing_parent);
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_parent_checked ~parent_attribute:"value" ~attribute:"value"
       parent_source);
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_parent_checked ~points:wrong ~attribute:"value" parent_source);
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_parent_checked ~grain:0 ~attribute:"value" parent_source);
  expect_code "invalid_edge_transport"
    (Edge_transport_ops.run_parent_checked ~parent_attribute:" " ~attribute:"value"
       parent_source);
  let cancelled = Cancel.create () in Cancel.cancel cancelled;
  expect_code "cancelled"
    (Edge_transport_ops.run_parent_checked ~cancel:cancelled ~attribute:"value"
       parent_source);
  ...
  let run_parent domains = Parallel.run ~domains (fun () ->
      Edge_transport_ops.run_parent_checked ~grain:127 ~attribute:"distance"
        ~operation:Edge_transport_ops.Transport_total ~integrate_constant:true
