...
let source ?cancel geometry =
  ...
  if not (Topology.all_triangles topology) then
    Error "input must contain only triangle polygons"
  else if List.exists (fun attribute ->
      match Attribute.owner attribute with
      | Attribute.Point | Attribute.Detail -> false
      | Attribute.Vertex | Attribute.Primitive -> true)
      (Geometry.attributes geometry)
      || List.exists (fun group -> Group.owner group <> Group.Point)
        (Geometry.groups geometry)
      || Geometry.edge_groups geometry <> [] then
    Error "repair requires point/detail attributes and point groups only"
  else
    ...
    Ok { geometry; vertices; faces }

let rebuild_array (mesh : raw) faces = Ok { mesh with faces }

let output ?cancel raw =
  Cancel.check_opt cancel;
  let point_count = Geometry.point_count raw.geometry in
  ...
  Array.iteri (fun face (a, b, c) ->
    ...
    vertex_points.(offset + 2) <- c) raw.faces;
  let primitive_offsets = Array.init (Array.length raw.faces + 1)
      (fun face -> face * 3) in
  let attributes = Geometry.attributes raw.geometry |> List.filter (fun attribute ->
    not (Attribute.owner attribute = Attribute.Point
         && Attribute.name attribute = "N")
    && (Attribute.owner attribute = Attribute.Point
       || Attribute.owner attribute = Attribute.Detail)) in
  let groups = Geometry.groups raw.geometry in
  Result.bind (Topology.polygons_owned ~point_count ~vertex_points
    ~primitive_offsets) (fun topology ->
    Result.bind (Geometry.create ~positions:(Geometry.positions raw.geometry)
      ~topology ~attributes ~groups ()) (Normal_ops.run ?cancel))
...
let analyze_raw ?(epsilon = 1e-12) (mesh : raw) =
  ...
  and faces = Array.copy mesh.faces in
...
let repair_t_junctions_raw ?(epsilon = 1e-8) (mesh : raw) =
  ...
  in
  let input = Array.copy mesh.faces in
...
let orient_consistently_raw (mesh : raw) =
  let faces = Array.copy mesh.faces in
...
let analyze ?cancel ?(epsilon = 1e-12) geometry =
  if not (Float.is_finite epsilon) || epsilon < 0. then
    Error (Error.make ~operation:"repair.analyze" ~code:"invalid_epsilon"
      "epsilon must be finite and non-negative")
  else Error.guard ~operation:"repair.analyze" ~code:"invalid_geometry"
    (fun () -> Result.map (analyze_raw ~epsilon) (source ?cancel geometry))

let repair_t_junctions ?cancel ?(epsilon = 1e-8) geometry =
  if not (Float.is_finite epsilon) || epsilon <= 0. then
    Error (Error.make ~operation:"repair.t_junctions" ~code:"invalid_epsilon"
      "epsilon must be finite and positive")
  else Error.guard ~operation:"repair.t_junctions" ~code:"invalid_geometry"
    (fun () -> Result.bind (source ?cancel geometry) (fun input ->
      Result.bind (repair_t_junctions_raw ~epsilon input) (output ?cancel)))

let orient_consistently ?cancel geometry =
  Error.guard ~operation:"repair.orient_consistently" ~code:"invalid_geometry"
    (fun () -> Result.bind (source ?cancel geometry) (fun input ->
      Result.bind (orient_consistently_raw input) (output ?cancel)))
