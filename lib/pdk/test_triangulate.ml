...
let test_identity_and_errors () =
  ...
  (match Triangulate.run ~primitives:(group Group.Primitive "curve" source) source with
   | Error error -> check (Error.code error = "invalid_topology")
  ...
  (match Triangulate.run ~primitives:wrong_owner source with
   | Error error -> check (Error.code error = "invalid_topology")
...
let test_already_triangular_identity () =
  ...
  let triangulated = Triangulate.run ~primitives:selected source |> get_pdk in
  let selected_triangles = group Group.Primitive "selected" triangulated in
  let unchanged = Triangulate.run ~primitives:selected_triangles triangulated
      |> get_pdk in
...
let test_collapsed_quad_and_deterministic_failure () =
  ...
  let run domains = Parallel.run ~domains (fun () ->
    match Triangulate.run ~grain:1 degenerate with
