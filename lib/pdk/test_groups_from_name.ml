...
let test_point_names_and_policies () =
  ...
  let ignored = Group_ops.groups_from_name_checked ~owner:Attribute.Point ~attribute:"name"
      source |> get_ok in
  ...
  let forced = Group_ops.groups_from_name_checked ~invalid_names:Group_ops.Force_valid
  ...
  let prefixed = Group_ops.groups_from_name_checked ~prefix:"piece_"
      ~owner:Attribute.Point ~attribute:"name" source |> get_ok in
...
let test_primitive_names () =
  let source = mesh () |> with_text Attribute.Primitive "material"
      [|"metal"; "wood"|] in
  let output = Group_ops.groups_from_name_checked ~owner:Attribute.Primitive
...
let test_conflicts_and_empty_output () =
  ...
  let replaced = Group_ops.groups_from_name_checked ~conflict:Group_ops.Name_replace
      ~owner:Attribute.Point ~attribute:"name" source |> get_ok in
  ...
  let unioned = Group_ops.groups_from_name_checked ~conflict:Group_ops.Name_union
      ~owner:Attribute.Point ~attribute:"name" source |> get_ok in
  ...
  let empty = point_cloud 3 |> with_text Attribute.Point "name"
      [|""; ""; ""|] in
  let unchanged = Group_ops.groups_from_name_checked ~owner:Attribute.Point
...
let test_bounds_and_failures () =
  let source = point_cloud 8 |> with_text Attribute.Point "name"
      [|"a"; "b"; "a"; "b"; "a"; "b"; "a"; "b"|] in
  expect_invalid (fun () -> Group_ops.groups_from_name_checked ~max_groups:1
      ~owner:Attribute.Point ~attribute:"name" source)
    "Groups from Name group-count bound";
  expect_invalid (fun () -> Group_ops.groups_from_name_checked ~max_payload_bytes:1
      ~owner:Attribute.Point ~attribute:"name" source)
    "Groups from Name packed-payload bound";
  expect_invalid (fun () -> Group_ops.groups_from_name_checked ~max_groups:(-1)
      ~owner:Attribute.Point ~attribute:"name" source)
    "Groups from Name rejects a negative group bound";
  expect_invalid (fun () -> Group_ops.groups_from_name_checked ~grain:0
      ~owner:Attribute.Point ~attribute:"name" source)
    "Groups from Name rejects a non-positive grain";
  expect_invalid (fun () -> Group_ops.groups_from_name_checked ~owner:Attribute.Vertex
      ~attribute:"name" source)
    "Groups from Name rejects vertex ownership";
  expect_invalid (fun () -> Group_ops.groups_from_name_checked ~owner:Attribute.Detail
      ~attribute:"name" source)
    "Groups from Name rejects detail ownership";
  expect_invalid (fun () -> Group_ops.groups_from_name_checked ~owner:Attribute.Point
  ...
  (match Group_ops.groups_from_name_checked ~cancel:cancelled ~owner:Attribute.Point
...
let test_name_from_groups () =
  ...
  let last = Group_ops.name_from_groups_checked ~default:"none" ~owner:Attribute.Point source
      |> get_ok in
  check (text_values Attribute.Point "name" last
      = [|"alpha"; "beta"; "beta"; "none"; "none"; "none"|])
    "Name from Groups stable last-group overlap";
  let first = Group_ops.name_from_groups_checked ~overlap:Group_ops.First_group
  ...
  expect_invalid (fun () -> Group_ops.name_from_groups_checked ~overlap:Group_ops.Error_on_overlap
  ...
  let filtered = Group_ops.name_from_groups_checked ~attribute:"piece" ~pattern:"alpha"
  ...
  let primitive = Group_ops.name_from_groups_checked ~owner:Attribute.Primitive
  ...
  let vertex = Group_ops.name_from_groups_checked ~owner:Attribute.Vertex vertex_source
      |> get_ok in
  check (text_values Attribute.Vertex "name" vertex
      = [|"corner"; ""; "corner"; ""; "corner"; ""|])
    "Name from Groups vertex ownership";
  expect_invalid (fun () -> Group_ops.name_from_groups_checked ~owner:Attribute.Detail source)
    "Name from Groups rejects detail ownership";
  let cancelled = Cancel.create () in
  Cancel.cancel cancelled;
  (match Group_ops.name_from_groups_checked ~cancel:cancelled ~owner:Attribute.Point source with
   | Error error -> check (Error.code error = "cancelled")
...
let test_scale_and_parallel_exactness () =
  ...
  let run domains = Parallel.run ~domains (fun () ->
    Group_ops.groups_from_name_checked ~grain:1_009 ~owner:Attribute.Point
  ...
  let round_trip geometry = Group_ops.name_from_groups_checked ~grain:1_009
...
let test_checked_boundaries () =
  let source = point_cloud 3 |> with_text Attribute.Point "name"
      [|"red"; "blue"; "red"|] in
  ...
  check (List.for_all2 same_group (Geometry.groups direct)
    ...
    "Groups from Name checked boundary changed ordered groups";
  let compare_errors operation direct compatibility =
    match direct, compatibility with
    | Error direct, Error compatibility ->
        check (Error.operation direct = operation
            && Error.to_string direct = Error.to_string compatibility)
          (operation ^ " checked boundary changed the typed error")
    | _ -> fail (operation ^ " checked boundary accepted invalid input") in
