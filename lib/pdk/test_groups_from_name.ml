...
let test_point_names_and_policies () =
  ...
  let ignored = Group_ops.groups_from_name ~owner:Attribute.Point ~attribute:"name"
      source |> get_ok in
  ...
  let forced = Group_ops.groups_from_name ~invalid_names:Group_ops.Force_valid
  ...
  let prefixed = Group_ops.groups_from_name ~prefix:"piece_"
      ~owner:Attribute.Point ~attribute:"name" source |> get_ok in
...
let test_conflicts_and_empty_output () =
  ...
  let replaced = Group_ops.groups_from_name ~conflict:Group_ops.Name_replace
      ~owner:Attribute.Point ~attribute:"name" source |> get_ok in
  ...
  let unioned = Group_ops.groups_from_name ~conflict:Group_ops.Name_union
      ~owner:Attribute.Point ~attribute:"name" source |> get_ok in
  ...
  let empty = point_cloud 3 |> with_text Attribute.Point "name"
      [|""; ""; ""|] in
  let unchanged = Group_ops.groups_from_name ~owner:Attribute.Point
...
let test_bounds_and_failures () =
  let source = point_cloud 8 |> with_text Attribute.Point "name"
      [|"a"; "b"; "a"; "b"; "a"; "b"; "a"; "b"|] in
  expect_invalid (fun () -> Group_ops.groups_from_name ~max_groups:1
      ~owner:Attribute.Point ~attribute:"name" source)
    "Groups from Name group-count bound";
  expect_invalid (fun () -> Group_ops.groups_from_name ~max_payload_bytes:1
      ~owner:Attribute.Point ~attribute:"name" source)
    "Groups from Name packed-payload bound";
  expect_invalid (fun () -> Group_ops.groups_from_name ~max_groups:(-1)
      ~owner:Attribute.Point ~attribute:"name" source)
    "Groups from Name rejects a negative group bound";
  expect_invalid (fun () -> Group_ops.groups_from_name ~grain:0
      ~owner:Attribute.Point ~attribute:"name" source)
    "Groups from Name rejects a non-positive grain";
  expect_invalid (fun () -> Group_ops.groups_from_name ~owner:Attribute.Vertex
      ~attribute:"name" source)
    "Groups from Name rejects vertex ownership";
  expect_invalid (fun () -> Group_ops.groups_from_name ~owner:Attribute.Detail
      ~attribute:"name" source)
    "Groups from Name rejects detail ownership";
  expect_invalid (fun () -> Group_ops.groups_from_name ~owner:Attribute.Point
  ...
  (match Group_ops.groups_from_name ~cancel:cancelled ~owner:Attribute.Point
...
let test_scale_and_parallel_exactness () =
  ...
  let run domains = Parallel.run ~domains (fun () ->
    Group_ops.groups_from_name ~grain:1_009 ~owner:Attribute.Point
  ...
  let round_trip geometry = Group_ops.name_from_groups ~grain:1_009
...
let test_checked_boundaries () =
  let source = point_cloud 3 |> with_text Attribute.Point "name"
      [|"red"; "blue"; "red"|] in
  let direct = Group_ops.groups_from_name ~owner:Attribute.Point
      ~attribute:"name" source |> get_ok
  and compatibility = Group_ops.groups_from_name ~owner:Attribute.Point
      ~attribute:"name" source |> get_ok in
  ...
  compare_errors "groups_from_name"
    (Group_ops.groups_from_name ~grain:0 ~owner:Attribute.Point
       ~attribute:"name" source)
    (Group_ops.groups_from_name ~grain:0 ~owner:Attribute.Point
       ~attribute:"name" source);
  let direct = Group_ops.name_from_groups ~owner:Attribute.Point direct
      |> get_ok
  and compatibility = Group_ops.name_from_groups ~owner:Attribute.Point compatibility
      |> get_ok in
  ...
  compare_errors "name_from_groups"
    (Group_ops.name_from_groups ~grain:0 ~owner:Attribute.Point direct)
    (Group_ops.name_from_groups ~grain:0 ~owner:Attribute.Point compatibility);
  let cancel = Cancel.create () in
  Cancel.cancel cancel;
  compare_errors "groups_from_name"
    (Group_ops.groups_from_name ~cancel ~owner:Attribute.Point
       ~attribute:"name" source)
    (Group_ops.groups_from_name ~cancel ~owner:Attribute.Point
       ~attribute:"name" source);
  compare_errors "name_from_groups"
    (Group_ops.name_from_groups ~cancel ~owner:Attribute.Point direct)
    (Group_ops.name_from_groups ~cancel ~owner:Attribute.Point compatibility)
