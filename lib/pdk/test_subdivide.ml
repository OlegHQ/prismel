...
let source () =
  ...
  |> Group_mesh.group_edges ~name:"marked_edges" |> get_pdk
...
let test_second_input_creases () =
  ...
  let triangle = Geometry.create ~positions:triangle_positions
      ...
      |> Group_mesh.group_edges ~name:"loop_source_edges" |> get_pdk in
...
let test_subdivision_holes () =
  ...
  let run domains = Parallel.run ~domains (fun () ->
    let input = make_grid () |> add_hole |> Group_mesh.group_edges ~name:"source_edges"
        |> get_pdk in
  ...
  let all_creased = Geometry.with_attribute
      ...
      |> get_string |> Group_mesh.group_edges ~name:"all_source_edges" |> get_pdk in
