...
let test_complement_and_combine () =
  ...
  let combined = Ops.group_combine ~grain:1 ~owner:Group_ops.Group_points
  ...
  let all_but = Ops.group_combine ~grain:1 ~owner:Group_ops.Group_points
  ...
  let unmatched = Ops.group_combine ~owner:Group_ops.Group_points ~name:"empty"
  ...
  let edge_combined = Ops.group_combine ~grain:1 ~owner:Group_ops.Group_edges
...
let test_range () =
  ...
  let filtered = Ops.group_range ~grain:1 ~owner:Group_ops.Group_points ~name:"filtered"
  ...
  let partition = Ops.group_range ~grain:1 ~owner:Group_ops.Group_points
  ...
  let unioned = base
      |> Ops.group_range ~grain:1 ~owner:Group_ops.Group_points ~name:"merged"
  ...
  let empty_extreme = Ops.group_range ~owner:Group_ops.Group_points ~name:"extreme"
  ...
  let vertices = Ops.group_range ~grain:1 ~owner:Group_ops.Group_vertices
  ...
  let edges = Ops.group_range ~grain:1 ~owner:Group_ops.Group_edges ~name:"edge_range"
...
let test_rename_invert_delete () =
  ...
  let sequential = Ops.group_rename ~rules:[
      ...
      { rename_owner = Some Group_ops.Group_points; rename_pattern = "part_*";
        rename_replacement = "final_*"; rename_conflict = Group_ops.Rename_error }]
      base |> get_ok in
  ...
  let inverted = Ops.group_invert ~owner:Group_ops.Group_points ~pattern:"piece_*"
  ...
  let edge_inverted = Ops.group_invert ~owner:Group_ops.Group_edges
...
let test_copy () =
  ...
  let skipped = Ops.group_copy ~grain:1 ~rules:rule ~conflict:Group_ops.Copy_skip
...
let test_parallel_exactness_and_cancellation () =
  ...
  let run domains = Parallel.run ~domains (fun () ->
    let ranged = Ops.group_range ~grain:257 ~owner:Group_ops.Group_points
        ~name:"bands" ~filter:{ select = 5; of_ = 13; offset = 3 }
        (Group_ops.Range_from_ends { start = 17; end_offset = 23 }) source |> get_ok in
    let combined = Ops.group_combine ~grain:257 ~owner:Group_ops.Group_points
  ...
  (match Ops.group_range ~cancel:cancelled ~owner:Group_ops.Group_points ~name:"x"
...
let test_transfer () =
  ...
  let no_op = Ops.group_transfer ~rules:[transfer_rule Group_ops.Group_points
  ...
  (match Ops.group_transfer ~rules:[transfer_rule Group_ops.Group_vertices "*" ""]
      ~source ~target () with
   | Error error -> check (Error.code error = "invalid_group")
