...
let compare_facet_keys key_a key_b key_c left right =
  let comparison = Int.compare key_a.(left) key_a.(right) in
  if comparison <> 0 then comparison else
  let comparison = Int.compare key_b.(left) key_b.(right) in
  if comparison <> 0 then comparison else
  let comparison = Int.compare key_c.(left) key_c.(right) in
  if comparison <> 0 then comparison else Int.compare left right
...
let build ?cancel constraints refinement =
  try
    ...
    and right_faces = Boolean_constraints.right_triangle_count constraints in
    let raw_count = ref 0 in
    let add_faces count get =
      for face = 0 to count - 1 do
        let count = match get refinement face with
          | None -> 1 | Some value -> Boolean_face_cdt.triangle_count value in
    ...
    let raw_points = Array.make corner_count fallback
    ...
    and raw = ref 0 in
    let append side face point_index source_point orientation =
      let facet = !raw in
      ...
      for local = 0 to 2 do
        raw_points.((facet * 3) + local) <- source_point local
      done;
      incr raw in
    let append_side side face_count get triangle_point =
      for face = 0 to face_count - 1 do
        ...
        | None ->
            append side face (-1)
              (fun local -> Implicit_point.explicit source
                (triangle_point constraints face local) |> Result.get_ok) 1
        | Some value ->
            ...
            for triangle = 0 to Boolean_face_cdt.triangle_count value - 1 do
              append side face triangle
                (fun local -> Boolean_face_cdt.Private.point value
    ...
    let occurrence_order = Array.init corner_count Fun.id in
    Array.sort (compare_points raw_points) occurrence_order;
    let vertex_of_occurrence = Array.make corner_count 0
    and unique_count = ref 0 and previous = ref (-1) in
    Array.iter (fun occurrence ->
      if !previous < 0 || not (Implicit_point.equal raw_points.(occurrence)
          raw_points.(!previous)) then begin
        previous := occurrence;
        incr unique_count
      end;
      vertex_of_occurrence.(occurrence) <- !unique_count - 1) occurrence_order;
    let vertices = if !unique_count = 0 then [||] else begin
      let output = Array.make !unique_count raw_points.(occurrence_order.(0))
      and output_index = ref (-1) and previous = ref (-1) in
      Array.iter (fun occurrence ->
        if !previous < 0 || not (Implicit_point.equal raw_points.(occurrence)
            raw_points.(!previous)) then begin
          incr output_index; previous := occurrence;
          output.(!output_index) <- raw_points.(occurrence)
        end) occurrence_order;
      output
    end in
    ...
    for facet = 0 to !raw_count - 1 do
      let a = vertex_of_occurrence.(facet * 3)
      and b = vertex_of_occurrence.((facet * 3) + 1)
      and c = vertex_of_occurrence.((facet * 3) + 2) in
      if a = b || b = c || c = a then
    ...
    Array.sort (compare_facet_keys key_a key_b key_c) facet_order;
    ...
    Array.sort (fun left right ->
      let comparison = Int.compare entry_first.(left) entry_first.(right) in
      if comparison <> 0 then comparison else
      let comparison = Int.compare entry_second.(left) entry_second.(right) in
      if comparison <> 0 then comparison else
      let comparison = Int.compare entry_facet.(left) entry_facet.(right) in
      if comparison <> 0 then comparison else Int.compare left right) edge_order;
