...
let remap_attribute ?cancel ?grain mapping attribute =
  Topology_remap.attribute ?cancel ~grain:(Option.value ~default:16_384 grain)
    mapping attribute
      ...
      let positions = if not points_changed then Geometry.positions geometry else
        ...
        Packed.Float3.Private.of_owned_exn
          ~x:(Topology_remap.select_float ?cancel
            ~grain:(Option.value ~default:16_384 grain) point_map positions.x)
          ~y:(Topology_remap.select_float ?cancel
            ~grain:(Option.value ~default:16_384 grain) point_map positions.y)
          ~z:(Topology_remap.select_float ?cancel
            ~grain:(Option.value ~default:16_384 grain) point_map positions.z) in
  ...
  try
    ...
    if !selected_count = 0 then Ok geometry else begin
      ...
      let positions = Packed.Float3.Private.of_owned_exn
          ~x:(Topology_remap.select_float ?cancel
            ~grain:(Option.value ~default:16_384 grain) point_map source_positions.x)
          ~y:(Topology_remap.select_float ?cancel
            ~grain:(Option.value ~default:16_384 grain) point_map source_positions.y)
          ~z:(Topology_remap.select_float ?cancel
            ~grain:(Option.value ~default:16_384 grain) point_map source_positions.z) in
