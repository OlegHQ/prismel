...
let test_random () =
  ...
  let run seed domains = Prismel.Parallel.run ~domains (fun () ->
      Ordering.sort_checked ~grain:3 ~owner:Ordering.Points ~key:(Ordering.Random seed) source
      |> get_ok) in
  ...
  (match Ordering.sort_checked ~selection:selected ~owner:Ordering.Points ~key:(Ordering.Random 42L)
      source with
...
let test_indirect () =
  ...
  let ranked = Ordering.sort_checked ~grain:1 ~owner:Ordering.Points ~key:Ordering.X
  ...
  let reordered = Ordering.sort_checked ~grain:1 ~owner:Ordering.Points
  ...
  let secondary = Ordering.sort_checked ~grain:1 ~owner:Ordering.Points
      ~key:(Ordering.Attribute_component { name = "secondary"; component = 0 })
      ~output_indices:"rank" combined_source |> get_ok in
  let combined = Ordering.sort_checked ~grain:1 ~owner:Ordering.Points
...
let test_restricted_and_errors () =
  ...
  let reordered = Ordering.sort_checked ~grain:1 ~selection:selected ~owner:Ordering.Points
  ...
  let expect_invalid geometry key message =
    match Ordering.sort_checked ~owner:Ordering.Points ~key geometry with
  ...
  (match Ordering.sort_checked ~owner:Ordering.Points ~key:Ordering.X ~combine_indices:true source with
   | Error error -> check (Error.code error = "invalid_sort")
       "combine Sort Indices requires output"
   | Ok _ -> fail "combine Sort Indices accepted no output");
  (match Ordering.sort_checked ~owner:Ordering.Points ~key:Ordering.X ~output_indices:"P" source with
   | Error error -> check (Error.code error = "invalid_sort")
...
let test_primitive_indirect () =
  ...
  let reordered = Ordering.sort_checked ~grain:1 ~owner:Ordering.Primitives
  ...
  let ranked = Ordering.sort_checked ~grain:1 ~owner:Ordering.Primitives ~key:(Ordering.Random 77L)
      ~output_indices:"rank" source |> get_ok in
...
let test_topology_and_spatial_keys () =
  ...
  let vertex_order = Ordering.sort_checked ~grain:1 ~owner:Ordering.Points
  ...
  let primitive_index = Ordering.sort_checked ~grain:1 ~owner:Ordering.Points
  ...
  let locality = Line_geometry.points
      ...
      |> with_int "id" [|0; 1; 2; 3|]
      |> Ordering.sort_checked ~grain:1 ~owner:Ordering.Points ~key:Ordering.Spatial_locality
      |> get_ok in
  ...
  let extreme = Line_geometry.points
      ...
      |> Ordering.sort_checked ~grain:1 ~owner:Ordering.Points ~key:Ordering.Spatial_locality in
  (match extreme with
   | Ok _ -> ()
   | Error error -> fail ("extreme spatial Sort failed: " ^ Error.to_string error));
  (match Line_geometry.points [|Float.nan, 0., 0.|]
      |> Ordering.sort_checked ~owner:Ordering.Points ~key:Ordering.Spatial_locality with
   | Error error -> check (Error.code error = "invalid_sort")
       "non-finite spatial Sort diagnostic"
   | Ok _ -> fail "spatial Sort accepted non-finite position");
  (match Ordering.sort_checked ~owner:Ordering.Primitives ~key:Ordering.By_vertex_order geometry with
   | Error error -> check (Error.code error = "invalid_sort")
...
let test_dense_exactness () =
  ...
  let run domains = Prismel.Parallel.run ~domains (fun () ->
      let random = Ordering.sort_checked ~grain:4096 ~owner:Ordering.Points
          ~key:(Ordering.Random 918273L) source |> get_ok in
      Ordering.sort_checked ~grain:4096 ~owner:Ordering.Points ~key:Ordering.X
  ...
  let locality domains = Prismel.Parallel.run ~domains (fun () ->
      Ordering.sort_checked ~grain:4096 ~owner:Ordering.Points ~key:Ordering.Spatial_locality source
      |> get_ok) in
