...
let create ?cancel ?(grain = 16_384) features =
  ...
  if total = 0 then { features; order = [||]; min_x = [||]; min_y = [||];
    ...
    let bounds = Bounds3_index.create ?cancel ~grain ~centroid_x ~centroid_y
        ~centroid_z ~item_min_x:fmin_x ~item_min_y:fmin_y
        ~item_min_z:fmin_z ~item_max_x:fmax_x ~item_max_y:fmax_y
        ~item_max_z:fmax_z () in
    { features; order = bounds.order; min_x = bounds.min_x; min_y = bounds.min_y;
