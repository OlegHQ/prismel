...
let test_point_components () =
  ...
  let ranged = Group_ops.range ~grain:1 ~owner:Group_ops.Group_points
  ...
  let filtered = Group_ops.range ~grain:1 ~owner:Group_ops.Group_points
  ...
  let inverted = Group_ops.range ~grain:1 ~owner:Group_ops.Group_points
  ...
  let partitioned = Group_ops.range ~grain:1 ~owner:Group_ops.Group_points
  ...
  let orphans = Line_geometry.points [|(0., 0., 0.); (1., 0., 0.); (2., 0., 0.)|]
      |> Group_ops.range ~grain:1 ~owner:Group_ops.Group_points ~name:"each_first"
  ...
  let empty = Line_geometry.points [||]
      |> Group_ops.range ~grain:1 ~owner:Group_ops.Group_points ~name:"empty"
           ~connectivity:(disconnected ())
           (Group_ops.Range_start_end { start = 0; end_ = 0 }) |> get_ok in
...
let test_region_and_primitive_modes () =
  let source = fixture () in
  let first_faces = Group_ops.range ~grain:1 ~owner:Group_ops.Group_primitives
  ...
  let second_region = Group_ops.range ~grain:1 ~owner:Group_ops.Group_primitives
  ...
  let absent_region = Group_ops.range ~grain:1 ~owner:Group_ops.Group_points
...
let test_attribute_components () =
  ...
  let points = Group_ops.range ~grain:1 ~owner:Group_ops.Group_points
  ...
  let loose = Group_ops.range ~grain:1 ~owner:Group_ops.Group_points
  ...
  let strict = Group_ops.range ~grain:1 ~owner:Group_ops.Group_points
  ...
  let combined = Group_ops.range ~grain:1 ~owner:Group_ops.Group_points
      ~name:"combined"
      ~connectivity:(connected ~attributes:"piece" ~collision:{
        ...
        keep_boundary = true } ())
      (Group_ops.Range_start_end { start = 0; end_ = 0 }) combined_source |> get_ok in
  ...
  let primitives = Group_ops.range ~grain:1 ~owner:Group_ops.Group_primitives
...
let test_collision_and_region_policy () =
  ...
  let split = Group_ops.range ~grain:1 ~owner:Group_ops.Group_primitives
  ...
  let check_ordinary_collision source owner pattern message =
    let collision = {
      Group_ops.collision_owner = owner;
      collision_pattern = pattern;
      keep_boundary = true;
    } in
    let result = Group_ops.range ~grain:1 ~owner:Group_ops.Group_primitives
  ...
  check_ordinary_collision
    ...
    Group_ops.Group_points "point_cut"
  ...
  check_ordinary_collision
    ...
    Group_ops.Group_vertices "vertex_cut"
  ...
  let omitted = Group_ops.range ~grain:1 ~owner:Group_ops.Group_primitives
      ~name:"omitted"
      ~connectivity:(connected
        ~collision:{ collision with keep_boundary = false } ())
      (Group_ops.Range_start_end { start = 0; end_ = max_int }) two_faces |> get_ok in
  expect_members [] (group Group.Primitive "omitted" omitted)
    "Group Range did not disregard collision-boundary elements";
  let source = fixture () in
  let preserve = Group_ops.range ~grain:1 ~owner:Group_ops.Group_primitives
      ~name:"preserve"
  ...
  let masked = Group_ops.range ~grain:1 ~base:"mask"
  ...
  let remove = Group_ops.range ~grain:1 ~owner:Group_ops.Group_primitives
      ~name:"remove"
...
let test_validation_and_cancellation () =
  let source = fixture () in
  (match Group_ops.range ~owner:Group_ops.Group_vertices ~name:"bad"
      ~connectivity:(disconnected ())
  ...
  (match Group_ops.range ~owner:Group_ops.Group_points ~name:"bad"
  ...
  (match Group_ops.range ~cancel:cancelled ~owner:Group_ops.Group_points ~name:"bad"
      ~connectivity:(disconnected ())
      (Group_ops.Range_start_end { start = 0; end_ = 1 }) source with
...
let test_parallel_exactness_and_scale () =
  ...
  let run domains = Parallel.run ~domains (fun () ->
    Group_ops.range ~grain:1_009 ~owner:Group_ops.Group_points ~name:"first"
      ~connectivity:(disconnected ())
      (Group_ops.Range_start_end { start = 0; end_ = 0 }) source |> get_ok) in
  ...
  let run_attribute domains = Parallel.run ~domains (fun () ->
    Group_ops.range ~grain:1_009 ~owner:Group_ops.Group_points ~name:"stripe_first"
...
let test_multiple_ranges () =
  ...
  let result = Group_ops.ranges ~grain:1 ~rules source |> get_ok in
  ...
  let identity = Group_ops.ranges ~rules:[disabled] source |> get_ok in
  ...
  let invalid = Group_ops.range_rule ~connectivity:(disconnected ())
      ...
      (Group_ops.Range_start_end { start = 0; end_ = 0 }) in
  (match Group_ops.ranges ~rules:[List.hd rules; invalid] source with
   | Error error -> check (Error.code error = "invalid_group")
  ...
  let cancelled = Cancel.create () in
  Cancel.cancel cancelled;
  (match Group_ops.ranges ~cancel:cancelled ~rules source with
   | Error error -> check (Error.code error = "cancelled")
