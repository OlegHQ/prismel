...
let test_reverse () =
  let run domains = Parallel.run ~domains (fun () ->
    Reverse_faces.run ~grain:1 (fixture ()) |> get_pdk) in
...
let test_local_and_shift () =
  ...
  let local = Reverse_faces.run ~grain:1 ~primitives:selected source |> get_pdk in
  ...
  let shift domains offset = Parallel.run ~domains (fun () ->
    let source = fixture () in
    Reverse_faces.run ~grain:1 ~primitives:(group Group.Primitive "selected" source)
      ~operation:(Reverse_faces.Shift_vertices offset) source |> get_pdk) in
  ...
  let identity = Reverse_faces.run ~grain:1 ~primitives:selected
      ~operation:(Reverse_faces.Shift_vertices 4) source |> get_pdk in
  ...
  let empty_output = Reverse_faces.run ~grain:1 ~primitives:empty source |> get_pdk in
...
let test_errors_and_cancellation () =
  ...
  (match Reverse_faces.run ~primitives:wrong_owner source with
