...
let check_length_modes () =
  let line = Line_geometry.polyline [|(0., 0., 0.); (2.5, 0., 0.)|] |> get_ok in
  let even = Resample_curves.run ~maximum_segment_length:1. line |> get_ok in
  ...
  let short_last = Resample_curves.run ~maximum_segment_length:1.
      ~even_last_segment:false line |> get_ok in
  let x = x_positions short_last in
  check (x = [|0.; 1.; 2.; 2.5|]) "short final segment sampling";
  let capped = Resample_curves.run ~segments:2 ~maximum_segment_length:1.
      ~even_last_segment:false line |> get_ok in
  ...
  let square = Line_geometry.polyline ~closed:true
      ...
      |> get_ok in
  let closed = Resample_curves.run ~maximum_segment_length:10.
...
let check_diagnostics () =
  let bent = Line_geometry.polyline [|(0., 0., 0.); (1., 0., 0.); (1., 3., 0.)|]
      |> get_ok in
  let result = Resample_curves.run ~segments:4 ~curve_u_attribute:"curveu"
...
let check_group_and_overrides () =
  let first = Line_geometry.polyline [|(0., 0., 0.); (2., 0., 0.)|] |> get_ok
  and second = Line_geometry.polyline
  ...
  let restricted = Resample_curves.run ~primitives:selected ~segments:4 source
      |> get_ok in
  ...
  let run domains = Parallel.run ~domains (fun () ->
      Resample_curves.run ~grain:1 ~segments:7 ~maximum_segment_length:0.2
...
let check_validation () =
  ...
  expect_invalid (Resample_curves.run line);
  expect_invalid (Resample_curves.run ~segments:0 line);
  expect_invalid (Resample_curves.run ~maximum_segment_length:0. line);
  expect_invalid (Resample_curves.run ~maximum_segment_length:Float.nan line);
  expect_invalid (Resample_curves.run ~segments:max_int line);
  expect_invalid (Resample_curves.run ~segments:2 ~curve_u_attribute:" " line);
  expect_invalid (Resample_curves.run ~segments:2 ~curve_u_attribute:"P" line);
  expect_invalid (Resample_curves.run ~segments:2 ~curve_u_attribute:"u"
      ~distance_attribute:"u" line);
  expect_invalid (Resample_curves.run ~segments:2
  ...
  expect_invalid (Resample_curves.run ~segments:2 closed);
  let cancelled = Cancel.create () in
  Cancel.cancel cancelled;
  (match Resample_curves.run ~cancel:cancelled ~segments:2 line with
