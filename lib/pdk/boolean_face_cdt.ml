...
let build ?cancel ?(point_location = Walk) ?(constraint_recovery = Trace)
  ...
  try
    ...
    for local = 0 to 2 do
      points.(local) <- Implicit_point.explicit source
          (face_triangle_point constraints side triangle local) |> Result.get_ok
    done;
    ...
    for constraint_index = 0 to constraint_count - 1 do
      ...
      while not (edge_exists key) do
        ...
        if !attempts > attempt_limit then
          invalid_arg "constraint recovery did not converge";
        ...
        if not flipped then
          invalid_arg "constraint recovery found no flippable crossing edge";
        incr attempts
      done;
