type side = Boolean_constraints.side = Left | Right
...
let parallel_sort_indices ?cancel compare values =
  ...
  if count < run_size * 2 then Array.sort compare values
    ...
    Prismel_math.Parallel.for_ ~chunk_size:1 ~start:0 ~finish:(runs - 1) (fun run ->
    ...
    while !width < count do
      ...
      Prismel_math.Parallel.for_ ~chunk_size:1 ~start:0 ~finish:(merges - 1) (fun merge ->
        Cancel.check_opt cancel;
