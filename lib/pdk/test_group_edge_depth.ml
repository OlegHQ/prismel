...
let test_depth_and_disconnected_components () =
  ...
  let zero = Group_ops.group_edge_depth ~depth:0 ~point_group:"seed" ~name:"zero"
      source |> get_ok in
  expect_members [4; 10; 12] (point_group "zero" zero)
    "Group Edge Depth includes only seeds at depth zero";
  let two = Group_ops.group_edge_depth ~depth:2 ~point_group:"seed" ~name:"two"
      source |> get_ok in
  expect_members [2; 3; 4; 5; 6; 9; 10; 11; 12] (point_group "two" two)
    "Group Edge Depth bounded multi-source distance";
  let flooded = Group_ops.group_edge_depth ~depth:max_int ~point_group:"seed"
...
let test_merge_algebra () =
  ...
  let intersection = Group_ops.group_edge_depth ~merge:Group_ops.Group_intersection
  ...
  let union = Group_ops.group_edge_depth ~merge:Group_ops.Group_union ~depth:1
  ...
  let subtract = Group_ops.group_edge_depth ~merge:Group_ops.Group_subtract ~depth:1
      ~point_group:"seed" ~name:"target" source |> get_ok in
  expect_members [0] (point_group "target" subtract)
    "Group Edge Depth subtraction";
  let absent_intersection = Group_ops.group_edge_depth
  ...
  let absent_subtract = Group_ops.group_edge_depth ~merge:Group_ops.Group_subtract
  ...
  let in_place = Group_ops.group_edge_depth ~depth:1 ~point_group:"seed" ~name:"seed"
      source |> get_ok in
...
let test_failures_and_cancellation () =
  let source = sample () in
  expect_invalid (fun () -> Group_ops.group_edge_depth ~depth:(-1)
  ...
  (match Group_ops.group_edge_depth ~cancel:cancelled ~depth:1
...
let test_parallel_exactness_and_scale () =
  ...
  let run domains = Parallel.run ~domains (fun () ->
    Group_ops.group_edge_depth ~grain:1_009 ~depth:64 ~point_group:"center"
