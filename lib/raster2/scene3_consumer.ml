...
type prepared={geometry:Scene3.prepared;texture:Triangle.texture option;cull:Triangle.cull;blend:Composite.blend;depth_stencil:Depth_stencil.state}
...
let render ~target ~clear ~clear_depth ~clear_stencil ~draws=
 ...
 Array.iter(fun draw->match Scene3_lighting.prepare_with_shadows draw.lighting draw.shadows with Error e->fail(Lighting_error e)|Ok lighting->
  ...
  match Scene3.prepare~matrix:draw.matrix~viewport:draw.viewport~scissor:draw.scissor~topology:draw.topology~vertices~indices:draw.indices with Error e->fail(Geometry_error e)|Ok geometry->prepared:={geometry;texture=draw.texture;cull=draw.cull;blend=draw.blend;depth_stencil=draw.depth_stencil}::!prepared)draws;
 ...
 List.iter(fun draw->Array.iter(fun(a,b,c)->Triangle.draw~color~depth:(Option.map snd depth)~depth_state:draw.depth_stencil~blend:draw.blend~cull:draw.cull~clip:draw.geometry.clip~texture:draw.texture a b c)draw.geometry.triangles)(List.rev !prepared);
