...
type texture={texture:Texture.t;filter:Texture.filter;address_u:Texture.address;address_v:Texture.address}
...
module Private : sig
  val draw_solid_xy : color:Surface.t -> blend:Composite.blend -> cull:cull ->
    clip:clip -> packed:int -> float -> float -> float -> float -> float -> float -> unit
end
