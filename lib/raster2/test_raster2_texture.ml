...
let ()=List.iter(fun frame->let expected=render frame in let workers=Array.init 4(fun _->Domain.spawn(fun()->render frame))in Array.iter(fun worker->assert(Domain.join worker=expected))workers)[1;600];
  ...
  let borrowed_levels=Array.mapi(fun i(w,h)->let level=ok(Surface.create~width:w~height:h())in Surface.clear level colors.(i);level)dimensions in
  let borrowed=ok(Texture.Private.create_levels_borrowed~color_space:Linear borrowed_levels)in
  if ok(Texture.sample borrowed~address_u:Clamp~address_v:Clamp~filter:Nearest
      ~u:0.5~v:0.5~lod:0.)<>colors.(0)then failwith"borrowed mip identity";
  Surface.clear borrowed_levels.(0) 0x123456ffl;
  if ok(Texture.sample borrowed~address_u:Clamp~address_v:Clamp~filter:Nearest
      ~u:0.5~v:0.5~lod:0.)<>0x123456ffl then failwith"borrowed mip was copied";
  List.iter(fun filter->List.iter(fun address_u->List.iter(fun address_v->List.iter(fun(u,v,lod)->let checked=ok(Texture.sample explicit~address_u~address_v~filter~u~v~lod)and direct=Texture.Private.sample_int_unchecked explicit~address_u~address_v~filter(Float.Array.of_list[u;v;lod;0.;0.;0.])in if checked<>Int32.of_int direct then failwith"private sampler semantic drift")[-2.,-1.,0.;-1.000001,-0.999999,0.;-1.25,-0.75,0.;-0.000001,0.,0.;0.2,0.8,1.;2.4,1.7,1.5]) [Clamp;Repeat;Mirror]) [Clamp;Repeat;Mirror]) [Nearest;Bilinear;Trilinear];
  ...
  let maxima=Array.make_matrix 2 3 0. and clamp_maxima=Array.make_matrix 2 3 0.
  and coordinates=Float.Array.of_list[0.37;0.61;1.25;0.;0.;0.]in
  List.iteri(fun space texture->List.iteri(fun filter_index filter->List.iter(fun address_u->List.iter(fun address_v->
    ...
    let per_sample=(Gc.allocated_bytes()-.before)/.100_000. in
    maxima.(space).(filter_index)<-max maxima.(space).(filter_index)per_sample;
    if address_u=Clamp&&address_v=Clamp then clamp_maxima.(space).(filter_index)<-per_sample) [Clamp;Repeat;Mirror]) [Clamp;Repeat;Mirror]) [Nearest;Bilinear;Trilinear]) [explicit;srgb_sample];
  Printf.printf"Raster2 private sampler allocation N/B/T: Linear %.2f/%.2f/%.2f, sRGB %.2f/%.2f/%.2f bytes/sample\n"maxima.(0).(0)maxima.(0).(1)maxima.(0).(2)maxima.(1).(0)maxima.(1).(1)maxima.(1).(2);
  Printf.printf"Raster2 private Clamp sampler N/B/T: Linear %.2f/%.2f/%.2f, sRGB %.2f/%.2f/%.2f\n"clamp_maxima.(0).(0)clamp_maxima.(0).(1)clamp_maxima.(0).(2)clamp_maxima.(1).(0)clamp_maxima.(1).(1)clamp_maxima.(1).(2);
  if Array.exists(fun value->value>32.1)maxima.(0)||maxima.(1).(0)>32.1||maxima.(1).(1)>176.1||maxima.(1).(2)>368.1 then failwith"private sampler allocation regression";
