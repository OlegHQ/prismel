...
let blend mode s d=match mode with Copy|Replace->s|Source_over|Alpha->over s d
|Add|Multiply|Screen|Subtract->
  let sa=ch s 0 and da=ch d 0 in
  let alpha=sa*255+da*(255-sa)in
  if alpha=0 then 0l else
  let f n=let source=ch s n and destination=ch d n in
    let mixed=match mode with
      |Add->clamp(source+destination)
      |Multiply->(source*destination+127)/255
      |Screen->255-(((255-source)*(255-destination)+127)/255)
      |Subtract->max 0(destination-source)
      |_->assert false in
    let premultiplied=source*sa*(255-da)+destination*da*(255-sa)+mixed*sa*da in
    clamp((premultiplied+alpha/2)/alpha)in
  rgba(f 24)(f 16)(f 8)((alpha+127)/255)
