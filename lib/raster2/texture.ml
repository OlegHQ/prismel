...
let[@inline always] linear_of_srgb x=let x=float x/.255. in if x<=0.04045 then x/.12.92 else((x+.0.055)/.1.055)**2.4
let[@inline always] srgb_of_linear x=let x=max 0.(min 1. x)in let x=if x<=0.0031308 then 12.92*.x else 1.055*.(x**(1./.2.4))-.0.055 in int_of_float(x*.255.+.0.5)
...
let[@inline always] address mode x=match mode with Clamp->max 0.(min 1. x)|Repeat->x-.floor x|Mirror->let base=floor x and fraction=x-.floor x in if int_of_float(abs_float base)land 1=0 then fraction else 1.-.fraction
