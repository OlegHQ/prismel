...
let create_common driver configuration before_device_destroy families_to_make variants_to_make supplied=match Ogpu.Backend.create_device driver with Error _ as e->e|Ok device->
  ...
  get_cleanup(match Ogpu.Backend.create_queue device with Error _ as e->e|Ok queue->
    ...
    get_cleanup(match Ogpu.Backend.create_texture device(texture_descriptor configuration),variants[]requested with
      |Ok target,Ok pipelines->Ok{device;queue;surface;target;pipelines;cache=[];auxiliary_cache=[];texture_cache=[];uploaded=0L;dead=false;before_device_destroy}
...
let prepare_auxiliary value(source:auxiliary_resource)=
  ...
  |None->
    ...
    List.iter(fun old->ignore(Ogpu.Backend.destroy_buffer old.auxiliary_buffer))replaced;
    let cache=item::others in let keep,evict=List.mapi(fun i x->i,x)cache|>List.partition(fun(i,_)->i<64)in
    List.iter(fun(_,old)->ignore(Ogpu.Backend.destroy_buffer old.auxiliary_buffer))evict;
...
let render_resources ?(clear=(0.,0.,0.,0.)) value draws=if value.dead then error"Scene_execution.render"Ogpu.Error.Stale_handle"renderer is destroyed"else
  ...
  let supported=List.for_all(fun(family,blend,_,_,_)->List.exists(fun variant->variant.family=family&&variant.blend=blend)value.pipelines)draws in
  ...
  let rec prepare_all acc=function []->Ok(List.rev acc)|(family,blend,texture,auxiliary,(draw:draw))::rest->match prepare value draw.mesh with Error _ as e->e|Ok mesh->match texture with Some source->(match prepare_texture value source with Error _ as e->e|Ok texture->prepare_aux family blend auxiliary draw mesh (Some(source,texture)) acc rest)|None->prepare_aux family blend auxiliary draw mesh None acc rest
  and prepare_aux family blend auxiliary draw mesh texture acc rest=match auxiliary with None->prepare_all((family,blend,draw.state,texture,None,mesh)::acc)rest|Some source->match prepare_auxiliary value source with Error _ as e->e|Ok buffer->match prepare_texture value source.texture with Error _ as e->e|Ok texture2->prepare_all((family,blend,draw.state,texture,Some(source,buffer,texture2),mesh)::acc)rest in
  match prepare_all[]draws with Error _ as e->e|Ok prepared->match Ogpu.Backend.acquire value.surface with Error _ as e->e|Ok(`Timeout|`Occluded)->Ok false|Ok`Device_lost->error"Scene_execution.render"Device_lost"device lost"|Ok(`Acquired frame)->
    let resources=`Texture value.target::List.concat_map(fun(_,_,_,texture,auxiliary,item)->`Buffer item.buffer::(match texture with None->[]|Some(_,cached)->[`Texture cached.texture])@(match auxiliary with None->[]|Some(_,buffer,texture)->[`Buffer buffer.auxiliary_buffer;`Texture texture.texture]))prepared in
...
let destroy value=if value.dead then Ok()else(value.dead<-true;List.iter(fun item->ignore(Ogpu.Backend.destroy_buffer item.buffer))value.cache;value.cache<-[];List.iter(fun item->ignore(Ogpu.Backend.destroy_buffer item.auxiliary_buffer))value.auxiliary_cache;value.auxiliary_cache<-[];List.iter(fun item->ignore(Ogpu.Backend.destroy_texture item.texture))value.texture_cache;value.texture_cache<-[];ignore(Ogpu.Backend.destroy_texture value.target);List.iter(fun variant->ignore(Ogpu.Backend.destroy_pipeline variant.pipeline))value.pipelines;ignore(Ogpu.Backend.destroy_surface value.surface);ignore(Ogpu.Backend.destroy_queue value.queue);match value.before_device_destroy()with Error _ as e->e|Ok()->Ogpu.Backend.destroy_device value.device)
