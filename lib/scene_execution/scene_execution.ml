...
let pipeline device family blend samples=let capabilities=Ogpu.Backend.capabilities device in let open Result in
  ...
  bind(Ogpu.Pipeline.create_render~blend capabilities{backend="mock";label=Some"scene-execution";layout;vertex;vertex_entry="scene_vertex";fragment=Some fragment;fragment_entry=Some"scene_fragment";color_format=Rgba8_unorm;depth_format=No_depth;sample_count=samples})(fun portable->
...
let resize value configuration=
  match Ogpu.Backend.create_texture value.device(texture_descriptor configuration)with Error _ as e->e|Ok target->
  let rec create made=function []->Ok(List.rev made)|(samples,_)::rest->match Ogpu.Backend.create_texture value.device(multisample_descriptor configuration samples)with Error e->List.iter(fun(_,texture)->ignore(Ogpu.Backend.destroy_texture texture))made;Error e|Ok texture->create((samples,texture)::made)rest in
  match create[]value.multisample_targets with Error e->ignore(Ogpu.Backend.destroy_texture target);Error e|Ok multisample_targets->
  match Ogpu.Backend.configure value.surface configuration with Error e->ignore(Ogpu.Backend.destroy_texture target);List.iter(fun(_,texture)->ignore(Ogpu.Backend.destroy_texture texture))multisample_targets;Error e|Ok()->let old=value.target and old_multisample=value.multisample_targets in value.target<-target;value.multisample_targets<-multisample_targets;List.iter(fun(_,texture)->ignore(Ogpu.Backend.destroy_texture texture))old_multisample;Ogpu.Backend.destroy_texture old
...
let destroy value=if value.dead then Ok()else(value.dead<-true;List.iter(fun item->ignore(Ogpu.Backend.destroy_buffer item.buffer))value.cache;value.cache<-[];List.iter(fun item->ignore(Ogpu.Backend.destroy_buffer item.auxiliary_buffer))value.auxiliary_cache;value.auxiliary_cache<-[];List.iter(fun item->ignore(Ogpu.Backend.destroy_texture item.texture))value.texture_cache;value.texture_cache<-[];List.iter(fun(_,texture)->ignore(Ogpu.Backend.destroy_texture texture))value.multisample_targets;value.multisample_targets<-[];ignore(Ogpu.Backend.destroy_texture value.target);List.iter(fun variant->ignore(Ogpu.Backend.destroy_pipeline variant.pipeline))value.pipelines;ignore(Ogpu.Backend.destroy_surface value.surface);ignore(Ogpu.Backend.destroy_queue value.queue);match value.before_device_destroy()with Error _ as e->e|Ok()->Ogpu.Backend.destroy_device value.device)
