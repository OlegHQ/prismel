...
type state={viewport:int*int*int*int;scissor:int*int*int*int;cull:Ogpu.Render_pass.cull;depth_compare:Ogpu.Render_pass.comparison;depth_write:bool;depth_load:Ogpu.Render_pass.load;depth_clear:float;transform_uniforms:bytes option}
...
type t={device:Ogpu.Backend.device;queue:Ogpu.Backend.queue;surface:Ogpu.Backend.surface;mutable target:Ogpu.Backend.texture;mutable multisample_targets:(int*Ogpu.Backend.texture)list;mutable depth_targets:(int*Ogpu.Backend.texture)list;pipelines:pipeline_variant list;mutable cache:cached list;mutable prepared_cache:prepared_run list;mutable auxiliary_cache:cached_auxiliary list;mutable texture_cache:cached_texture list;mutable uploaded:int64;mutable dead:bool;before_device_destroy:unit->(unit,Ogpu.Error.t)result}
...
let pipeline device family blend samples=let capabilities=Ogpu.Backend.capabilities device in let open Result in
  ...
  let depth_format=match family with Scene2->Ogpu.Pipeline.No_depth|Scene3|Scene3_textured|Scene3_shadow->Depth32_float in
...
let allocate_targets device configuration samples=
  ...
  match depths[]samples with Error e->ignore(Ogpu.Backend.destroy_texture target);destroy_targets multisample_targets;Error e|Ok depth_targets->Ok(target,multisample_targets,depth_targets)
let create_common driver configuration before_device_destroy families_to_make variants_to_make samples_to_make supplied=match Ogpu.Backend.create_device driver with Error _ as e->e|Ok device->
  ...
  match Ogpu.Backend.create_surface device configuration with Error e->ignore(Ogpu.Backend.destroy_queue queue);cleanup();Error e|Ok surface->
    ...
    (match allocate_targets device configuration samples with
      |Ok(target,multisample_targets,depth_targets)->Ok{device;queue;surface;target;multisample_targets;depth_targets;pipelines;cache=[];prepared_cache=[];auxiliary_cache=[];texture_cache=[];uploaded=0L;dead=false;before_device_destroy}
...
let pass value family samples state load clear=
  ...
  Ogpu.Render_pass.create~raster_state:{cull=state.cull;depth_compare=state.depth_compare;depth_write=state.depth_write}(Ogpu.Backend.device_handle value.device){colors=[|Some{texture;resolve;load;store=(if samples=1 then Store else Resolve);clear}|];depth;stencil=None;viewport={x;y;width;height};scissor={x=sx;y=sy;width=sw;height=sh}}
...
let resize value configuration=
  ...
  match allocate_targets value.device configuration samples with Error _ as e->e|Ok(target,multisample_targets,depth_targets)->
  match Ogpu.Backend.configure value.surface configuration with Error e->ignore(Ogpu.Backend.destroy_texture target);destroy_targets multisample_targets;destroy_targets depth_targets;Error e|Ok()->let old=value.target and old_multisample=value.multisample_targets and old_depth=value.depth_targets in value.target<-target;value.multisample_targets<-multisample_targets;value.depth_targets<-depth_targets;destroy_targets old_multisample;destroy_targets old_depth;Ogpu.Backend.destroy_texture old
