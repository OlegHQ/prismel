...
let render_family ?(clear=(0.,0.,0.,0.)) value draws=if value.dead then error"Scene_execution.render"Ogpu.Error.Stale_handle"renderer is destroyed"else
  let rec prepare_all acc=function []->Ok(List.rev acc)|(family,blend,(draw:draw))::rest->match prepare value draw.mesh with Error _ as e->e|Ok mesh->prepare_all((family,blend,draw.state,mesh)::acc)rest in
