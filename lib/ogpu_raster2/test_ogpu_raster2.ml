...
let ()=
  ...
  let before_atomic=get(Ogpu.Backend.read_buffer upload~offset:0L~length:16)in
  let atomic=Ogpu.Backend.Transfer[|
    Ogpu.Transfer_pass.Fill_buffer(Ogpu.Backend.buffer_id upload,0L,4L,0xff);
    Fill_buffer(Ogpu.Backend.buffer_id upload,511L,4L,0xee)|]in
  (match Ogpu.Backend.submit queue atomic~resources:[`Buffer upload]~pipelines:[]with
   |Error _->()|Ok _->failwith"invalid transfer batch submitted");
  if get(Ogpu.Backend.read_buffer upload~offset:0L~length:16)<>before_atomic then
    failwith"invalid transfer batch mutated earlier operation";
