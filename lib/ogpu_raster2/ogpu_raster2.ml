    ...
    let create_queue () =
      ...
      let submit command ~resources ~pipelines =
        ...
        let render submission =
          ...
          let missing_sampler=List.exists(fun(d:Ogpu.Render_pass.draw)->List.exists(fun(t:Ogpu.Render_pass.texture_binding)->t.stage=Ogpu.Command.Fragment&&not(List.exists(fun(s:Ogpu.Render_pass.sampler_binding)->s.stage=t.stage&&s.index=t.index)d.samplers))d.textures)draws in
          if draws=[]||pipelines=[]||missing_sampler||List.exists(fun id->Option.is_none(find id))required then
          ...
          |Some(Texture({levels;depth},_))->let color=levels.(0)in
              ...
              let draw(d:Ogpu.Render_pass.draw)=match List.find_opt(fun(b:Ogpu.Render_pass.buffer_binding)->b.stage=Ogpu.Command.Vertex&&b.index=0)d.buffers with
                ...
                |Some binding->match find binding.buffer_id with
                  |Some(Buffer vertices)->(let sampled=List.find_opt(fun(t:Ogpu.Render_pass.texture_binding)->t.stage=Ogpu.Command.Fragment&&t.index=0)d.textures in let texture=match sampled with None->Ok None|Some binding->match List.find_opt(fun(s:Ogpu.Render_pass.sampler_binding)->s.stage=Ogpu.Command.Fragment&&s.index=binding.index)d.samplers,find binding.texture_id with Some sampler,Some(Texture({levels;_},_))->let first=match sampler.sampler.mip_filter with No_mip->0|Nearest_mip|Linear_mip->min(Array.length levels-1)(int_of_float(floor sampler.sampler.lod_min))in let sampled_levels=Array.sub levels first(Array.length levels-first)in let capacity=Array.fold_left(fun n surface->n+Bytes.length(Raster2.Surface.bytes surface))0 sampled_levels in(match Raster2.Texture.create_levels~color_space:Raster2.Texture.Linear~hard_capacity:capacity sampled_levels with Ok texture->let filter=match sampler.sampler.mip_filter,sampler.sampler.min_filter with Linear_mip,_->Raster2.Texture.Trilinear|_,Linear->Bilinear|_,Nearest->Nearest and address=function Ogpu.Types.Clamp_to_edge->Raster2.Texture.Clamp|Repeat->Repeat|Mirror_repeat->Mirror in Ok(Some{Raster2.Triangle.texture;filter;address_u=address sampler.sampler.address_u;address_v=address sampler.sampler.address_v})|Error _->error"Ogpu_raster2.render"Invalid_argument"fragment texture is invalid")|None,_->error"Ogpu_raster2.render"Invalid_argument"fragment sampler is absent"|_,_->error"Ogpu_raster2.render"Invalid_argument"fragment texture is absent"in match texture with Error _ as e->e|Ok texture->let textured=Option.is_some texture in let indices=match d.index with
