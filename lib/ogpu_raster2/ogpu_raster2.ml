      ...
      let submit command ~resources ~pipelines =
        ...
        let render submission =
          ...
          |Some(Texture({color;depth},_))->
              ...
              let vertex bytes offset index = let base=Int64.to_int offset+index*16 in
                {Raster2.Triangle.x=Int64.float_of_bits(Bytes.get_int64_le bytes base);y=Int64.float_of_bits(Bytes.get_int64_le bytes(base+8));depth=0.;color=0x4080BFFFl;u=0.;v=0.}in
              let draw(d:Ogpu.Render_pass.draw)=match List.find_opt(fun(b:Ogpu.Render_pass.buffer_binding)->b.stage=Ogpu.Command.Vertex&&b.index=0)d.buffers with
                ...
                |Some binding->match find binding.buffer_id with
                  |Some(Buffer vertices)->let indices=match d.index with
                    ...
                    let triangle a b c=Raster2.Triangle.draw~color~depth:(Option.map(fun _->depth)descriptor.depth)~depth_state~blend:Raster2.Composite.Copy~cull:Cull_none~clip~texture:None(vertex vertices binding.offset a)(vertex vertices binding.offset b)(vertex vertices binding.offset c)in
        ...
        let result=match command with Ogpu.Backend.Render submission->render submission|Compute _->error"Ogpu_raster2.compute"Unsupported"software compute is unsupported"|Transfer _->error"Ogpu_raster2.transfer"Unsupported"software transfer is not qualified"in
