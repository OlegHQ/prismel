...
let create ?(capabilities=Capabilities.minimum_m1)()=
  ...
  let create_device()=let device_token=token c and device_handle=Handle.create_device()in Ok{Backend.device_token;device_handle;capabilities;
    ...
    create_surface=(fun _->let id=token c and frame=ref 0L in c.surfaces<-c.surfaces+1;Ok({surface_token=id;configure=(fun _->Ok());acquire=(fun()->if c.lost then Ok`Device_lost else(frame:=Int64.succ !frame;Ok(`Acquired({Backend.frame_token = !frame}:Backend.driver_frame))));present=(fun _->add c"present";Ok());discard=(fun _->add c"discard";Ok());destroy_surface=(fun()->c.surfaces<-c.surfaces-1;Ok())}:Backend.driver_surface));destroy_device=(fun()->Handle.destroy_device device_handle;add c"destroy-device";Ok())}in
