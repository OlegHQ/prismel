...
let create ?(capabilities=Capabilities.minimum_m1)()=
  ...
  let create_device()=let device_token=token c and device_handle=Handle.create_device()in Ok{Backend.device_token;device_handle;capabilities;
    ...
    create_queue=(fun()->let id=token c in c.queues<-c.queues+1;Ok{queue_token=id;submit=(fun command~resources~pipelines->mock_submit c~operation:"Backend_mock.submit"~commit:(fun epoch->Printf.sprintf"submit:%Ld"epoch)command~resources~pipelines);complete_through=(fun epoch->if epoch<=c.completed||epoch>c.epoch then error"Backend_mock.complete"Error.Invalid_argument"epoch is invalid"else(c.completed<-epoch;add c(Printf.sprintf"complete:%Ld"epoch);Ok()));destroy_queue=(fun()->c.queues<-c.queues-1;Ok())});
    create_surface=(fun _->let id=token c and frame=ref 0L in c.surfaces<-c.surfaces+1;Ok({surface_token=id;configure=(fun _->if c.fail_configure then(c.fail_configure<-false;error"Backend_mock.configure"Error.Invalid_state"injected configure failure")else Ok());acquire=(fun()->if c.lost then Ok`Device_lost else(frame:=Int64.succ !frame;Ok(`Acquired({Backend.frame_token = !frame}:Backend.driver_frame))));present=(fun ~queue ~source _->add c(Printf.sprintf"present:%Ld:%Ld"queue source);Ok());submit_present=(fun ~queue ~source command~resources~pipelines frame->mock_submit c~operation:"Backend_mock.submit_present"~commit:(fun epoch->Printf.sprintf"submit-present:%Ld:%Ld:%Ld:%Ld"queue source frame.Backend.frame_token epoch)command~resources~pipelines);discard=(fun _->add c"discard";Ok());destroy_surface=(fun()->c.surfaces<-c.surfaces-1;Ok())}:Backend.driver_surface));destroy_device=(fun()->Handle.destroy_device device_handle;add c"destroy-device";Ok())}in
