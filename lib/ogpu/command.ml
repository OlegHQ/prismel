...
type state=Recording|In_pass of pass|Ended|Presented
...
let push_debug value label=if String.contains label '\000' then Error(Error.make"Ogpu.Command.push_debug"Error.Invalid_argument"debug label contains NUL")else match value.state with Ended|Presented->state_error"Ogpu.Command.push_debug""encoder has ended"|Recording|In_pass _->value.debug<-label::value.debug;append value(Push_debug label);Ok()
let pop_debug value=match value.state,value.debug with (Ended|Presented),_->state_error"Ogpu.Command.pop_debug""encoder has ended"|_,[]->state_error"Ogpu.Command.pop_debug""debug stack is empty"|_,_::rest->value.debug<-rest;append value Pop_debug;Ok()
