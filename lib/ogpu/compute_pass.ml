...
module Private=struct
  let of_description value=describe value
  let map_resource_ids f value={value with commands=Array.map(function Command.Declare_resource r->Command.Declare_resource{r with resource_id=f r.resource_id}|x->x)value.commands}
end
