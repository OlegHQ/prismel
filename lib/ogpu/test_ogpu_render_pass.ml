...
let ()=
  ...
  let draw:Ogpu.Render_pass.draw={pipeline_key="p";buffers=[{stage=Ogpu.Command.Fragment;index=0;buffer_id=3L;offset=0L}];textures=[{stage=Ogpu.Command.Fragment;index=0;texture_id=4L}];samplers=[{stage=Ogpu.Command.Fragment;index=0;sampler}];primitive=Triangle_list;vertex_start=0;vertex_count=3;instance_count=1;index=None}in
  ignore(ok(Ogpu.Render_pass.submit pass[draw]));
  expect Ogpu.Error.Invalid_argument(Ogpu.Render_pass.submit pass
    [{draw with primitive=Line_list;index=None}]);
  ignore(ok(Ogpu.Render_pass.submit pass
    [{draw with primitive=Line_list;index=Some(Uint32,3L,0L,2)}]));
  expect Ogpu.Error.Invalid_argument(Ogpu.Render_pass.submit pass
    [{draw with primitive=Line_list;index=Some(Uint32,3L,0L,3)}]);
  ignore(ok(Ogpu.Render_pass.submit pass
    [{draw with primitive=Point_list;index=Some(Uint32,3L,0L,1)}]));
  expect Ogpu.Error.Invalid_argument(Ogpu.Render_pass.submit pass[{draw with instance_count=0}]);
  expect Ogpu.Error.Invalid_argument(Ogpu.Render_pass.submit pass[{draw with instance_count=2}]);
  ignore(ok(Ogpu.Render_pass.submit pass[{draw with instance_count=2;index=Some(Uint32,3L,0L,3)}]));
  ignore(ok(Ogpu.Render_pass.submit pass[]));
