...
let run () =let d=Ogpu.Handle.create_device()and f=Ogpu.Handle.create_device()in
 ...
 let bad=create d in reject(copy_buffer bad~src~src_offset:(-1L)~dst~dst_offset:0L~length:1L);reject(buffer_to_texture bad~src~offset:0L~bytes_per_row:4L~bytes_per_image:32L~dst:tex~mip:0~origin:o~extent:n);
 let volume=texture~device:d(Ogpu.Handle.create~device:d){(td[Texture_copy_src;Texture_copy_dst])with depth=4;mip_levels=3}in
 let two_slices={width=2;height=2;depth=2}in
 reject(buffer_to_texture bad~src~offset:0L~bytes_per_row:256L~bytes_per_image:512L~dst:volume~mip:2~origin:o~extent:two_slices);
 reject(buffer_to_texture bad~src~offset:3584L~bytes_per_row:256L~bytes_per_image:512L~dst:volume~mip:0~origin:o~extent:two_slices);
 let foreign=buffer~device:f(Ogpu.Handle.create~device:f)(bd[Copy_src])in reject(copy_buffer bad~src:foreign~src_offset:0L~dst~dst_offset:0L~length:1L);Ogpu.Handle.destroy bh;reject(copy_buffer bad~src~src_offset:0L~dst~dst_offset:0L~length:1L);
