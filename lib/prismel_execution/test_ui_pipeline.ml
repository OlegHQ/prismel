...
let capture execution lower =
  ...
  ignore (get (Prismel_execution.Private.step ~clear:(1., 1., 1., 1.)
    submission [ batch ]));
  get (Prismel_execution.capture execution)
  ...
  List.iter (fun density ->
    ...
    | Ok execution ->
        Fun.protect ~finally:(fun () ->
          ignore (Prismel_execution.destroy execution)) (fun () ->
          let resource _ = None in
          let expected = capture execution (fun submission ->
            Prismel_execution.Private.lower_scene2 submission ~density
              ~resource (old_ir ())) in
          let actual = capture execution (fun submission ->
            Prismel_execution.Private.lower_ui submission ~density
