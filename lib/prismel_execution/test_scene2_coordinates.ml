...
let render execution ir =
  let draws = get (Prismel_execution.lower_scene2 execution ~density:1
...
let with_target ~logical_width ~logical_height ~drawable_width ~drawable_height f =
  match Prismel_execution.create_offscreen
  ...
  | Ok execution ->
      Fun.protect
        ~finally:(fun () -> ignore (Prismel_execution.destroy execution))
        (fun () -> f execution)
