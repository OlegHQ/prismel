...
let get_resource=function Ok value->value|Error error->
  failwith(Format.asprintf"%a"Runtime_resources.pp_error error)
...
let run () =
  ...
  |Ok execution->
      try
        ...
        ignore(get(Prismel_execution.step~clear:(1.,0.,0.,1.)execution[]));
        let first=get(Prismel_execution.capture execution)in
        ...
        ignore(get(Prismel_execution.capture_into execution
        ...
        expect_error"offscreen caller-owned capture accepted a short destination"
          (Prismel_execution.capture_into execution
            ~destination:(Bytes.create(Bytes.length first-1)));
        ignore(get(Prismel_execution.resize execution~logical_width:5
          ~logical_height:4~drawable_width:5~drawable_height:4));
        ignore(get(Prismel_execution.step~clear:(0.,0.,1.,1.)execution[]));
        let resized=get(Prismel_execution.capture execution)in
        ...
        let facts=get(Prismel_execution.presentation_facts execution)in
        ...
        let render_retained version draw=
          ...
          let batch=get(Prismel_execution.Private.adopt_draws submission[draw])in
          ignore(get(Prismel_execution.Private.step~identity:"offscreen-retained"
            ~version submission[batch]))in
        let red=scene3_draw"retained-red"1. in
        render_retained 1L red;
        let uploaded=(get(Prismel_execution.stats execution)).uploaded_bytes in
        render_retained 1L(scene3_draw"ignored-same-version"0.);
        require((get(Prismel_execution.stats execution)).uploaded_bytes=uploaded)
        ...
        let lower_segment version=
          let submission=get(Prismel_execution.Private.begin_submission execution)in
          ignore(get(Prismel_execution.Private.lower_scene2_segment submission
            ~identity:77L~version~cacheable:true~density:1~resource:(fun _->None)
            segment_ir));
          Prismel_execution.Private.cancel submission in
        lower_segment 1L;
        lower_segment 1L;
        let entries,hits,misses=
          Prismel_execution.Private.retained_scene2_segment_stats execution in
        ...
        let invalid=get(Prismel_execution.Private.begin_submission execution)in
        expect_error"invalid retained Scene2 identity was accepted"
          (Prismel_execution.Private.lower_scene2_segment invalid
            ~identity:0L~version:1L~cacheable:true~density:1
            ~resource:(fun _->None)segment_ir);
        let incomplete=get(Prismel_execution.Private.begin_submission execution)in
        let incomplete_batch=get(Prismel_execution.Private.adopt_draws incomplete[red])in
        expect_error"retained identity without version was accepted"
          (Prismel_execution.Private.step~identity:"incomplete"incomplete
            [incomplete_batch]);
        let stats=get(Prismel_execution.stats execution)in
        ...
        let replace image=
          ...
          Runtime_resources.Image.replace image~width:1~height:1
            ~rgba:(Bytes.of_string(String.make 1 red^"\x00\x00\xff"))in
        Fun.protect
          ~finally:(fun()->ignore(Runtime_resources.Image.destroy image))
          (fun()->
            ...
            let resolve=function 1->Some(Prismel_execution.Image image)|_->None in
            ignore(get(Prismel_execution.lower_scene2 execution~density:1
              ~resource:resolve image_ir));
            ignore(get_resource(replace image));
            let first=get(Prismel_execution.Private.begin_submission execution)in
            ignore(get(Prismel_execution.Private.lower_scene2 first~density:1
              ~resource:resolve image_ir));
            ignore(get_resource(replace image));
            let failed=get(Prismel_execution.Private.begin_submission execution)in
            ignore(get(Prismel_execution.Private.lower_scene2 failed~density:1
              ~resource:resolve image_ir));
            ignore(get_resource(replace image));
            expect_error"later lowering failure was accepted"
              (Prismel_execution.Private.lower_scene2 failed~density:1
                ~resource:resolve missing_ir);
            let overlap=get(Prismel_execution.Private.begin_submission execution)in
            ignore(get(Prismel_execution.Private.lower_scene2 overlap~density:1
              ~resource:resolve image_ir));
            ignore(get_resource(replace image));
            Prismel_execution.Private.cancel first;
            ignore(get_resource(replace image));
            Prismel_execution.Private.cancel first;
            Prismel_execution.Private.cancel overlap;
            let step_failed=get(Prismel_execution.Private.begin_submission execution)in
            let step_draws=get(Prismel_execution.Private.lower_scene2 step_failed
              ~density:1~resource:resolve image_ir)in
            ignore(get_resource(replace image));
            let step_overlap=get(Prismel_execution.Private.begin_submission execution)in
            ignore(get(Prismel_execution.Private.lower_scene2 step_overlap
              ~density:1~resource:resolve image_ir));
            ignore(get_resource(replace image));
            expect_error"non-finite clear unexpectedly submitted"
              (Prismel_execution.Private.step~clear:(Float.nan,0.,0.,1.)
                step_failed[step_draws]);
            ignore(get_resource(replace image));
            expect_error"consumed failed submission was reusable"
              (Prismel_execution.Private.step step_failed[]);
            Prismel_execution.Private.cancel step_failed;
            Prismel_execution.Private.cancel step_overlap;
            let cross_first=get(Prismel_execution.Private.begin_submission execution)in
            let cross_batch=get(Prismel_execution.Private.lower_scene2 cross_first
              ~density:1~resource:resolve image_ir)in
            let cross_second=get(Prismel_execution.Private.begin_submission execution)in
            expect_error"cross-submission draw batch was accepted"
              (Prismel_execution.Private.step cross_second[cross_batch]);
            Prismel_execution.Private.cancel cross_first;
            ...
            let raising_resolver=function
              |1->Some(Prismel_execution.Image image)
              |2->incr second_lookups;
                  if !second_lookups>1 then raise Exit
                  else Some(Prismel_execution.Image image)
              |_->None in
            let exceptional=get(Prismel_execution.Private.begin_submission execution)in
            expect_error"raising resolver escaped the typed boundary"
              (Prismel_execution.Private.lower_scene2 exceptional
                ~density:1~resource:raising_resolver two_image_ir);
            let exception_probe=get(Prismel_execution.Private.begin_submission execution)in
            ignore(get(Prismel_execution.Private.lower_scene2 exception_probe
              ~density:1~resource:resolve image_ir));
            ignore(get_resource(replace image));
            Prismel_execution.Private.cancel exception_probe;
            let succeeded=get(Prismel_execution.Private.begin_submission execution)in
            let succeeded_batch=get(Prismel_execution.Private.lower_scene2 succeeded
              ~density:1~resource:resolve image_ir)in
            let expected=get_resource(Runtime_resources.Image.pixels image)in
            ignore(get(Prismel_execution.Private.step succeeded[succeeded_batch]));
            let submitted=get(Prismel_execution.capture execution)in
            ...
            ignore(get_resource(replace image));
            let replaced=get(Prismel_execution.Private.begin_submission execution)in
            let replaced_batch=get(Prismel_execution.Private.lower_scene2 replaced
              ~density:1~resource:resolve image_ir)in
            let expected=get_resource(Runtime_resources.Image.pixels image)in
            ignore(get(Prismel_execution.Private.step replaced[replaced_batch]));
            pixel(get(Prismel_execution.capture execution))0 expected
              "replaced image pixels did not reach the presented frame";
            ignore(get_resource(replace image));
            let labels=Array.init 45(fun index->
              get_resource(Runtime_resources.Image.create~width:1~height:1
                ~rgba:(Bytes.of_string
                  (String.init 4(function 0->Char.chr index|3->'\xff'|_->'\000')))))in
            Fun.protect
              ~finally:(fun()->Array.iter(fun label->
                ignore(Runtime_resources.Image.destroy label))labels)
              (fun()->
                ...
                and resolve_label id=
                  if id>=100&&id<145 then
                    Some(Prismel_execution.Image labels.(id-100))
                  else None in
                let open_submission()=
                  let submission=get(Prismel_execution.Private.begin_submission execution)in
                  ignore(get(Prismel_execution.Private.lower_scene2 submission
                ...
                Array.iteri(fun index label->
                  ...
                  ignore(get_resource(Runtime_resources.Image.replace label
                    ~width:1~height:1~rgba:(rgba((index+1)land 255))));
                  ignore(get_resource(Runtime_resources.Image.replace label
                    ~width:1~height:1~rgba:(rgba((index+2)land 255)))))labels;
                Prismel_execution.Private.cancel no_lease;
                let warm=open_submission()in
                Prismel_execution.Private.cancel warm;
                ...
                for _frame=1 to 600 do
                  ...
                  Prismel_execution.Private.cancel submission
                done;
                ...
                require(Prismel_execution.Private.snapshot_count_for_test execution<=256)
                  "image snapshot cache exceeded entry capacity";
                let last=labels.(44)in
                let blue=Bytes.of_string"\000\000\xff\xff"in
                ignore(get_resource(Runtime_resources.Image.replace last
                  ~width:1~height:1~rgba:blue));
                let mutation=get(Prismel_execution.Private.begin_submission execution)in
                let batch=get(Prismel_execution.Private.lower_scene2 mutation
                  ~density:1~resource:resolve_label labels_ir)in
                ignore(get(Prismel_execution.Private.step mutation[batch]));
                pixel(get(Prismel_execution.capture execution))0 blue
            ...
            let large=get_resource(Runtime_resources.Image.create~width:640
              ~height:480~rgba:large_a)in
            Fun.protect
              ~finally:(fun()->ignore(Runtime_resources.Image.destroy large))
              (fun()->
                let resolve_large=function
                  |1->Some(Prismel_execution.Image large)|_->None in
                let cache_before=Prismel_execution.Private.snapshot_count_for_test execution in
                Gc.full_major();
                let rss_before=rss_kib()and allocated_before=Gc.allocated_bytes()in
                let rss_middle=ref rss_before in
                for frame=1 to 600 do
                  ignore(get_resource(Runtime_resources.Image.replace large
                ...
                require(Prismel_execution.Private.snapshot_count_for_test execution=cache_before)
                  "large transient image entered the immutable snapshot cache";
                ignore(get_resource(Runtime_resources.Image.replace large
                  ~width:640~height:480~rgba:large_b));
                let submission=get(Prismel_execution.Private.begin_submission execution)in
                let batch=get(Prismel_execution.Private.lower_scene2 submission
                  ~density:1~resource:resolve_large image_ir)in
                ignore(get(Prismel_execution.Private.step submission[batch]));
                pixel(get(Prismel_execution.capture execution))0
            ...
            Fun.protect
              ~finally:(fun()->Array.iter(fun cached->
                ignore(Runtime_resources.Image.destroy cached))eviction_images)
              (fun()->Array.iter(fun cached->
                ignore(get(Prismel_execution.lower_scene2 execution~density:1
                  ~resource:(function 1->Some(Prismel_execution.Image cached)|_->None)
                  image_ir)))eviction_images;
                require(Prismel_execution.Private.snapshot_count_for_test execution=256)
                  "image snapshot cache did not evict to its exact entry capacity");
            let destroy_first=get(Prismel_execution.Private.begin_submission execution)in
            ignore(get(Prismel_execution.Private.lower_scene2 destroy_first
              ~density:1~resource:resolve image_ir));
            ignore(get_resource(replace image));
            let destroy_second=get(Prismel_execution.Private.begin_submission execution)in
            ignore(get(Prismel_execution.Private.lower_scene2 destroy_second
              ~density:1~resource:resolve image_ir));
            ignore(get_resource(replace image));
            ignore(get(Prismel_execution.destroy execution));
            ignore(get_resource(replace image));
            expect_error"destroyed coordinator began a submission"
              (Prismel_execution.Private.begin_submission execution);
            Prismel_execution.Private.cancel destroy_first;
            Prismel_execution.Private.cancel destroy_second);
        ignore(get(Prismel_execution.destroy execution));
      ...
      with exn->ignore(Prismel_execution.destroy execution);raise exn
