...
let ()=
  ...
  Input.update_mouse_pos 2 3;Input.press_mouse_button Input.LeftButton;Input.update_mouse_pos 4 8;
  let events=[Event.MousePressed(Input.LeftButton,(2,3));Event.MouseMoved(4,8)]in
  if Event.process_events events 0 (Some(fun count _->count+1))<>2
    ||Input.mouse_pos()<>(4,8)||Input.mouse_delta()<>(-1,1)then failwith"public event/input semantics";
