...
let prepare ~resources ~camera ~viewport:(x,y,width,height as viewport) scene =
  ...
  else let failure=ref None and entries=ref[]in List.iteri(fun number(drawing:Scene3.Private.drawing)->if !failure=None then let view=Mesh.Private.view drawing.mesh in match view.mode,drawing.mode,vertices view with
    |Mesh.Triangles,Scene3.Faces,Ok vertex_bytes when Array.length view.indices mod 3=0->let texture=match drawing.texture with None->Ok None|Some value->Result.map Option.some(resources.texture value)in let shadow=match Scene3.Private.shadows scene with []->Ok None|value::_->Result.map Option.some(resources.shadow value)in(match texture,shadow with Error error,_|_,Error error->failure:=Some error|Ok texture,Ok auxiliary->let mesh:Scene_execution.mesh={key=Printf.sprintf"scene3:%d:%s"number(Digest.to_hex(Digest.bytes vertex_bytes));vertices=vertex_bytes;vertex_count=Array.length view.vertices;indices=indices view.indices;index_count=Array.length view.indices}in let state:Scene_execution.state={viewport=(x,y,width,height);scissor=(x,y,width,height);cull=cull drawing.cull;depth_compare=comparison drawing.depth.comparison;depth_write=drawing.depth.write;depth_load=Ogpu.Render_pass.Load;depth_clear=Scene3.Private.depth_clear scene;transform_uniforms=Some(uniforms~camera~viewport scene drawing);stencil_state=None;stencil_load=Ogpu.Render_pass.Load;stencil_clear=Scene3.Private.stencil_clear scene}in let family=match auxiliary,texture with Some _,_->Scene_execution.Scene3_shadow|None,Some _->Scene3_textured|None,None->Scene3 in entries:={Scene_execution.family;blend=blend drawing.blend;texture;auxiliary;samples=Scene3.Private.samples scene;draw={mesh;state}}::!entries)
