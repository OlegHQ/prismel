...
let prepare ~resources:_ ~camera ~viewport:(x,y,width,height as viewport) scene =
  if width<=0||height<=0 then Error Invalid_viewport
    ...
    |Mesh.Triangles,Scene3.Faces,Ok vertex_bytes when Array.length view.indices mod 3=0->let mesh:Scene_execution.mesh={key=Printf.sprintf"scene3:%d:%s"number(Digest.to_hex(Digest.bytes vertex_bytes));vertices=vertex_bytes;vertex_count=Array.length view.vertices;indices=indices view.indices;index_count=Array.length view.indices}in let state:Scene_execution.state={viewport=(x,y,width,height);scissor=(x,y,width,height);cull=cull drawing.cull;depth_compare=comparison drawing.depth.comparison;depth_write=drawing.depth.write;depth_load=Ogpu.Render_pass.Load;depth_clear=Scene3.Private.depth_clear scene;transform_uniforms=Some(uniforms~camera~viewport scene drawing);stencil_state=None;stencil_load=Ogpu.Render_pass.Load;stencil_clear=Scene3.Private.stencil_clear scene}in entries:={Scene_execution.family=Scene3;blend=blend drawing.blend;texture=None;auxiliary=None;samples=Scene3.Private.samples scene;draw={mesh;state}}::!entries
