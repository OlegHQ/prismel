...
module Backend : sig
  ...
  val drain_web_events : unit -> web_event list
  ...
  val send_web_audio : web_audio_command -> unit
  val download_web_frame : filename:string -> (unit,string) result
  val add_web_text_input_region : x:int -> y:int -> width:int -> height:int -> focused:bool -> unit
end
