...
type t={root:string;watch:bool;images:(string,Image.t)Hashtbl.t;
  ...
  music:(string,Audio.Music.t)Hashtbl.t;mutable destroyed:bool}
let create ?(root=".")?(watch=false)()={root;watch;images=Hashtbl.create 32;
  fonts=Hashtbl.create 32;samples=Hashtbl.create 32;music=Hashtbl.create 32;destroyed=false}
...
let image value path=let path=resolve value path in cached value.images path(fun()->Image.load path)
let image_exn value path=match image value path with Ok result->result|Error message->failwith message
let font value ~size path=let path=resolve value path and key=Printf.sprintf"%s:%d"path size in cached value.fonts key(fun()->match Font.load path size with Ok result->Ok result|Error(`Msg message)->Error message)
let font_exn value ~size path=match font value ~size path with Ok result->result|Error message->failwith message
let sample value path=let path=resolve value path in cached value.samples path(fun()->Audio.Sample.load path)
let sample_exn value path=match sample value path with Ok result->result|Error message->failwith message
let music value path=let path=resolve value path in cached value.music path(fun()->Audio.Music.load path)
...
let refresh value=if value.watch then Ok[]else Ok[]
let clear value=Hashtbl.iter(fun _ item->Image.destroy item)value.images;
  ...
  Hashtbl.clear value.images;Hashtbl.clear value.fonts;Hashtbl.clear value.samples;Hashtbl.clear value.music
