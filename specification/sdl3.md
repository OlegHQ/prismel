...
| Blocking initial-domain call | `Sdl3.Event.wait` with a nonzero timeout | The stub releases the OCaml runtime system only around `SDL_WaitEventTimeout`.  Its timeout is copied by value, the reusable event union stays in native storage, no OCaml heap pointer is retained, and the runtime is reacquired before copying the active payload into OCaml.  A zero timeout does not release the runtime system. |
