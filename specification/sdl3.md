...
| --- | --- | --- |
| Pure or any-thread query | error printers; generated `Version` facts; `Version.validate`; `Version.linked`, `revision`, and extension version queries; `Thread.is_initial_domain`; `Thread.is_sdl_main_thread`; immutable handle generations and modes; TTF installed-font path discovery | These either do no native work or call an SDL function whose pinned header says it is safe from any thread.  They never acquire or destroy a resource. |
