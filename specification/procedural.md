...
let grid_node input p =
  Custom.map ~label:"deform" ~operation:"custom-deform"
    ...
    (fun ~parameters:_ ~context:_ geometry ->
      (* Call typed PDK operations here. *)
      Ok geometry)
```
...
`prismel.sop_catalog` contains inspectable catalog constructors. Its parameter
...
The generated `parameters_factory`
binds the same defaults and rebuild function used by inspection. Marking the
...
work to ordinary Procedural/PDK operators; controls are never synthesized from
...
outward-scale model while retaining Prismel's explicit packed render boundary:
...
the immutable parameter record, declared `Context`, and immutable PDK inputs.
...
- attributes/groups: typed constant float/int/vector/quaternion/color creation,
  ...
  while normalized Float4 quaternion output is explicitly a Prismel extension
