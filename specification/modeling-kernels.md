...
| Rewire Vertices | No former single owner | `Pdk.Mesh_edit_ops.rewire_vertices` owns owner-dependent selection promotion, recursive point-map resolution, corner-to-point mutation, newly-unused point compaction, and union corner-edge ancestry; Procedural stores only typed names/policies and never edits topology or retains mutable connectivity |
...
| Polygon bevel | No former single owner | `Pdk.Poly_modeling.poly_bevel` owns selected-edge eligibility, face-ring slide/collision planning, cross-face ring splits, connected continuation/corner topology, profile sampling, cardinality-first packed output, payload ancestry, and generated groups; Procedural only resolves a named native edge group and immutable parameters |
...
| Edge crease authoring | No former single owner | `Pdk.Mesh_edit_ops.crease` owns unique-edge reduction, coherent incident-corner `creaseweight` updates, and optional vertex-color endpoint visualization for the existing Subdivide kernel; Procedural resolves only a named topology-affine edge group and cache identity |
...
| Polygon-curve cutting | No former single owner | `Pdk.Poly_modeling.poly_cut` owns point/edge event classification, threshold interpolation, change subdivision, fragment planning, point compaction, every-owner payload/group/native-edge ancestry, and closed-fragment policy; Procedural resolves only named primitive/point/edge groups and immutable parameters |
...
| Edge cusp / Facet cusp | Formerly Facet-local fan splitting | One `Facet.split_points_on_edge_ends` packed kernel owns point-fan partitioning and one-to-many payload/group/native-edge ancestry; `Pdk.Edge_modeling_ops.edge_cusp` supplies explicit path-end masks while Facet supplies dihedral masks |
