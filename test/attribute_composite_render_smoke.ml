...
let surface color =
  Sop.grid ~columns:28 ~rows:18 ~size:4. ()
  |> Sop.set_color ~owner:Pdk.Attribute.Point color

let graph mode =
  let source = surface (Color.hex_exn "#f97316") in
  let target = surface (Color.hex_exn "#22d3ee")
      |> Sop.bend ~length:4. ~bend_angle:1.15 in
  let geometry = match mode with
    | `Source -> source
    | `Target -> target
    | `Composite -> source |> Sop.attribute_composite
        ~operation:Pdk.Ops.Composite_mean ~weight:0.42
        ~point_attributes:"P Cd" ~allow_position:true
        ~vertex_attributes:"^*" ~primitive_attributes:"^*"
        ~detail_attributes:"^*"
        ~inputs:[Sop.attribute_composite_input ~weight:0.58 target] in
  geometry |> Sop.normals ~owner:Pdk.Attribute.Vertex
...
let () =
  let keep = match Sys.getenv_opt "PRISMEL_KEEP_TEST_ARTIFACTS" with
    | Some value -> List.mem (String.lowercase_ascii value)
        ["1";"true";"yes";"on"]
    | None -> false in
  let root = Filename.temp_file "prismel-attribute-composite-render-" "" in
  ...
  let one_file = Filename.concat one "composite-000000.png"
  and four_file = Filename.concat four "composite-000000.png"
  and source_file = Filename.concat source "source-000000.png"
  and target_file = Filename.concat target "target-000000.png" in
  Fun.protect ~finally:(fun () ->
    if keep then Printf.printf "Attribute Composite render artifacts: %s\n%!" root
    else begin
      ...
      render one "composite" 1 (graph `Composite);
      render four "composite" 4 (graph `Composite);
      ...
      if not (String.equal one_png four_png) then
        failwith "Attribute Composite one/four-domain framebuffer pixels differ";
      if String.equal one_png source_png || String.equal one_png target_png then
        failwith "Attribute Composite visual output equals an endpoint";
      if String.length one_png < 1_000 then
        failwith "Attribute Composite PNG is unexpectedly empty");
  print_endline "Attribute Composite render smoke passed"
