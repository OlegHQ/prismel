...
let source () = Sop.polyline
    [|(-2.2,-0.6,0.);(-1.8,0.65,0.);(-0.1,-0.35,0.);
      (0.35,0.85,0.);(2.1,-0.2,0.)|]

let reference () = Sop.polyline
    [|(-2.2,-0.6,0.);(-1.2,0.15,0.);(-0.2,-0.1,0.);
      (0.8,0.4,0.);(2.1,-0.2,0.)|]

let graph ~relaxed =
  let curve = if relaxed then source () |> Sop.edge_relax
      ~reference:(reference ()) ~iterations:160 ~step_size:0.5 ~tolerance:1e-7
    else source () in
  curve
  ...
  |> Sop.set_color ~owner:Pdk.Attribute.Point
       (if relaxed then Color.hex_exn "#a3e635" else Color.hex_exn "#f97316")
...
let () =
  let keep = match Sys.getenv_opt "PRISMEL_KEEP_TEST_ARTIFACTS" with
    | Some value -> List.mem (String.lowercase_ascii value)
        ["1";"true";"yes";"on"]
    | None -> false in
  let root = Filename.temp_file "prismel-edge-relax-render-" "" in
  Sys.remove root; Unix.mkdir root 0o700;
  let one = Filename.concat root "one" and four = Filename.concat root "four"
  and control = Filename.concat root "control" in
  let one_file = Filename.concat one "relaxed-000000.png"
  and four_file = Filename.concat four "relaxed-000000.png"
  and control_file = Filename.concat control "control-000000.png" in
  Fun.protect ~finally:(fun () ->
    if keep then Printf.printf "Edge Relax render artifacts: %s\n%!" root
    else begin
      ...
      render one "relaxed" 1 (graph ~relaxed:true);
      render four "relaxed" 4 (graph ~relaxed:true);
      render control "control" 1 (graph ~relaxed:false);
      ...
      if not (String.equal one_png four_png) then
        failwith "Edge Relax one/four-domain framebuffer pixels differ";
      if String.equal one_png control_png then
        failwith "Edge Relax visual output equals the source control";
      if String.length one_png < 1_000 then
        failwith "Edge Relax PNG is unexpectedly empty");
  print_endline "Edge Relax render smoke passed"
